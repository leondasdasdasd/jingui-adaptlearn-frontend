import { cancelOpenMaicJob, createOpenMaicClassroom, getOpenMaicJob } from '../../lib/openMaicApi.js';
import { generateQuestions, reviewLessonContentQuality } from '../../lib/questionApi.js';
import {
...
import { readTeacherContent, writeTeacherContent } from '../data/teacherContentRepository.js';
...
function contentFromPublishedVersion(version, existing) {
  ...
  const normalized = normalizePublishedContentPackage(version.contentPackage || {});
  const questions = flattenPublishedQuestions(normalized);
  return {
    ...existing,
    lessonId: version.textbookLessonId || existing.lessonId,
    preQuestions: questions.filter((item) => String(item.purpose).toUpperCase() === 'PRE').map((item) => ({
      ...item,
      knowledgePointIds: item.knowledgePointIds || item.knowledgeObjectiveIds || [],
    })),
    postQuestions: questions.filter((item) => ['PRACTICE', 'POST'].includes(String(item.purpose).toUpperCase())).map((item) => ({
      ...item,
      knowledgePointIds: item.knowledgePointIds || item.knowledgeObjectiveIds || [],
    })),
    learningContent: normalized.learningContent || existing.learningContent,
    assessmentMatrices: normalized.assessmentMatrices || existing.assessmentMatrices || {},
...
async function prepareBrowserContent(lesson, existing, { signal } = {}) {
  ...
  const localIsNewerDraft = existing.status === 'draft'
    && existing.updatedAt
    && (!published.publishedAt || new Date(existing.updatedAt) > new Date(published.publishedAt));
  return localIsNewerDraft ? existing : contentFromPublishedVersion(published, existing);
}
...
export function getBrowserWholeLessonGenerationController() {
  ...
  singleton = createWholeLessonGenerationController({
    ...
    pollOpenMaicJob: (jobId, options) => resolveClassroomGeneration({ jobId }, {
