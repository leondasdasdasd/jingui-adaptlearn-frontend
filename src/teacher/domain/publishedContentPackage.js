...
const slotByDifficulty = { 1: 'D1_FOUNDATION', 2: 'D2_DIRECT', 3: 'D3_STANDARD', 4: 'D4_VARIANT', 5: 'D5_TRANSFER' };
...
function projectQuestionToTypeContract(question) {
  ...
  return Object.fromEntries(Object.entries(question || {}).filter(([field]) => (
    !QUESTION_TYPE_OWNED_FIELDS.has(field) || allowedContentFields.has(field)
  )));
}
...
function clonePublishedValue(value) {
  ...
  if (value && typeof value === 'object') {
    return Object.fromEntries(Object.entries(value).map(([key, item]) => [key, clonePublishedValue(item)]));
  }
  return value;
}
...
function approvedAssessmentMatrices(matrices = {}) {
  ...
  return Object.fromEntries(Object.entries(matrices || {}).map(([knowledgePointId, matrix]) => [
    knowledgePointId,
    {
...
export function restoreTeacherDraftFromPublishedVersion({
  ...
  const knowledgeQuestions = Object.values(normalized.knowledgePracticePools || {}).flat();
  const compositeQuestions = normalized.compositeReviewPool || [];

  return {
    lessonId: normalized.lesson?.id || '',
    preQuestions: diagnosticQuestions.map((question) => restoredQuestion(question, 'diagnostic')),
    postQuestions: [
      ...knowledgeQuestions.map((question) => restoredQuestion(question, 'knowledge')),
      ...compositeQuestions.map((question) => restoredQuestion(question, 'review')),
    ],
    learningContent: clonePublishedValue(contentPackage.learningContent || normalized.learningContent || {
      composite: null,
      knowledgePoints: [],
    }),
...
function normalizedQuestion(question, purpose, { omitPoolPartition = false } = {}) {
  const evidenceProfile = knowledgeEvidenceProfile(question);
  const normalized = {
    ...question,
    purpose,
    knowledgeObjectiveIds: question.knowledgePointIds || question.knowledgeObjectiveIds || [],
    ...
    blueprintSlotId: question.blueprintSlotId || slotByDifficulty[Number(question.difficulty)] || 'D3_STANDARD',
  ...
  return { ...projected, platformQuestion: toQuestionPlatformSerialized(projected) };
}
...
function uniqueQuestionList(questions = []) {
  ...
  return questions.map((question, index) => {
    ...
    return {
      ...question,
      id,
      platformQuestion: question.platformQuestion && typeof question.platformQuestion === 'object'
...
function buildKnowledgePracticePools(questions, knowledgePoints) {
  ...
  questions.filter((item) => item.phase !== 'review' && !isMisboundDiagnostic(item)).forEach((question) => {
  ...
  const result = Object.fromEntries(knowledgePoints.map((item) => [item.id, []]));
  byKnowledgePoint.forEach((items) => {
    ...
    const boundedItems = items.slice(0, PRACTICE_POOL_MAX_SIZE_PER_KNOWLEDGE_POINT);
    const blueprint = practicePoolBlueprint(knowledgePointId, boundedItems.length);
    ...
    const lockedItems = boundedItems.map((item, index) => {
      ...
      const slot = declaredSlot && !usedSlots.has(declaredSlot.id)
        ? declaredSlot
        : blueprint.find((candidate) => !usedSlots.has(candidate.id)) || blueprint[index] || null;
      if (slot) usedSlots.add(slot.id);
      if (!slot) return item;
      const hasPlannedBlueprint = Boolean(item.plannedQuestionType && item.assessmentFocus);
      return {
        ...item,
        blueprintSlotId: slot.id,
        difficulty: hasPlannedBlueprint ? item.difficulty : slot.difficulty,
        adaptiveRole: hasPlannedBlueprint ? item.adaptiveRole : slot.adaptiveRole,
      };
    });
    result[knowledgePointId] = uniqueQuestionList(lockedItems)
      .map((item) => normalizedQuestion(item, 'PRACTICE', { omitPoolPartition: true }));
  });
  return result;
}
...
function buildCompositeReviewPool(questions) {
  ...
  return reviewItems.slice(0, blueprint.length).map((item, index) => {
    ...
    return normalizedQuestion({
      ...
      blueprintSlotId: slot.id,
      difficulty: hasPlannedBlueprint ? item.difficulty : slot.difficulty,
      taskCategory: hasPlannedBlueprint ? item.taskCategory : slot.taskCategory,
      adaptiveRole: hasPlannedBlueprint ? item.adaptiveRole : slot.adaptiveRole,
      recommendedQuestionTypes: hasPlannedBlueprint
        ? item.recommendedQuestionTypes : slot.recommendedQuestionTypes,
    }, 'POST');
  });
}

export function buildPublishedContentPackage({ lesson, content, coveredKnowledgeObjectiveIds = [] }) {
  const composite = content.learningContent?.composite || content.openMaic || {};
  const compositeCoverage = coveredKnowledgeObjectiveIds.length
  ...
  const compositeReady = Boolean(composite.classroomUrl) && !composite.partial && composite.status !== 'partial';
  if (!compositeReady) unavailableItems.push('COMPOSITE_OPENMAIC');
  lesson.knowledgePoints.forEach((knowledgePoint) => {
    const runtime = knowledgeContent.find((item) => item.knowledgeObjectiveId === knowledgePoint.id)?.openMaic;
    if (!runtime?.classroomUrl || runtime.partial || runtime.status === 'partial') unavailableItems.push(`KNOWLEDGE_OPENMAIC:${knowledgePoint.id}`);
  });
  const diagnosticQuestionPool = uniqueQuestionList(content.preQuestions || []).map((item) => normalizedQuestion(item, 'PRE'));
  const knowledgePracticePools = buildKnowledgePracticePools(content.postQuestions || [], lesson.knowledgePoints);
  const compositeReviewPool = buildCompositeReviewPool(content.postQuestions || []);
  ...
  const normalizedPracticePools = Object.fromEntries(Object.entries(knowledgePracticePools).map(([id, pool]) => [id, pool.map(globallyUnique)]));
  ...
  return {
    ...
    learningContent: {
      ...
      knowledgePoints: lesson.knowledgePoints.map((knowledgePoint) => {
        const runtime = knowledgeContent.find((item) => item.knowledgeObjectiveId === knowledgePoint.id)?.openMaic || {};
        const runtimeReady = Boolean(runtime.classroomUrl) && !runtime.partial && runtime.status !== 'partial';
        return {
