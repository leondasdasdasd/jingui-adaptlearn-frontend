...
import {
  DEFAULT_LEARNING_GENERATION_POLICY,
...
export const DEFAULT_MULTI_LESSON_GENERATION_POLICY = DEFAULT_LEARNING_GENERATION_POLICY;
export const normalizeMultiLessonGenerationPolicy = normalizeLearningGenerationPolicy;
...
function uniqueBy(items, keyOf, conflictMessage) {
  ...
  items.forEach((item) => {
    ...
    if (current && JSON.stringify(current) !== JSON.stringify(item)) throw new Error(conflictMessage(item));
...
function questionKey(question) {
  return String(question.id || '').trim() || String(question.stem || '').replace(/\s+/g, '');
}
...
export function hasReusableKnowledgeAssets(version) {
  ...
  const runtimes = new Map((content?.learningContent?.knowledgePoints || [])
    .map((item) => [item.knowledgeObjectiveId, item.openMaic]));
...
export function validateSelectedLessonVersions(versions = []) {
  ...
  versions.forEach((version) => {
    if (!version?.id || !version?.textbookLessonId || !version?.contentPackage) {
...
export function multiLessonGenerationLessonId(versions = []) {
  const sourceIds = versions
    .map((version) => String(typeof version === 'string' ? version : version?.id || '').trim())
    .filter(Boolean)
    .slice(0, MAX_LESSONS);
  const compactIds = sourceIds.map((id) => id.replace(/[^a-zA-Z0-9_-]/g, '').slice(0, 12) || 'source');
  return `multi:${compactIds.join(':') || 'lesson'}`.slice(0, GENERATION_LESSON_ID_MAX_LENGTH);
}
...
export function sourceLessonSummaries(versions = [], lessonById = {}) {
  return versions.map((version, index) => ({
    ...
    title: lessonById[version.textbookLessonId]?.title || version.contentPackage?.lesson?.title || '教材课时',
...
export function mergeSourceLessonAssets(versions = [], lessonById = {}) {
  ...
  const knowledgeRuntimes = uniqueBy(
    normalized.flatMap(({ content }) => content.learningContent?.knowledgePoints || []),
    (item) => item.knowledgeObjectiveId,
    (item) => `知识点“${objectives.find((value) => value.id === item.knowledgeObjectiveId)?.name || item.knowledgeObjectiveId}”的学习内容版本不一致`,
  );
  const knowledgePracticePools = {};
  normalized.forEach(({ version, lessonId, content }) => {
    Object.entries(content.knowledgePracticePools || {}).forEach(([knowledgePointId, questions]) => {
      knowledgePracticePools[knowledgePointId] ||= [];
      const existing = new Set(knowledgePracticePools[knowledgePointId].map(questionKey));
...
export function buildMultiLessonContentPackage({
  ...
  if (enabled.compositeExplanation && (!classroom?.classroomId || !classroom?.classroomUrl)) {
    throw new Error('课堂学习还没有准备好');
  }
  if (enabled.preAssessment && !diagnosticQuestions?.length) throw new Error('课前测验还没有准备好');
  if (enabled.postAssessment && !reviewQuestions?.length) throw new Error('课时巩固还没有准备好');
  return {
