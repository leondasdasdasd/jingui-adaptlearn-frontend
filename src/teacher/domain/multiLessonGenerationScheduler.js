...
const QUESTION_POOL_ALIASES = new Set(['questions', 'quality', 'quality_check', 'repair']);
...
function normalizeLimits(overrides = {}, defaults, label) {
  return Object.fromEntries(POOL_NAMES.map((pool) => [
    pool,
    positiveInteger(
      overrides[pool]
        ?? (pool === GENERATION_RESOURCE_POOLS.QUESTIONS ? overrides.quality : undefined)
        ?? defaults[pool],
      `${label}.${pool}`,
    ),
  ]));
...
function normalizePerLessonLimits(overrides = {}, poolLimits) {
  return Object.fromEntries(POOL_NAMES.map((pool) => {
    ...
    const legacyQualityExplicit = pool === GENERATION_RESOURCE_POOLS.QUESTIONS
      && !explicit
...
function canonicalPool(value) {
  ...
  if (QUESTION_POOL_ALIASES.has(normalized)) return GENERATION_RESOURCE_POOLS.QUESTIONS;
  if (normalized === GENERATION_RESOURCE_POOLS.OPENMAIC) return GENERATION_RESOURCE_POOLS.OPENMAIC;
  return '';
}
...
function taskSnapshot(task) {
  return {
    ...
    pool: task.pool,
    status: task.status,
    queuePosition: task.status === GENERATION_TASK_STATUSES.QUEUED ? task.queuePosition : null,
    metadata: task.metadata,
    createdAt: task.createdAt,
    startedAt: task.startedAt,
    finishedAt: task.finishedAt,
    value: task.status === GENERATION_TASK_STATUSES.COMPLETED ? task.value : undefined,
    error: task.error,
  };
}
...
export function createMultiLessonGenerationScheduler({
  ...
  const perLessonLimits = normalizePerLessonLimits(perLessonConcurrency, poolLimits);
  const pools = Object.fromEntries(POOL_NAMES.map((pool) => [
    pool,
  ...
  function addLessonToPool(poolState, lessonId) {
    ...
    if (!poolState.lessonOrder.includes(lessonId)) poolState.lessonOrder.push(lessonId);
  }
  ...
  function contendingLessonCount(poolState) {
    return poolState.lessonOrder.filter((lessonId) => (
  ...
  function refreshQueuePositions(poolName) {
    ...
    const queues = new Map(poolState.lessonOrder.map((lessonId) => [
      lessonId,
      queuedTasksForLesson(poolState, lessonId),
    ]));
    let cursor = poolState.cursor;
    let position = 1;
    let remaining = [...queues.values()].reduce((sum, queue) => sum + queue.length, 0);
  ...
  function isIdle() {
    return POOL_NAMES.every((poolName) => {
      ...
      return ![...tasks.values()].some((task) => (
        task.pool === poolName && task.status === GENERATION_TASK_STATUSES.QUEUED
      ));
    });
  }
  ...
  function resolveIdleWaiters() {
    ...
    idleWaiters.forEach((resolve) => resolve(snapshot));
    idleWaiters.clear();
  }
  ...
  function releaseActiveSlot(task) {
    ...
    const nextCount = Math.max(0, activeCountForLesson(poolState, task.lessonId) - 1);
  ...
  function startTask(task) {
    ...
    Promise.resolve()
      ...
      .then(
        ...
        (error) => {
          const canceled = task.controller.signal.aborted || error?.name === 'AbortError';
          finishTask(
            task,
            canceled ? GENERATION_TASK_STATUSES.CANCELED : GENERATION_TASK_STATUSES.FAILED,
            error,
          );
        },
      );
  ...
  function normalizeTask(input) {
    ...
    sequence += 1;
    const id = String(input.id || (moduleId
      ? `${lessonId}:${moduleId}`
      : `${lessonId}:${pool}:${sequence}`));
    if (tasks.has(id) || reservedTaskIds.has(id)) throw new Error(`生成任务 id 重复：${id}`);
  ...
  function enqueueTasks(inputs = []) {
    ...
    try {
      ...
      tasks.forEach((_task, taskId) => reservedTaskIds.add(taskId));
      throw error;
    }
    ...
    touchedPools.forEach((poolName) => refreshQueuePositions(poolName));
    normalized.forEach((task) => {
  ...
  function taskInputsForLesson(lessonOrConfig, maybeTasks) {
    const config = typeof lessonOrConfig === 'string'
    ...
    const lessonTasks = config.tasks || [
      ...(config.questions || []).map((task) => ({ ...task, pool: 'questions' })),
      ...(config.openmaic || []).map((task) => ({ ...task, pool: 'openmaic' })),
      ...(config.quality || []).map((task) => ({ ...task, pool: 'quality' })),
      ...(config.quality_check || []).map((task) => ({ ...task, pool: 'quality_check' })),
  ...
  function enqueueLessons(lessonConfigs = []) {
    return enqueueTasks(lessonConfigs.flatMap((config) => taskInputsForLesson(config)));
  }
  ...
  function cancelLesson(lessonId, reason = '') {
    ...
    tasks.forEach((task) => {
      if (task.lessonId !== normalizedLessonId) return;
      if (cancelTask(task, reason)) {
        canceledCount += 1;
        touchedPools.add(task.pool);
      }
    });
    touchedPools.forEach((poolName) => {
      refreshQueuePositions(poolName);
      drainPool(poolName);
    });
    resolveIdleWaiters();
