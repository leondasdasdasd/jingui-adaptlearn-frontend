...
function compactMessages(messagesByScope) {
  return Object.fromEntries(Object.entries(scopedObject(messagesByScope)).map(([scope, messages]) => {
    const compacted = (Array.isArray(messages) ? messages : []).slice(-MAX_MESSAGES_PER_SCOPE).map((message) => ({
      id: String(message?.id || ''),
      role: message?.role === 'user' ? 'user' : 'assistant',
      text: String(message?.text || '').replace(/。。+/g, '。'),
    })).filter((message) => message.id && message.text
      && !/^后台任务 .+ 没有全部完成：正在把整课任务写入数据库/.test(message.text));
    const failedRunPrefixes = new Set(compacted.flatMap((message) => {
      const match = message.text.match(/^后台任务 (\S+) 没有全部完成/);
      return match ? [match[1]] : [];
    }));
    return [scope, compacted.filter((message) => {
...
export function normalizeTeacherAgentSession(lessonId, value) {
  if (!value || value.lessonId !== lessonId) return emptyTeacherAgentSession(lessonId);
  return {
    ...
    stepStatusesByScope: Object.fromEntries(
      Object.entries(scopedObject(value.stepStatusesByScope)).map(([scope, statuses]) => [
        scope,
        Object.fromEntries(Object.entries(scopedObject(statuses)).map(([stepId, status]) => [
          stepId,
...
export function readTeacherAgentSession(lessonId) {
  return normalizeTeacherAgentSession(
    lessonId,
    readJson(teacherStorageKey(storageKeys.teacherAgentSession(lessonId)), null),
  );
}
...
export function writeTeacherAgentSession(lessonId, value) {
  ...
  return writeJson(teacherStorageKey(storageKeys.teacherAgentSession(lessonId)), session);
}
