...
/**
 * 将内部 D1-D5 编码转换为面向用户的纯星级文案。
 * @param {string | number} difficulty 原始难度
 * @returns {string} 本地化星级名称
 */
 ...
 * @param difficulty
 */
export function difficultyStarCount(difficulty) {
  return Number(localizedDifficultyLevel(difficulty).slice(1));
}
 ...
 * @param difficulty
 */
function localizedDifficultyLevel(difficulty) {
  return difficulty == null || String(difficulty).trim() === ""
    ? "D3"
    : normalizeDifficulty(difficulty);
}
...
export function difficultyBadgeClassName(difficulty) {
  return difficultyClassNames[`D${difficultyStarCount(difficulty)}`];
}
