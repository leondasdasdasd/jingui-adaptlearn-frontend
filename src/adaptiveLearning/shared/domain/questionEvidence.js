...
function normalizeChoiceId(question, value) {
  ...
  if (direct) return normalizeAnswerText(getOptId(direct));

  const byText = options.find((option) => {
    const text = normalizeAnswerText(getOptText(option));
  ...
  if (ordinal) {
    ...
    if (getOptId(option)) return normalizeAnswerText(getOptId(option));
  }
  return raw;
}
