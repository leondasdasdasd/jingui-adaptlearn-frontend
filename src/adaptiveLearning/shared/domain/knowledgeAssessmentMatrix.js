import {
  ...
  assessmentPolicy,
  assessmentPolicyIds,
} from "./assessmentMatrixPolicy.js";
...
/**
 * 仅识别有明确数学领域坐标的旧矩阵；科学/物理缺策略时必须失败，禁止静默套用数学。
 * @param {object} rawMatrix 原始矩阵。
 * @returns {string} 已校验策略 ID。
 */
function assessmentPolicyIdFromMatrix(rawMatrix) {
  const explicit = compactText(
    rawMatrix.assessmentPolicyId || rawMatrix.policyVersion,
  );
  if (explicit) return assessmentPolicy(explicit).id;
  const domains = (Array.isArray(rawMatrix.cells) ? rawMatrix.cells : [])
    .map((cell) => compactText(cell?.domain).toUpperCase())
    .filter(Boolean);
  const legacyMathDomains = new Set(["CR", "PJ", "M", "SF"]);
  if (
    domains.length > 0 &&
    domains.every((domain) => legacyMathDomains.has(domain))
  ) {
    return ASSESSMENT_POLICY_IDS.math;
  }
  throw new Error("评估矩阵缺少明确的策略版本");
}
...
export function normalizeKnowledgeAssessmentMatrix(rawMatrix = {}) {
  const scopeId = compactText(rawMatrix.scopeId || rawMatrix.knowledgePointId);
  const knowledgePointIds = [
    ...new Set(
      (Array.isArray(rawMatrix.knowledgePointIds)
        ? rawMatrix.knowledgePointIds
        : [rawMatrix.knowledgePointId]
      )
        .map((value) => compactText(value))
        .filter(Boolean),
    ),
  ];
  const assessmentPolicyId = assessmentPolicyIdFromMatrix(rawMatrix);
  return {
    assessmentPolicyId,
    policyVersion: assessmentPolicyId,
    scopeId,
    ...
    cells: (Array.isArray(rawMatrix.cells) ? rawMatrix.cells : []).map(
      (rawCell) => {
        ...
        return {
          matrixCellId: assessmentMatrixCellId(scopeId, domain, targetLevel),
          domain,
          targetLevel,
          ...
          variationRequirements: compactStringList(
            rawCell?.variationRequirements || [rawCell?.variationRequirement],
            4,
          ),
...
export function validateKnowledgeAssessmentMatrices(
  ...
  const matrices = rawMatrices.map((matrix) =>
    normalizeKnowledgeAssessmentMatrix(matrix),
  );
  const knownIds = [
    ...new Set(
  ...
  const allowedPolicyIds = new Set(assessmentPolicyIds());
  ...
  for (const [matrixIndex, matrix] of matrices.entries()) {
    ...
    const policy = assessmentPolicy(matrix.assessmentPolicyId);
    const allowedDomains = new Set(policy.domains.map((item) => item.id));
    if (!allowedPolicyIds.has(matrix.assessmentPolicyId))
      errors.push(
        issue(
          "INVALID_ASSESSMENT_POLICY",
          `${path}.assessmentPolicyId`,
          `未知评估策略 ${matrix.assessmentPolicyId}`,
        ),
      );
    ...
    for (const [cellIndex, cell] of matrix.cells.entries()) {
      ...
      if (!allowedDomains.has(cell.domain))
        errors.push(
          issue(
            "INVALID_DOMAIN",
            `${cellPath}.domain`,
        ...
        cells: [
          {
            ...
            variationRequirements: ["更换表示方式或边界条件后仍能准确转换"],
          ...
          },
          {
            ...
            variationRequirements: ["更换真实情境和信息表达后仍能建立同类关系"],
...
function sortedActiveCells(matrix) {
  const policy = assessmentPolicy(matrix.assessmentPolicyId);
  const policyDomainOrder = new Map(
    policy.domains.map((domain, index) => [domain.id, index]),
  );
  return [...matrix.cells].sort(
    (left, right) =>
      roleWeight[right.role] - roleWeight[left.role] ||
      (policyDomainOrder.get(left.domain) ??
        domainOrder.get(left.domain) ??
        99) -
        (policyDomainOrder.get(right.domain) ??
          domainOrder.get(right.domain) ??
          99) ||
...
export function practicePoolBlueprintFromAssessmentMatrix(
  ...
  return baseSlots.map((slot, index) => {
    ...
    return {
      id: slot.id,
      knowledgePointId: slot.knowledgePointId,
      difficulty,
      questionType,
