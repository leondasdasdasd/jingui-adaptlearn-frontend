...
export default function AssessmentMatrixCellDetail({ cell }) {
  const domain = assessmentDomains(trans, cell.assessmentPolicyId).find(
    (item) => item.id === cell.domain,
  );
  const level = assessmentLevels(trans, cell.assessmentPolicyId).find(
    (item) => item.id === cell.level,
  );
      ...
      <div className="assessment-matrix-detail-grid">
        <div className="assessment-matrix-detail-main">
          ...
          <section>
            <h4>
              {trans(
                "adaptiveLearning.assessment.variationRequirements",
                "变化要求",
              )}
            </h4>
            {cell.variationRequirements.length > 0 ? (
              <ul>
                {cell.variationRequirements.map((item, index) => (
              ...
              <p className="assessment-matrix-empty-copy">
                {trans(
                  "adaptiveLearning.assessment.missingVariationRequirements",
                  "尚未填写变化要求",
                )}
              </p>
            )}
...
AssessmentMatrixCellDetail.propTypes = {
  cell: PropTypes.shape({
    ...
    variationRequirements: PropTypes.arrayOf(PropTypes.string).isRequired,
