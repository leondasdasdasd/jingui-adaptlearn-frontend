...
import { canonicalizeAssessmentTypeSlotsInContent } from "../domain/assessmentTypeSlotMigration.js";
...
export function readTeacherContent() {
  ...
  return canonicalizeTeacherContentCollection(
    normalizeTeacherContentRuntimes(normalizeLessonContent(merged)),
  );
}
 ...
 * @param content
 */
function canonicalizeTeacherContentCollection(content) {
  return Object.fromEntries(
    Object.entries(content).map(([lessonId, lessonContent]) => [
      lessonId,
      canonicalizeAssessmentTypeSlotsInContent(lessonContent),
    ]),
  );
}
...
export function writeTeacherContent(content) {
  const stored = writeJson(
    ...
    canonicalizeTeacherContentCollection(content),
  );
  if (stored)
