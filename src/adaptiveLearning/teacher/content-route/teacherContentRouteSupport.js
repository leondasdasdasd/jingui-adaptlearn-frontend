...
const SUBJECT_SCOPE_IDS = Object.freeze({
  数学: "math",
  科学: "science",
  物理: "physics",
});
const PUBLISHER_SCOPE_IDS = Object.freeze({
  浙教版: "zhejiang",
  人教版: "pep",
  北师大版: "bnup",
  苏科版: "sukeh",
 ...
 * @param definitions
 */
function mappedScopeId(explicitId, localizedName, definitions) {
  return String(explicitId || definitions[localizedName] || "");
}
 ...
 * @param lesson
 */
function inferredQuestionSourceScope(lesson) {
  const course = lesson?.course || {};
  const grade = String(course.gradeKey || lesson?.gradeKey || "");
  return {
    subject: mappedScopeId(
      course.subjectKey,
      lesson?.subject || course.subject,
      SUBJECT_SCOPE_IDS,
    ),
    publisher: mappedScopeId(
      course.publisherKey,
      lesson?.publisher || course.publisher,
      PUBLISHER_SCOPE_IDS,
    ),
    grade,
    volume: String(course.volume || (grade.endsWith("-down") ? "down" : "up")),
  };
}

/**
 * 把真实目录与内置数学目录统一映射为题源业务范围，平台数字 ID 仍由 repository 查询。
 * @param {object} lesson - 带课程上下文的当前课时。
 * @returns {object} 稳定的学科、教材、年级和册别范围。
 */
export function questionSourceScopeForLesson(lesson) {
  const explicit = lesson?.course?.questionSourceScope;
  return explicit?.subject && explicit?.grade
    ? explicit
    : inferredQuestionSourceScope(lesson);
}
