...
import { assessmentPolicyIdForSubject } from "../../shared/domain/assessmentMatrixPolicy";
import { normalizeKnowledgeAssessmentMatrix } from "../../shared/domain/knowledgeAssessmentMatrix";
...
} from "../domain/assessmentTypeSlot";
import { appendQuestionBoundToSlot } from "../domain/slotQuestionAppend";
import { createAssessmentSlotActions } from "./createAssessmentSlotActions";
import { slotQuestionAppendIssueMessage } from "./slotQuestionAppendPresentation";
import {
...
export function createTeacherContentQuestionActions({
  ...
  const resolveKnowledgeAssessmentContext = (knowledgePointId) => {
    ...
    const knowledgePoints = isComposite
    ...
    return {
      ...
      knowledgePoint,
      knowledgePoints,
      assessmentPolicyId: assessmentPolicyIdForSubject(
        lesson.subject || lessonPayload.subject,
      ),
      sourceContent,
  ...
  const generateKnowledgePointAssessmentMatrix = async (knowledgePointId) => {
    ...
    try {
      ...
      const response = await generateAssessmentMatrices({
        lesson: lessonPayload,
        scopeId: knowledgePointId,
        knowledgePointIds: knowledgePoints.map((item) => item.id),
        assessmentPolicyId,
        knowledgePoints,
        teacherInstruction: isComposite
          ? "生成整课综合能力的稀疏评估矩阵，覆盖整课各知识点的综合应用、问题解决与迁移建构，不生成题目。每个矩阵格必须一次性给出目标行为、证据标准和变化要求。"
          : "独立生成该知识点的稀疏评估矩阵，不生成题目。每个矩阵格必须一次性给出目标行为、证据标准和变化要求。",
      });
      const rawAssessmentMatrix =
        response.assessmentMatrix ||
        response.assessmentMatrices?.[knowledgePointId] ||
        response.assessmentMatrices?.composite;
      const assessmentMatrix = rawAssessmentMatrix
  ...
  const generateKnowledgePointQuestionSlots = async (knowledgePointId) => {
    ...
    try {
      ...
      const rawSlots =
        response.assessmentQuestionSlots?.[knowledgePointId] ||
        response.assessmentQuestionSlots?.composite ||
        [];
      const slots = prepareAssessmentTypeSlots({
        assessmentPolicyId,
        composite: isComposite,
        knowledgePointIds: isComposite
          ? knowledgePoints.map((item) => item.id)
          : [knowledgePoint.id],
        matrix: assessmentMatrix,
        questions: sourceContent.postQuestions || [],
        rawSlots,
      });
      if (slots.length === 0)
        throw new Error(
  ...
  const generateKnowledgePointQuestionPool = async (
    knowledgePointId,
    targetSlotId = "",
  ) => {
    ...
    const retryingPartialRun =
      ...
      retrySlotIds.size > 0;
    const slotsToGenerate = targetSlotId
      ? blueprintSlots.filter(
          (slot) => String(slot.id) === String(targetSlotId),
        )
      : retryingPartialRun
    ...
    if (slotsToGenerate.length === 0) {
      setNotice(
        noticeMessage(
          "error",
          trans(
            "adaptiveLearning.assessment.targetSlotMissing",
            "目标题目插槽不存在",
          ),
        ),
      );
    ...
    const initialSlots = (targetSlotId ? slotsToGenerate : blueprintSlots).map(
      (slot) => {
        ...
        await generateQuestionSlotsConcurrently(
          {
            ...
            poolBlueprintSlots: assessmentQuestionGenerationSlots({
  ...
  const assessmentSlotActions = createAssessmentSlotActions({ saveDraft });
