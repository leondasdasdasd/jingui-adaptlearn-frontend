      ...
      return current < required
        ? [
            taskIssue(
              ...
              `${task.label}的${difficulty}星题有 ${current} 道，还需补充 ${required - current} 道`,
            ),
          ]
        : [];
    },
...
function expectedSlotIssues(task, slot, slotsById) {
  ...
  return [
    taskIssue(
      ...
      `${slot.primaryKnowledgePointId} 的 ${slot.diagnosticRole} 槽位应为 ${difficultyNumber(slot.difficulty)} 星难度`,
      question.id,
      blueprintSlotDetails(slot),
    ),
  ];
