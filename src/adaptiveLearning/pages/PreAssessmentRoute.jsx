...
import { scopedQuizDraftId } from "../shared/domain/learningSessionPolicy";
import { isTeacherLessonPreview } from "../student/application/teacherLessonPreview";
...
export default function PreAssessmentRoute() {
  ...
  const historicalMastery = useMemo(
    () =>
      prepareHistoricalMasteryForPolicy({
        ...
        historicalMastery: isTeacherLessonPreview(session)
          ? {}
          : {
  ...
  if (error) {
    return (
      <GeneratingPage
        ...
        onBack={() => {
          if (isTeacherLessonPreview(session)) {
            exitLearningSession(routes.directory);
          } else {
            resetSession();
  ...
  return (
    <QuizPage
      draftId={scopedQuizDraftId(
        session.selection,
        `pre:${session.selection.contentVersionId || session.selection.section.id}`,
      )}
      mode="pre"
      lessonTitle={session.selection.section.title}
      questions={session.preQuestions}
      ...
      onComplete={complete}
      onExit={() => exitLearningSession(routes.directory)}
    />
  );
}
