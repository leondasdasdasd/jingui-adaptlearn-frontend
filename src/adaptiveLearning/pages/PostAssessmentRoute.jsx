...
import { questionsForLearningUnit } from "../student/application/learningUnitQuestionMapper.js";
...
export default function PostAssessmentRoute() {
  ...
  const questions = questionsForLearningUnit(context, unit).map((question) =>
  ...
  if (questions.length === 0) {
    return (
      <LearningResourceStatePage
        ...
        onBack={() => exitLearningSession(routes.directory)}
      />
    );
  }
  ...
  return (
    <QuizPage
      ...
      draftId={scopedQuizDraftId(
        selection,
        `post:${selection.contentVersionId || selection.section.id}:${unit?.id || "legacy"}`,
      )}
      mode="post"
      lessonTitle={selection.section.title}
      questions={questions}
