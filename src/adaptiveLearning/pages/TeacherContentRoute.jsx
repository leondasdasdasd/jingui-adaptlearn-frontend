...
import { createTeacherLessonPreviewSession } from "../student/application/teacherLessonPreview";
...
} from "../teacher/content-route/teacherContentRouteSupport";
...
import { readCachedCurriculumLesson } from "../teacher/data/curriculumCatalogRepository";
...
export default function TeacherContentRoute() {
  ...
  const lesson = useMemo(
    () => readCachedCurriculumLesson(lessonId) || findLessonById(lessonId),
    [lessonId],
  );
  ...
  const startTeacherPreview = useCallback(() => {
    if (!selectedPublishedVersion || startingPreview) return;
    setStartingPreview(true);
    const returnPath = `/adaptive-learning/teacher/textbook-lessons/${encodeURIComponent(lesson.id)}/content`;
    try {
      startTransientSession(
        createTeacherLessonPreviewSession({
          lesson,
          publishedVersion: selectedPublishedVersion,
          returnPath,
        }),
      );
      ...
      setNotice(
        trans(
          "adaptiveLearning.content.previewStartFailed",
          "无法启动试做，请检查浏览器存储权限后重试。",
        ),
      );
    }
  }, [
  ...
  const viewModel = {
    ...
    publishStatus,
    publishing,
    questionSourceScope: questionSourceScopeForLesson(lesson),
    reviewQuestions,
    selectedKpId,
    selectedPublishedVersion,
    startingPreview,
