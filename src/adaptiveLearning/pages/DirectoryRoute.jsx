...
export default function DirectoryRoute() {
  ...
  const enterClassroom = useCallback(
    async (periodId, accessToken) => {
      ...
      setSession({
        ...
        publishedContent: {
          assessmentMatrices: publishedLesson.assessmentMatrices,
  ...
  useEffect(() => {
    ...
    getStudentLearningPeriods(accessToken, { signal: controller.signal })
      .then((payload) => {
        const items = normalizeStudentClassroomDirectory(
          payload,
          activeCourse,
        ).map((item) => ({
          ...item,
  ...
  const start = async (selection, { entryMode = "assessment_first" } = {}) => {
    const isNewLesson = entryMode === "new_lesson";
    clearAllQuizDrafts();
    setEntryState({ loading: true, error: "" });
    ...
    if (fixedIdentity) {
      try {
        serverSession = await createSelfStudySession(
          ...
          { learningMode: isNewLesson ? "NEW_LESSON" : "ASSESSMENT_FIRST" },
        );
    ...
    const newLessonPlan = isNewLesson
      ? createNewLessonLearningPlan(
          availableKnowledgePoints,
          unseenPublished.generationPolicy,
        )
      : null;
    const learningFlow = newLessonPlan
      ? createLessonLearningFlow(newLessonPlan)
      : emptySession.learningFlow;
    setSession({
      ...emptySession,
      selection: {
        ...selection,
      ...
      publishedContent: {
        assessmentMatrices: unseenPublished.assessmentMatrices,
    ...
    navigate(
      isNewLesson
        ? routeForLearningUnit(
            activeLearningUnit(learningFlow),
            routes.complete,
          )
        : routes.preAssessment,
    );
