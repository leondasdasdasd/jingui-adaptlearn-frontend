...
function sourceLabel(source) {
  ...
  if (source === "recommendation" || source === "recommended") return "系统推荐";
  return "";
}
  ...
  const filteredChapters = useMemo(() => {
    ...
    return course.chapters
      .map((chapter) => {
        ...
        // 计算该章节的统计
        const chapterAllKp = chapter.sections.flatMap(
          (s) => s.knowledgePoints,
        );
  ...
  const totalMatchedPoints = useMemo(
    () =>
      filteredChapters.reduce(
        (sum, ch) => sum + ch.matchedPointsCount,
        0,
      ),
    [filteredChapters],
  );
  ...
  return (
    ...
    >
      <div className="km-page-wrapper">
        {/* 顶部视图切换：知识点掌握 vs 学习记录 */}
        <div className="km-view-switcher-bar">
          <div
            className="km-nav-tab-strip"
            role="tablist"
            aria-label="学习进度视图"
          >
            <button
              ...
              className={`km-nav-tab-btn ${activeTab === "mastery" ? "active" : ""}`}
            ...
            <button
              ...
              className={`km-nav-tab-btn ${activeTab === "history" ? "active" : ""}`}
              ...
              >
                ...
                >
                  ...
                  <span className="km-filter-pct">
                    ({totalCount ? Math.round((masteredCount / totalCount) * 100) : 0}%)
                  </span>
                </button>
                <button
                  type="button"
                ...
                >
                  ...
                  <span className="km-filter-pct">
                    ({totalCount ? Math.round((studyingCount / totalCount) * 100) : 0}%)
                  </span>
                </button>
                <button
                  type="button"
                ...
                >
                  ...
                  <span className="km-filter-pct">
                    ({totalCount ? Math.round((needsReviewCount / totalCount) * 100) : 0}%)
                  </span>
                </button>
                <button
                  type="button"
                ...
                >
                  ...
                  <span className="km-filter-pct">
                    ({totalCount ? Math.round((notStartedCount / totalCount) * 100) : 0}%)
                  </span>
                </button>
              </div>
            </div>
                  ...
                  >
                    ...
                    >
                      <div className="km-chapter-header-left">
                        <span className="km-chapter-badge">{chapter.index}</span>
                      ...
                      <div className="km-chapter-header-right">
                        <div className="km-chapter-stats">
                          <span className="km-chapter-stat-text">
                            ...
                            {chapter.masteredInChapter}/{chapter.totalPointsInChapter})
                          </span>
                        ...
                        <div className="km-chapter-chevron" aria-hidden="true">
                          <ChevronDown size={16} />
                        </div>
                      </div>
                    </header>
                    ...
                    {!isCollapsed && (
                      <div className="km-chapter-body">
                        {chapter.filteredSections.map((lesson) => (
                          <article className="km-lesson-group" key={lesson.id}>
                            <div className="km-lesson-header">
                              <div className="km-lesson-header-left">
                                <span className="km-lesson-index-tag">
                                  {lesson.index}
                                </span>
                                <h3 className="km-lesson-title">{lesson.title}</h3>
                              </div>
                              <span className="km-lesson-count">
                                {lesson.filteredKnowledgePoints.length} 个知识点
                              </span>
                            </div>
                              ...
                              {lesson.filteredKnowledgePoints.map(
                                (knowledgePoint) => {
                                  ...
                                  return (
                                    ...
                                    >
                                      {/* 卡片顶部：状态标签与来源 */}
                                      <div className="km-point-card-top">
                                        <span
                                          className={`km-point-status-badge ${meta.tone}`}
                                        >
                                      ...
                                      {/* 卡片主体：知识点名称 */}
                                      <div className="km-point-card-body">
                                        <h4
                                      ...
                                      {/* 卡片底部：掌握度与行动入口 */}
                                      <div className="km-point-card-bottom">
                                        <div className="km-point-mastery-wrap">
                                          <div className="km-point-progress-bar">
                                            <div
                                              className="km-point-progress-fill"
                                              style={{
                                                width:
                                                  masteryValue != null
                                                    ? `${Math.min(
                                          ...
                                          <span className="km-point-mastery-val">
                                            {masteryValue != null
                                              ? `${masteryValue}%`
                                              : "—"}
                                          </span>
