...
/**
 * “上新课”不做诊断，强制按教材顺序学习每个知识点并完成配套练习。
 * @param {object[]} knowledgePoints 当前课时知识点。
 * @param {object} generationPolicy 教师发布的学习模块策略。
 * @returns {object} 可直接进入首个知识点的学习计划。
 */
export function createNewLessonLearningPlan(knowledgePoints, generationPolicy) {
  const assessment = enabledLearningModules(generationPolicy).postAssessment
    ? "POST"
    : "NONE";
  return {
    ...createAutomaticLearningPlan(
      ...
      {},
      {
        ...
        masteredKnowledgePointPolicy: "FORCE_LEARN",
      },
    ),
    source: "new_lesson",
  };
