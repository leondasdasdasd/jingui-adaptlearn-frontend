...
import { isTransientLearningSelection } from "../../shared/domain/learningSessionPolicy.js";
import {
  emitClientEvent,
  readJson,
  writeJson,
...
import { readActiveStudentSession } from "./studentSessionRepository.js";
...
export function recordLearningEvent(event, selectionOverride = null) {
  const active = readActiveStudentSession();
  const selection = selectionOverride || active.selection || {};
  if (isTransientLearningSelection(selection)) return null;
