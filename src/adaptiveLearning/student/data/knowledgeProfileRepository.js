...
} from "../../shared/domain/learningSessionPolicy.js";
 ...
 * @param selection
 */
export function markKnowledgePointLearned(
  knowledgePoint,
  lessonId = "",
  selection = null,
) {
  if (isTransientLearningSelection(selection)) return readKnowledgeProfile();
...
export function syncKnowledgeProfileFromSession(session) {
  const current = readKnowledgeProfile();
  if (isTransientLearningSession(session)) return current;
