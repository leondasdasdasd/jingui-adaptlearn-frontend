...
/**
 * 临时试做会话优先于正式学生会话，仅用于当前学习运行时。
 * @param {object | null} fallback 无会话时的默认值
 * @returns {object | null} 当前运行时会话
 */
export function readActiveStudentSession(fallback = {}) {
  return readJson(
    storageKeys.transientStudentSession,
...
/** @param {object} session 临时学习会话 */
export function writeTransientStudentSession(session) {
  return writeJson(storageKeys.transientStudentSession, session);
}

/** 清除临时会话，不影响正式学生学习现场。 */
export function clearTransientStudentSession() {
  removeStoredValue(storageKeys.transientStudentSession);
}
...
/**
 * 只清理指定会话命名空间下的草稿。
 * @param {string} draftIdPrefix 草稿 ID 前缀
 */
export function clearQuizDraftsByPrefix(draftIdPrefix) {
  if (!draftIdPrefix) return;
  for (const draftId of draftIds()) {
    if (draftId.startsWith(draftIdPrefix)) clearQuizDraft(draftId);
  }
}
