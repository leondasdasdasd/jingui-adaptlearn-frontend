  ...
  return (
    <section className="pre-answer-detail">
      ...
      <article className={`pre-answer-detail-card ${state}`}>
        <header className="pre-answer-detail-header">
          <div className="pre-answer-detail-header-left">
            <h2>
              {preAssessmentResultText("questionNumber", {
                index: index + 1,
              })}
            </h2>
            <span className="pre-answer-detail-type">
              {question.type
                ? localizedQuestionType(question.type)
                : copy.practiceQuestion}
            </span>
            {question.difficulty && (
              <DifficultyBadge difficulty={question.difficulty} variant="tag" />
            )}
          </div>
    ...
    <section className="pre-mastery-page" aria-label={copy.masteryAria}>
      <div className="pre-mastery-list">
        {knowledgePoints.map((kp) => {
          ...
          const stopReason = preAssessmentStopReason(item.diagnosisReason);

          return (
            <article className={`pre-mastery-card ${meta.tone}`} key={kp.id}>
              <div className="pre-mastery-info">
                <div className="pre-mastery-title-row">
                  <h3 className="pre-mastery-name">{kp.name}</h3>
                  {meta.label && (
                    <span className={`pre-mastery-badge ${meta.tone}`}>
                      {meta.label}
                    </span>
                  )}
                </div>

                <div className="pre-mastery-details">
                  {stopReason && (
                    <span className="pre-mastery-tag pre-mastery-tag-stop-reason">
                      {stopReason}
                    </span>
                  )}
              ...
              <div className="pre-mastery-metric-panel">
                <div className="pre-mastery-metric-header">
                  {confidence != null && (
                    <span className="pre-mastery-tag pre-mastery-tag-confidence">
                      <Sparkles size={13} aria-hidden="true" />
                      <span>置信度 {Math.round(confidence)}%</span>
                    </span>
                  )}
                  <div className="pre-mastery-score-box">
                    <strong>
                      {covered ? value : "—"}
                      {covered && <small>%</small>}
                    </strong>
                ...
                <div
                  className="pre-mastery-progress-bar"
                  aria-label={
                    covered
      ...
      <div className="result-wrap pre-result-wrap">
        <section className="pre-result-summary">
          ...
          <div className="pre-result-summary-copy">
            {!hasAdministeredQuestions && copy.pathReady && (
              <span>{copy.pathReady}</span>
            )}
