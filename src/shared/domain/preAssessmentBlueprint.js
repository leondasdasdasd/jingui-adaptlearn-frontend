...
const PRE_ASSESSMENT_ROLE_ASSESSMENT_FOCUS = Object.freeze({
  [PRE_ASSESSMENT_DIAGNOSTIC_ROLE.STANDARD_PROBE]: '用标准变式检查学生能否独立选择规则并完成至少两个依赖环节',
  [PRE_ASSESSMENT_DIAGNOSTIC_ROLE.FOUNDATION_BRANCH]: '确认学生能否识别基本对象、调用直接规则并得到封闭结果',
  [PRE_ASSESSMENT_DIAGNOSTIC_ROLE.STANDARD_CONFIRMATION]: '改变表述或条件后，检查学生能否修正中间量并完成三步依赖推理',
  [PRE_ASSESSMENT_DIAGNOSTIC_ROLE.TRANSFER_BRANCH]: '在陌生情境中检查建模、两次依赖计算、隐藏条件处理和结果验证',
});

export const PRE_ASSESSMENT_PRIMARY_SLOTS_PER_KNOWLEDGE_POINT = ORDERED_ROLES.length;
...
export function buildPreAssessmentBlueprint(knowledgePoints = []) {
  return knowledgePoints.flatMap((knowledgePoint) => ORDERED_ROLES.map((diagnosticRole) => ({
...
export function diagnosticSlotForQuestion(question = {}) {
  const primaryKnowledgePointId = knowledgeEvidenceProfile(question).primaryKnowledgePointId;
  const diagnosticRole = String(question.diagnosticRole || '').toUpperCase();
  if (!primaryKnowledgePointId || !ORDERED_ROLES.includes(diagnosticRole)) return null;
  return {
...
export function isValidDiagnosticDifficulty(value) {
  const normalized = typeof value === 'string' && /^D[1-5]$/i.test(value.trim())
    ? Number(value.trim().slice(1)) : Number(value);
  return DIFFICULTY_LEVELS.includes(normalized);
}

export function missingPreAssessmentBlueprintSlots(questions = [], knowledgePoints = []) {
  const occupied = new Set(questions.map(diagnosticSlotForQuestion).filter(Boolean).map((slot) => slot.id));
  return buildPreAssessmentBlueprint(knowledgePoints).filter((slot) => !occupied.has(slot.id));
}

export function hasCompletePreAssessmentBlueprint(questions = [], knowledgePoints = []) {
  return missingPreAssessmentBlueprintSlots(questions, knowledgePoints).length === 0;
}
