...
const MASTERED_KNOWLEDGE_OPTIONS = new Set(['SKIP', 'VERIFY_ONCE', 'FORCE_LEARN']);

export function normalizeLearningGenerationPolicy(policy = {}) {
  return {
    compositeExplanation: COMPOSITE_EXPLANATION_OPTIONS.has(policy?.compositeExplanation)
    ...
    masteredKnowledgePointPolicy: MASTERED_KNOWLEDGE_OPTIONS.has(policy?.masteredKnowledgePointPolicy)
