...
export const COMPOSITE_REVIEW_DIFFICULTY_SEQUENCE = Object.freeze([3, 4, 4, 4, 5, 5]);
export const COMPOSITE_REVIEW_RECOMMENDED_TYPES = Object.freeze({
  3: Object.freeze(['short_answer', 'multiple_choice', 'ordering']),
  4: Object.freeze(['short_answer', 'multiple_choice', 'ordering', 'line_connect']),
...
const PRACTICE_POOL_TASK_CATEGORIES = Object.freeze([
  'calculation', 'concept_or_calculation', 'calculation',
  'calculation', 'application', 'calculation',
  'application', 'calculation', 'application', 'application',
  'application', 'application', 'calculation',
...
const PRACTICE_POOL_QUESTION_TYPES = Object.freeze([
  'single_choice', 'judgement', 'fill_blank',
  'text_marker', 'classification', 'matching',
  'multiple_choice', 'word_builder', 'ordering', 'short_answer',
  'line_connect', 'multiple_choice', 'short_answer',
...
const RECOMMENDED_TYPES_BY_DIFFICULTY = Object.freeze({
  ...
  4: ['single_choice', 'multiple_choice', 'fill_blank', 'classification', 'matching', 'line_connect', 'ordering', 'short_answer'],
...
export function compositeReviewCount(knowledgePointCount = 0) {
  ...
  return count <= 0 ? 0 : Math.max(COMPOSITE_REVIEW_POOL_SIZE, Math.min(10, count + 2));
}

export function compositeReviewBlueprint(count = COMPOSITE_REVIEW_POOL_SIZE) {
  const total = Math.max(1, Math.min(PRACTICE_POOL_MAX_SIZE_PER_KNOWLEDGE_POINT, Number(count) || COMPOSITE_REVIEW_POOL_SIZE));
  return Array.from({ length: total }, (_, index) => {
    const difficulty = COMPOSITE_REVIEW_DIFFICULTY_SEQUENCE[
      index % COMPOSITE_REVIEW_DIFFICULTY_SEQUENCE.length
    ];
    const calculationSlot = index % COMPOSITE_REVIEW_DIFFICULTY_SEQUENCE.length === 1;
    return {
      id: `review:${index + 1}`,
      difficulty: `D${difficulty}`,
      taskCategory: calculationSlot ? 'calculation' : 'application',
      adaptiveRole: difficulty === 5 ? 'transfer' : difficulty === 4 ? 'challenge' : 'comprehensive',
      questionType: [
        'multiple_choice', 'ordering', 'short_answer',
        'line_connect', 'short_answer', 'ordering',
      ][index % COMPOSITE_REVIEW_DIFFICULTY_SEQUENCE.length],
      assessmentFocus: [
...
export function practicePoolBlueprint(knowledgePointId, count = PRACTICE_POOL_SIZE_PER_KNOWLEDGE_POINT) {
  const total = Math.max(1, Math.min(PRACTICE_POOL_MAX_SIZE_PER_KNOWLEDGE_POINT, Number(count) || 0));
  const baselineLevels = Object.entries(PRACTICE_POOL_DIFFICULTY_COUNTS)
    .flatMap(([difficulty, levelCount]) => Array.from({ length: levelCount }, () => Number(difficulty)));
  ...
  return slots.map((difficultyLevel, index) => ({
    ...
    adaptiveRole: difficultyLevel <= 2 ? 'remediation' : difficultyLevel === 3 ? 'standard' : 'challenge',
    ...
    taskCategory: PRACTICE_POOL_TASK_CATEGORIES[index]
      || ((index - PRACTICE_POOL_TASK_CATEGORIES.length) % 2 === 0 ? 'calculation' : 'application'),
    questionType: PRACTICE_POOL_QUESTION_TYPES[index]
      || RECOMMENDED_TYPES_BY_DIFFICULTY[difficultyLevel][index
        % RECOMMENDED_TYPES_BY_DIFFICULTY[difficultyLevel].length],
    assessmentFocus: PRACTICE_POOL_ASSESSMENT_FOCI[index]
