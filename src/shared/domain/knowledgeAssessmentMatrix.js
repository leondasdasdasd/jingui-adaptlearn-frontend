...
export const ASSESSMENT_MATRIX_LEVELS = Object.freeze(['A', 'B', 'C', 'D', 'E']);
export const ASSESSMENT_MATRIX_ROLES = Object.freeze(['CORE', 'SUPPORT', 'EXTENSION']);
...
const domainOrder = new Map(ASSESSMENT_MATRIX_DOMAINS.map((domain, index) => [domain, index]));
const levelOrder = new Map(ASSESSMENT_MATRIX_LEVELS.map((level, index) => [level, index]));
...
export function normalizeKnowledgeAssessmentMatrix(rawMatrix = {}) {
  ...
  return {
    policyVersion: compactText(rawMatrix.policyVersion) || KNOWLEDGE_ASSESSMENT_MATRIX_POLICY_VERSION,
    knowledgePointId,
...
export function validateKnowledgeAssessmentMatrices(raw, {
  knowledgePoints = [],
  ...
  const knownIds = [...new Set((Array.isArray(knowledgePoints) ? knowledgePoints : [])
    .map((item) => compactText(typeof item === 'object' ? item?.id || item?.knowledgePointId : item))
    .filter(Boolean))];
  const knownIdSet = new Set(knownIds);
  const allowedTypeSet = new Set(allowedQuestionTypes);

  if (!source || typeof source !== 'object' || Array.isArray(source)) {
    errors.push(issue('INVALID_OBJECT', 'matrixPlan', '评估矩阵计划必须是对象'));
  }
  if (!rawMatrices.length) errors.push(issue('MATRIX_COUNT_BELOW_MINIMUM', 'matrices', '至少需要一个知识点评估矩阵'));

  const matrixIds = new Set();
  matrices.forEach((matrix, matrixIndex) => {
    const path = `matrices[${matrixIndex}]`;
    if (!matrix.knowledgePointId) errors.push(issue('MISSING_KNOWLEDGE_POINT_ID', `${path}.knowledgePointId`, '矩阵缺少知识点 ID'));
    if (matrixIds.has(matrix.knowledgePointId)) errors.push(issue('DUPLICATE_MATRIX', `${path}.knowledgePointId`, `知识点 ${matrix.knowledgePointId} 的矩阵重复`));
    matrixIds.add(matrix.knowledgePointId);
    if (knownIdSet.size && !knownIdSet.has(matrix.knowledgePointId)) errors.push(issue('UNKNOWN_KNOWLEDGE_POINT', `${path}.knowledgePointId`, `未知知识点 ${matrix.knowledgePointId}`));
    if (matrix.targetStatement.length < 4 || matrix.targetStatement.length > 240) errors.push(issue('INVALID_TARGET_STATEMENT', `${path}.targetStatement`, '目标陈述需为 4 至 240 个字符'));
    ...
    matrix.cells.forEach((cell, cellIndex) => {
      ...
      if (!ASSESSMENT_MATRIX_DOMAINS.includes(cell.domain)) errors.push(issue('INVALID_DOMAIN', `${cellPath}.domain`, `领域 ${cell.domain} 不在允许范围内`));
      if (!ASSESSMENT_MATRIX_LEVELS.includes(cell.targetLevel)) errors.push(issue('INVALID_LEVEL', `${cellPath}.targetLevel`, `层级 ${cell.targetLevel} 不在允许范围内`));
      if (!ASSESSMENT_MATRIX_ROLES.includes(cell.role)) errors.push(issue('INVALID_ROLE', `${cellPath}.role`, `角色 ${cell.role} 不在允许范围内`));
      if (cellIds.has(cell.matrixCellId)) errors.push(issue('DUPLICATE_CELL', cellPath, `评估格 ${cell.matrixCellId} 重复`));
      cellIds.add(cell.matrixCellId);
      if (cell.observableBehavior.length < 8 || cell.observableBehavior.length > 240) errors.push(issue('INVALID_OBSERVABLE_BEHAVIOR', `${cellPath}.observableBehavior`, '可观察行为需为 8 至 240 个字符'));
      if (!cell.evidenceCriteria.length) errors.push(issue('EVIDENCE_CRITERIA_MISSING', `${cellPath}.evidenceCriteria`, '适用格至少需要一条证据标准'));
  ...
  knownIds.forEach((knowledgePointId) => {
    if (!matrixIds.has(knowledgePointId)) errors.push(issue('MATRIX_MISSING', 'matrices', `缺少知识点 ${knowledgePointId} 的评估矩阵`));
  });
...
export function fallbackKnowledgeAssessmentMatrices(knowledgePoints = []) {
  return (Array.isArray(knowledgePoints) ? knowledgePoints : []).map((point) => {
    const knowledgePointId = compactText(point?.id || point?.knowledgePointId);
    const name = compactText(point?.name || point?.title || knowledgePointId);
    const objective = compactText(point?.objective || point?.description || point?.learningObjective || name);
...
function sortedActiveCells(matrix) {
  return [...matrix.cells].sort((left, right) => (
...
function evidenceCellSequence(matrix, count) {
  ...
  while (sequence.length < count) {
    const weighted = cells.flatMap((cell) => Array.from({ length: roleWeight[cell.role] || 1 }, () => cell));
