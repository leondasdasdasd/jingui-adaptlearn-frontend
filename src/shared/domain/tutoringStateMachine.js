import { MASTERY_THRESHOLD, isMasteredValue } from './masteryPolicy.js';
...
export const tutoringStateMeta = Object.freeze({
  DIAGNOSING: { label: '正在回顾错因', nextAction: '说清共同错误原因和下次检查方法' },
  READY_TO_CONTINUE: { label: '错因已经确认', nextAction: '用一道未见新题重新验证' },
  REMEDIATION: { label: '正在重点讲解', nextAction: '完成讲解后用未见新题重新验证' },
  REVALIDATING: { label: '正在重新验证', nextAction: '独立完成未见新题，不能依赖讲解或提示' },
  COMPLETE: { label: '重新验证通过', nextAction: '继续后续学习' },
  NEEDS_SUPPORT: { label: '重新验证未通过', nextAction: '保留为待巩固，建议教师后续关注' },
});
...
export function createTutoringSession(intervention, details = {}) {
  const transition = transitionRecord(tutoringStates.PRACTICING, tutoringStates.DIAGNOSING, {
    ...details,
    reasonCode: intervention?.trigger || 'PRACTICE_INTERVENTION',
    evidenceQuestionIds: (intervention?.evidence || []).map((item) => item.questionId).filter(Boolean),
  });
  return {
...
export function transitionTutoringSession(session, toState, details = {}) {
  ...
  return {
    ...
    state: toState,
    updatedAt: transition.occurredAt,
    completedAt: [tutoringStates.COMPLETE, tutoringStates.NEEDS_SUPPORT].includes(toState)
...
export function diagnosisTargetState(diagnosis) {
  ...
  return diagnosis.needsRemediation ? tutoringStates.REMEDIATION : tutoringStates.READY_TO_CONTINUE;
}

export function normalizeRevalidationDecision(state, decision) {
  if (state === tutoringStates.REVALIDATING && decision?.status === 'needs_intervention') {
    return { ...decision, status: 'needs_support', reason: 'REVALIDATION_NOT_PASSED' };
  }
  return decision;
}
...
export function revalidationDecisionForScore(scoreRatio, mastery) {
  ...
  const reason = passed
    ? 'REVALIDATION_PASSED'
    : passedScore ? 'REVALIDATION_MASTERY_BELOW_TARGET' : 'REVALIDATION_NOT_PASSED';
  return {
