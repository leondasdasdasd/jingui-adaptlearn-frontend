...
export function normalizeAnswerText(value) {
  return canonicalMathAnswerSyntax(value).replace(/\s+/g, '').replace(/[，。；]/g, '').toLowerCase();
}
...
function normalizeChoiceId(question, value) {
  ...
  const direct = options.find((option) => normalizeAnswerText(option?.id) === raw);
...
function numericValue(value) {
  const source = canonicalMathAnswerSyntax(value).replaceAll('，', ',').replace(/\s+/g, '');
...
function signedNumericToken(value) {
  const source = canonicalMathAnswerSyntax(value).replace(/\s+/g, '');
  const match = source.match(/^([+\-±]?)(?:(\d+(?:\.\d+)?)|(\.\d+))$/);
  if (!match) return null;
  const magnitude = Math.abs(Number(`${match[2] || ''}${match[3] || ''}`));
  if (!Number.isFinite(magnitude)) return null;
  return { magnitude, sign: match[1] === '-' ? 'negative' : match[1] === '±' ? 'both' : 'positive' };
}

function explicitSignGroupMagnitudes(answerPool) {
  const magnitudes = new Set();
  const separateTokens = [];
  const groupByMagnitude = (tokens) => tokens.reduce((groups, token) => {
    const items = groups.get(token.magnitude) || [];
    items.push(token);
    groups.set(token.magnitude, items);
    return groups;
  }, new Map());
  answerPool.flat(Infinity).forEach((entry) => {
    const raw = String(entry ?? '').trim();
    if (!raw) return;
    const tokens = raw.split(/[,，、;；]+/).map(signedNumericToken).filter(Boolean);
    ...
    if (tokens.length > 1) {
      ...
      byMagnitude.forEach((items, magnitude) => {
        const signs = new Set(items.map((item) => item.sign));
        if (signs.has('both') || (signs.has('positive') && signs.has('negative'))) magnitudes.add(magnitude);
      });
    }
  });
...
function explicitSignGroupMatches(answerPool, submitted) {
  ...
  return Boolean(token && explicitSignGroupMagnitudes(answerPool).has(token.magnitude));
}
...
function normalizedObject(value) {
  ...
  return Object.fromEntries(Object.entries(value).map(([key, item]) => [
    normalizeAnswerText(key), normalizeAnswerText(item),
  ]));
}

function connectionEdges(value, fromKey, toKey) {
  if (Array.isArray(value)) {
    return value.map((edge) => `${normalizeAnswerText(edge?.[fromKey])}->${normalizeAnswerText(edge?.[toKey])}`)
  ...
  return Object.entries(value).flatMap(([from, targets]) => (
    (Array.isArray(targets) ? targets : []).map((to) => (
      `${normalizeAnswerText(from)}->${normalizeAnswerText(to)}`
    ))
  ));
}

function setScore(expectedValues, actualValues) {
  const expected = new Set(expectedValues.map(normalizeAnswerText).filter(Boolean));
...
export function objectiveScoreRatio(question, submitted) {
  ...
  if (question.type === 'text_marker') {
    return setScore(Array.isArray(question.answer) ? question.answer : [], Array.isArray(submitted) ? submitted : []);
  }
  ...
  if (question.type === 'ordering') {
    const expected = Array.isArray(question.answer) ? question.answer.map(normalizeAnswerText) : [];
    const actual = Array.isArray(submitted) ? submitted.map(normalizeAnswerText) : [];
    if (!expected.length || actual.length !== expected.length) return 0;
    return expected.every((value, index) => actual[index] === value) ? 1 : 0;
  }
  if (question.type === 'judgement') {
    return normalizeAnswerText(submitted) === normalizeAnswerText(question.answer) ? 1 : 0;
  }
  if (question.type === 'multiple_choice') {
    const expected = new Set(choiceValues(question.answer).map((value) => normalizeChoiceId(question, value)));
    const actual = new Set(choiceValues(submitted).map((value) => normalizeChoiceId(question, value)));
    if (!expected.size) return 0;
    if (actual.size !== expected.size) return 0;
    return [...expected].every((value) => actual.has(value)) ? 1 : 0;
  }
  if (question.type === 'single_choice') {
    return normalizeChoiceId(question, submitted) === normalizeChoiceId(question, question.answer) ? 1 : 0;
  }
  if (question.type === 'fill_blank' && question.answerKind === 'numeric') {
    ...
    const correctCount = expected.filter((answer, index) => {
      const answerPool = [answer, ...acceptableAnswersForBlank(question, index, multiple)];
      return explicitSignGroupMatches(answerPool, actual[index])
        || answerPool.some((candidate) => numericAnswerMatches(question, candidate, actual[index]));
    }).length;
  ...
  if (question.type === 'fill_blank' && Array.isArray(question.answer)) {
    ...
    const acceptedByBlank = Array.isArray(question.acceptableAnswers)
      ...
      : [];
    const correctCount = expected.filter((answer, index) => {
      const answerPool = [answer, ...(Array.isArray(acceptedByBlank[index]) ? acceptedByBlank[index] : [])];
  ...
  if (question.type === 'fill_blank' && explicitSignGroupMatches(answerPool, submitted)) return 1;
  const acceptable = answerPool.map(normalizeAnswerText);
...
export function knowledgeEvidenceProfile(question = {}) {
  ...
  if (!ids.length) return { primaryKnowledgePointId: '', knowledgePointWeights: {} };
  ...
  const primaryKnowledgePointId = ids.includes(question.primaryKnowledgePointId)
    ...
    : [...ids].sort((a, b) => Number(supplied[b] || 0) - Number(supplied[a] || 0))[0];
  return {
    primaryKnowledgePointId,
    knowledgePointWeights: Object.fromEntries(ids.map((id) => [
      id,
      id === primaryKnowledgePointId ? 1 : Math.min(0.3, Math.max(0.05, Number(supplied[id] || 0.3))),
    ])),
  };
}
