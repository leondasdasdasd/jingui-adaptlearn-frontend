...
export const U1_ALGORITHM_VERSION = 'U1-R11-production+evidence-gate-v6-adaptive-pre-stop-single-90';
...
export const U1_CONFIG = Object.freeze({
  ...
  priorSd: 1.15,
  discrimination: 1.1,
  difficultyAnchors: Object.freeze({ D1: -2.2, D2: -1.05, D3: 0, D4: 1.05, D5: 2.15 }),
  ...
  sourceFactors: Object.freeze({ PRE: 0.9, PRACTICE: 1.5, POST: 1.8, REVALIDATION: 1.9, REVIEW: 1.8 }),
  roleFactors: Object.freeze({ PRIMARY: 1, SECONDARY_OBSERVED: 0.3, SECONDARY: 0.3, CONTEXT_ONLY: 0 }),
  hintFactors: Object.freeze({ NONE: 1, LIGHT: 0.72, HEAVY: 0.35, ANSWER_SHOWN: 0.12 }),
...
const GRID = Object.freeze(Array.from(
...
const normalDensity = (value, mean, sd) => Math.exp(-0.5 * ((value - mean) / sd) ** 2);

export function normalizeU1Difficulty(value, fallback = 'D3') {
  if (typeof value === 'string' && /^D[1-5]$/i.test(value.trim())) return value.trim().toUpperCase();
  const numeric = Number(value);
  if (Number.isFinite(numeric)) return U1_DIFFICULTIES[Math.max(0, Math.min(4, Math.round(numeric) - 1))];
  return fallback;
}

function normalizeWeights(weights) {
  const total = weights.reduce((sum, value) => sum + value, 0);
  if (!Number.isFinite(total) || total <= 0) return GRID.map(() => 1 / GRID.length);
  return weights.map((value) => value / total);
}

function priorWeights(mean = U1_CONFIG.priorMean, sd = U1_CONFIG.priorSd) {
  return normalizeWeights(GRID.map((theta) => normalDensity(theta, mean, Math.max(0.1, sd))));
}

function targetMasteryFromWeights(weights, difficulty = U1_CONFIG.targetDifficulty) {
  return GRID.reduce((sum, theta, index) => (
...
function probabilityAtDifficulty(theta, difficulty) {
  return sigmoid(U1_CONFIG.discrimination * (
    theta - U1_CONFIG.difficultyAnchors[normalizeU1Difficulty(difficulty)]
  ));
}

function scoreRatio(evidence = {}) {
  if (Number.isFinite(Number(evidence.scoreRatio))) return clamp(evidence.scoreRatio);
...
function effectiveWeight(evidence = {}) {
  const source = String(evidence.source || evidence.purpose || 'PRE').toUpperCase();
  const role = String(evidence.role || 'PRIMARY').toUpperCase();
  const hint = evidence.hintUsed === true
    ? 'LIGHT' : String(evidence.hint || evidence.hintLevel || 'NONE').toUpperCase();
  const novelty = String(evidence.novelty || 'NEW').toUpperCase();
  return (U1_CONFIG.sourceFactors[source] ?? 1)
...
function trailingCorrectStreak(state) {
  let streak = state.trace.length ? 0 : Number(state.correctStreak || 0);
...
export function createU1State({
  ...
  const hasPrior = priorMastery !== null && priorMastery !== undefined && priorMastery !== ''
    && Number.isFinite(normalizedPrior);
  const probability = hasPrior ? clamp(normalizedPrior > 1 ? normalizedPrior / 100 : normalizedPrior) : 0.5;
  ...
  const normalizedPriorConfidence = Number.isFinite(confidence)
    ...
    : 0;
  const priorSd = hasPrior && Number.isFinite(confidence)
    ? Math.max(0.55, U1_CONFIG.priorSd * (1 - 0.45 * clamp(confidence > 1 ? confidence / 100 : confidence)))
    : U1_CONFIG.priorSd;
  const priorMean = hasPrior ? calibratedPriorMean(probability, priorSd) : U1_CONFIG.priorMean;
  return {
    knowledgePointId,
    ...
    priorPrimaryEvidenceCount: Math.max(0, Number(priorPrimaryEvidenceCount) || 0),
...
function summarizePosterior(state) {
  ...
  const mastery = 100 * GRID.reduce((sum, theta, index) => (
    sum + probabilityAtDifficulty(theta, target) * state.weights[index]
  ), 0);
  const lowerBound = 100 * probabilityAtDifficulty(weightedQuantile(state.weights, 0.1), target);
  const upperBound = 100 * probabilityAtDifficulty(weightedQuantile(state.weights, 0.9), target);
  ...
  const difficultyCoverage = primary.length
    ? clamp(new Set(primary.map((item) => item.difficulty)).size / 3 * 0.55
      + (primary.some((item) => U1_DIFFICULTIES.indexOf(item.difficulty) >= 2) ? 0.45 : 0))
    : 0;
  const mean = (values) => values.length
    ? values.reduce((sum, value) => sum + value, 0) / values.length : 0;
  const independence = mean(effective.map((item) => item.independence));
  const quality = mean(effective.map((item) => item.itemQuality * item.gradingConfidence));
  const recencyCoverage = effective.some((item) => item.source === 'REVALIDATION')
    ? 1 : effective.some((item) => ['POST', 'REVIEW'].includes(item.source)) ? 0.75 : effective.length ? 0.4 : 0;
  const currentEvidenceConfidence = effective.length ? 100 * (
    0.5 * posteriorConcentration + 0.2 * difficultyCoverage
    + 0.15 * independence + 0.1 * quality + 0.05 * recencyCoverage
  ) : 0;
  ...
  const confidence = Math.max(Number(state.priorConfidence || 0), currentEvidenceConfidence);
  const lastTwo = state.trace.slice(-2);
  const recentDelta = lastTwo.length >= 2
  ...
  const status = !effective.length && !state.hasHistoricalPrior ? 'NOT_ASSESSED'
    : normalizedMastery < U1_CONFIG.learningWarningThreshold ? 'NEEDS_LEARNING'
        : normalizedMastery >= U1_CONFIG.masteryThreshold ? 'MASTERED'
          : 'VERIFYING';
  return {
    ...
    primaryEvidenceCount: Number(state.priorPrimaryEvidenceCount || 0) + primary.length,
    status,
    nextAction: status === 'NOT_ASSESSED' ? '待测评'
      : status === 'NEEDS_LEARNING' ? '进入强制学习'
        : status === 'MASTERED' ? '进入综合迁移题'
          : '继续按目标难度测量',
...
export function applyU1Evidence(state, rawEvidence = {}) {
  ...
  const adaptive = effectiveWeightForState(state, rawEvidence, score, before.mastery);
  const weight = adaptive.weight;
  const anchor = U1_CONFIG.difficultyAnchors[difficulty];
  if (weight > 0) {
    const logWeights = GRID.map((theta, index) => {
      const probability = clamp(sigmoid(U1_CONFIG.discrimination * (theta - anchor)), 1e-9, 1 - 1e-9);
      const likelihood = score * Math.log(probability) + (1 - score) * Math.log(1 - probability);
      return Math.log(Math.max(state.weights[index], 1e-300)) + weight * likelihood;
    });
    const maxLog = Math.max(...logWeights);
    state.weights = normalizeWeights(logWeights.map((value) => Math.exp(value - maxLog)));
  }
  const evidence = {
    questionId: rawEvidence.questionId || rawEvidence.id || '',
    source: String(rawEvidence.source || rawEvidence.purpose || 'PRE').toUpperCase(),
    difficulty,
    scoreRatio: score,
    ...
    gradingConfidence: clamp(rawEvidence.gradingConfidence ?? rawEvidence.itemConfidence ?? 1),
    hint: rawEvidence.hintUsed === true ? 'LIGHT' : String(rawEvidence.hint || 'NONE').toUpperCase(),
...
export function calculateU1KnowledgeMastery({ knowledgePointId, evidence = [], prior = {} }) {
  ...
  evidence.forEach((item) => applyU1Evidence(state, item));
