...
import { createSerializedRichContent, repairEmbeddedChoiceDescriptions } from './questionContract.js';
import {
...
export function canUseQuestionPlatformPlayer(question) {
  return [
    'single_choice', 'multiple_choice', 'fill_blank', 'short_answer', 'judgement', 'ordering',
    'classification', 'matching', 'line_connect', 'text_marker', 'word_builder',
  ].includes(question?.type)
    && canUseQuestionPlatform(question);
}
...
function stripEmbeddedChoiceOptions(stem, optionTexts) {
  ...
  const options = optionTexts.map((item) => String(item || '').trim()).filter(Boolean);
  if (!source || !options.length) return source;

  const firstOptionAt = source.indexOf(options[0]);
  const containsEveryOption = firstOptionAt > 0
...
function normalizeChoiceStem(serialized, question) {
  if (!['single_choice', 'multiple_choice'].includes(question.type)) return serialized;
  const stemIndex = serialized.elements?.findIndex((item) => item.type === 'richText') ?? -1;
  const choice = serialized.elements?.find((item) => item.type === 'choice');
  if (stemIndex < 0 || !choice) return serialized;

  const optionTexts = (choice.options?.length ? choice.options : question.options || [])
    .map((option) => option.cells?.[0]?.text ?? option.text);
  const currentStem = serialized.elements[stemIndex].content?.text ?? question.stem;
...
function mergeRichContentItem(canonical, persisted) {
  if (!persisted || canonical?.content?.text !== persisted?.content?.text || !hasRichDocument(persisted.content)) {
    return canonical;
  }
...
function mergeShortAnswerSerialized(question, persisted) {
  ...
  return {
    ...persisted,
    ...canonical,
    elements: canonical.elements.map((item) => mergeRichContentItem(
      item,
      persisted.elements.find((candidate) => candidate.type === item.type),
    )),
    extras: canonical.extras.map((item) => mergeRichContentItem(
      item,
      (persisted.extras || []).find((candidate) => candidate.type === item.type),
    )),
  };
}

function inlineFillAnswerPool(element, id, index) {
  const group = (element.answers || []).find((item) => item.blankIds?.map(String).includes(id))
...
function mergeInlineFillSerialized(question, persisted) {
  ...
  const canonicalIds = (canonicalElement.blanks || []).map((blank) => String(blank?.id || blank));
  const persistedIds = (persistedElement.blanks || []).map((blank) => String(blank?.id || blank));
  const canPreserveStructure = canonicalElement.content?.text === persistedElement.content?.text
    && canonicalIds.length === persistedIds.length;
  if (!canPreserveStructure) return canonical;

  const answers = persistedIds.map((id, index) => ({
    answerPools: inlineFillAnswerPool(canonicalElement, canonicalIds[index], index),
    blankIds: [id],
  }));
  return {
    ...persisted,
    ...
    elements: [{
      ...
      content: hasRichDocument(persistedElement.content) ? persistedElement.content : canonicalElement.content,
    }],
  };
}
...
function rubricFromText(value, fallbackRubric = []) {
  return String(value || '').split(/\r?\n/).map((line) => line.trim()).filter(Boolean).map((line, index) => {
    ...
    const fallbackScore = Number(fallbackRubric[index]?.points ?? fallbackRubric[index]?.score);
    const parsedScore = Number(match?.[2]);
    return {
      point,
      points: Number.isFinite(parsedScore) && parsedScore > 0
        ? parsedScore
        : Number.isFinite(fallbackScore) && fallbackScore > 0 ? fallbackScore : 0,
    };
...
function recoverShortAnswerFields(question) {
  ...
  const extraText = (key) => (question.platformQuestion?.extras || [])
    .find((item) => item.type === key)
    ?.content?.text || '';
  const answer = String(question.answer || '').trim() ? question.answer : extraText('sampleAnswer');
  const analysis = String(question.analysis || '').trim() ? question.analysis : extraText('solvingProcess');
  const rubric = Array.isArray(question.rubric) && question.rubric.length
    ? question.rubric
    : rubricFromText(extraText('scoringRule'));
  const rubricScore = rubric.reduce((sum, item) => sum + Number(item.points || 0), 0);
  return {
    ...question,
    answer,
    analysis,
    rubric,
    maxScore: Number(question.maxScore) > 0 ? question.maxScore : rubricScore || question.maxScore,
  };
}

export function createQuestionPlatformDraft(question) {
  const repairedQuestion = repairEmbeddedChoiceDescriptions(question);
  const recoveredFillAnswers = repairedQuestion.type === 'fill_blank'
    ? fillAnswersFromQuestion(repairedQuestion)
    : [];
  const fillRecoveredQuestion = repairedQuestion.type === 'fill_blank'
    ? {
        ...repairedQuestion,
        answer: recoveredFillAnswers.length > 1 ? recoveredFillAnswers : recoveredFillAnswers[0] || '',
      }
    : repairedQuestion;
  ...
  const persisted = sourceQuestion.platformQuestion || toQuestionPlatformSerialized(sourceQuestion);
  const source = sourceQuestion.type === 'fill_blank'
  ...
  const serialized = normalizeChoiceStem(
    shouldRegenerateFillSerialized(sourceQuestion, source) ? toQuestionPlatformSerialized(sourceQuestion) : source,
    sourceQuestion,
  );
...
export function readQuestionPlatformDraft(draft, questionType, fallbackQuestion = {}) {
  const serialized = serializeQuestionContentDraft(draft);
  const stemElement = serialized.elements.find((item) => item.type === 'richText');
  const answerElement = serialized.elements.find((item) => (
    [
      'choice', 'fill', 'inlineFill', 'judgement', 'ordering', 'classification',
      'matching', 'lineConnect', 'textMarker', 'wordBuilder', 'textResponse',
    ].includes(item.type)
  ));
  const base = {
  ...
  if (questionType === 'short_answer') {
    const extraText = (key) => (serialized.extras || [])
      .find((item) => item.type === key)
      ?.content?.text || '';
    const rubric = rubricFromText(extraText('scoringRule'), fallbackQuestion.rubric);
  ...
  if (answerElement?.type === 'judgement') {
    ...
    return { ...base, options: [], answer: typeof value === 'boolean' ? String(value) : '' };
  }
  ...
  if (answerElement?.type === 'classification') {
    return {
      ...
      categories: (answerElement.categories || []).map((category) => ({
        id: category.id, text: category.content?.text || '',
      })),
      items: (answerElement.items || []).map((item) => ({ id: item.id, text: item.content?.text || '' })),
      options: [],
      answer: answerElement.answers || {},
    };
  }
  if (['matching', 'lineConnect'].includes(answerElement?.type)) {
    const columns = (answerElement.columns || []).map((column, columnIndex) => ({
      id: column.columnId || `column-${columnIndex + 1}`,
      items: (column.items || []).map((item, itemIndex) => ({
        id: item.itemId || item.id || `item-${columnIndex + 1}-${itemIndex + 1}`,
        text: item.content?.text || '',
      })),
    }));
    const answer = Object.entries(answerElement.answers || {}).flatMap(([from, values]) => (
      (values || []).map((to) => answerElement.type === 'matching'
  ...
  if (answerElement?.type === 'textMarker') {
    const segments = (answerElement.content?.json || []).flatMap((node) => node.children || [])
      .filter((node) => String(node?.text || ''))
      .map((node) => ({ text: node.text, ...(node.markerId ? { markerId: node.markerId } : {}) }));
    return { ...base, segments, options: [], answer: answerElement.answers || [] };
  }
  if (answerElement?.type === 'wordBuilder') {
    const template = (answerElement.content?.json || []).flatMap((node) => node.children || [])
      .map((node) => node.type === 'blank' ? `{{${node.blankId}}}` : String(node.text || ''))
      .join('');
    return {
      ...base,
  ...
  const fillAnswerPools = (answerElement?.blanks || []).map((blank) => {
    ...
    const group = answerElement?.answers?.find((item) => item.blankIds?.includes(blankId));
    return (group?.answerPools || group?.answers?.[blankId] || []).map((item) => String(item?.text || ''));
  });
...
export function adaptLegacyQuestion(question, answerValue) {
  ...
  if (interactive?.type === 'textResponse') {
    interactive.answers = answerValue && typeof answerValue === 'object' && !Array.isArray(answerValue)
      ? answerValue : createSerializedRichContent(String(answerValue || ''));
  }
  if (interactive?.type === 'fill') {
    const blankIds = question.platformQuestion?.elements?.find((item) => item.type === 'fill')?.blanks || ['blank-1'];
    const values = Array.isArray(answerValue) ? answerValue : [answerValue];
    interactive.answers = blankIds.map((blankId, index) => ({
      answerPools: [createSerializedRichContent(String(values[index] || ''))], blankIds: [blankId],
    }));
  }
  if (interactive?.type === 'inlineFill') {
    const inlineElement = serializedDraft.elements?.find((item) => item.type === 'inlineFill');
  ...
  if (interactive?.type === 'judgement') {
    interactive.answers = answerValue === 'true' || answerValue === true
      ? [true] : answerValue === 'false' || answerValue === false ? [false] : [];
  }
  if (interactive?.type === 'ordering') {
    const initialOrder = serializedDraft.elements?.find((item) => item.type === 'ordering')
      ?.sortOptions?.map((option) => option.id) || [];
    interactive.answers = Array.isArray(answerValue) && answerValue.length
      ? answerValue : initialOrder;
  }
  ...
  if (['matching', 'lineConnect'].includes(interactive?.type)) {
    interactive.answers = toConnectionResponseAnswers(question.type, answerValue, question.columns);
  }
