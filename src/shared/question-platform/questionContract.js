...
function choiceStructure(selectionType) {
  return {
    elements: [
      ...
      { config: { optionLabelStyle: 'upperAlpha', renderer: 'standard', selectionType }, name: '选项', type: 'choice' },
    ],
    extras: [], hasAnswer: true, isComposite: false,
  };
}
...
export const QUESTION_PLATFORM_STRUCTURES = {
  ...
  judgement: {
    elements: [
      { config: {}, name: '题干', type: 'richText' },
      { config: { judgeAnswerMode: 'correctWrong' }, name: '判断', type: 'judgement' },
    ],
    extras: [], hasAnswer: true, isComposite: false,
  },
  ordering: {
  ...
  word_builder: {
    elements: [
      { config: { builderMode: 'sentence' }, name: '组式', type: 'wordBuilder' },
    ],
    extras: [], hasAnswer: true, isComposite: false,
  },
};
...
export function createSerializedRichContent(value) {
  ...
  return { text, html: `<p>${escapeHtml(text)}</p>`, json: [{ type: 'paragraph', children: [{ text }] }] };
}

function stripRenderedChoiceLabel(option, index) {
  const fallbackId = ['A', 'B', 'C', 'D'][index] || String(index + 1);
  const id = String(option?.id || fallbackId).trim().toUpperCase();
  const text = String(option?.text ?? option ?? '').trim();
  if (new RegExp(`^${id}、[A-D](?:\\s|两|点)`, 'i').test(text)) {
    return { ...(typeof option === 'object' && option !== null ? option : {}), id, text };
  }
  const labelPattern = new RegExp(`^(?:[（(]${id}[）)]|${id}[.．、:：])\\s*`, 'i');
...
export function repairEmbeddedChoiceDescriptions(question) {
  const type = String(question?.type || '').trim().toLowerCase().replaceAll('-', '_');
  ...
  const strippedLabels = options.some((option, index) => (
    option.text !== String(originalOptions[index]?.text ?? originalOptions[index] ?? '').trim()
  ));
  const labelsOnly = options.length === 4
    && options.every((option, index) => String(option.text || '').trim().toUpperCase() === (option.id || ['A', 'B', 'C', 'D'][index]).toUpperCase());
  if (!labelsOnly) {
    return strippedLabels ? { ...question, options, platformQuestion: null } : question;
  }

  const source = String(question.stem || '').trim();
  const markers = [...source.matchAll(/(?:^|[\n\r；;。]\s*)([A-D])[.．、:：]\s*/g)];
  if (markers.length !== 4 || markers.map((match) => match[1]).join('') !== 'ABCD') {
    return strippedLabels ? { ...question, options, platformQuestion: null } : question;
  }
  const repairedOptions = markers.map((match, index) => {
    const start = match.index + match[0].length;
    const end = index + 1 < markers.length ? markers[index + 1].index : source.length;
    return { ...options[index], id: match[1], text: source.slice(start, end).trim().replace(/[；;。]+$/, '') };
  });
...
function createInlineFillContent(stem, blankIds) {
  ...
  const parts = source.split(/_{2,}/g);
  const children = [];
  ...
  return {
    text: json[0].children.map((item) => item.type === 'blank' ? '____' : item.text).join(''),
    html: `<p>${json[0].children.map((item) => item.type === 'blank'
      ? `<span data-rte-node="blank" data-blank-id="${item.blankId}" data-label="${item.label}"></span>`
      : escapeHtml(item.text)).join('')}</p>`,
    json,
  };
}
...
function createTextMarkerContent(segments = []) {
  ...
  const html = `<p>${children.map((item) => item.markerId
...
function createWordBuilderContent(template, blankIds) {
  ...
  const tokenPattern = /\{\{(B[1-9][0-9]*)\}\}/g;
  const children = [];
  let cursor = 0;
  for (const match of source.matchAll(tokenPattern)) {
    if (match.index > cursor) children.push({ text: source.slice(cursor, match.index) });
    children.push({ blankId: match[1], children: [{ text: '' }], label: match[1], type: 'blank' });
    cursor = match.index + match[0].length;
  }
  if (cursor < source.length) children.push({ text: source.slice(cursor) });
  const knownIds = new Set(children.filter((item) => item.type === 'blank').map((item) => item.blankId));
  blankIds.filter((id) => !knownIds.has(id)).forEach((id) => {
    children.push({ text: ' ' });
    children.push({ blankId: id, children: [{ text: '' }], label: id, type: 'blank' });
  });
  return {
    text: children.map((item) => item.type === 'blank' ? '____' : item.text).join(''),
    html: `<p>${children.map((item) => item.type === 'blank'
      ? `<span data-rte-node="blank" data-blank-id="${item.blankId}" data-label="${item.label}"></span>`
      : escapeHtml(item.text)).join('')}</p>`,
...
function fillAnswerPools(question) {
  ...
  return answers.map((answer, index) => {
    ...
    const alternatives = answers.length > 1
      ? acceptableAnswers[index]
      : acceptableAnswers;
    const pools = Array.isArray(alternatives) ? alternatives : alternatives ? [alternatives] : [];
    return [...new Set([answer, ...pools].filter((value) => value !== undefined && value !== null && String(value).trim() !== ''))]
      .map((value) => createSerializedRichContent(String(value)));
  });
}

function shortAnswerRubricText(rubric) {
  return (Array.isArray(rubric) ? rubric : []).map((item, index) => {
    const point = String(item?.point || item?.criterion || item?.description || '').trim();
...
export function toQuestionPlatformSerialized(question) {
  if (question?.type === 'short_answer') {
    ...
    return serializedDraft(
      ...
      [
        ...
        {
          content: createSerializedRichContent(shortAnswerRubricText(question.rubric)),
          type: 'scoringRule',
        },
        {
  ...
  if (question?.type === 'judgement') {
    ...
    const normalizedAnswer = typeof question.answer === 'boolean'
      ? question.answer
      : ['true', 'false'].includes(String(question.answer).trim().toLowerCase())
        ? String(question.answer).trim().toLowerCase() === 'true'
        : null;
    return serializedDraft(questionTypeKey, [
      { content: createSerializedRichContent(question.stem), type: 'richText' },
      { answers: normalizedAnswer === null ? [] : [normalizedAnswer], type: 'judgement' },
    ]);
  }
  if (question?.type === 'ordering') {
    ...
    const answerIsInitialOrder = sourceOrder.length === answer.length
      && sourceOrder.every((id, index) => id === answer[index]);
    const presentationOptions = answerIsInitialOrder && sourceOptions.length > 1
    ...
    return serializedDraft(questionTypeKey, [
      { content: createSerializedRichContent(question.stem), type: 'richText' },
      {
        ...
        sortOptions: presentationOptions.map((option) => ({
          content: createSerializedRichContent(option.text), id: option.id,
        })),
        type: 'ordering',
      },
    ]);
    ...
    return serializedDraft(questionTypeKey, [
      { content: createSerializedRichContent(question.stem), type: 'richText' },
      {
        ...
        categories: (question.categories || []).map((category) => ({
          content: createSerializedRichContent(category.text), id: category.id,
        })),
        items: (question.items || []).map((item) => ({
          content: createSerializedRichContent(item.text), id: item.id,
        })),
        type: 'classification',
      },
    ]);
  ...
  if (['matching', 'line_connect'].includes(question?.type)) {
    ...
    const elementType = question.type === 'matching' ? 'matching' : 'lineConnect';
    const answers = question.type === 'matching'
      ? normalizeConnectionAnswers(question.answer, 'leftItemId', 'rightItemId')
  ...
  if (question?.type === 'text_marker') {
    ...
    const markers = (question.segments || []).map((segment) => String(segment?.markerId || '').trim()).filter(Boolean);
    return serializedDraft(questionTypeKey, [
      { content: createSerializedRichContent(question.stem), type: 'richText' },
      {
  ...
  if (question?.type === 'word_builder') {
    ...
    const answers = question.answer && typeof question.answer === 'object' ? question.answer : {};
  ...
  const questionTypeKey = TYPE_KEYS[question.type];
  const answerElement = question.type === 'fill_blank'
    ? (() => {
    ...
    : {
        answers: { optionIds: Array.isArray(question.answer) ? question.answer : [question.answer] },
        columns: [{ content: createSerializedRichContent('选项'), id: 'column-1' }],
        options: (question.options || []).map((option) => ({ cells: [createSerializedRichContent(option.text)], id: option.id })),
        type: 'choice',
      };
  return serializedDraft(questionTypeKey, question.type === 'fill_blank' ? [answerElement] : [
    { content: createSerializedRichContent(question.stem), type: 'richText' },
    answerElement,
  ]);
}
