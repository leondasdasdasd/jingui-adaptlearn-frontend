...
function blankId(blank, index) {
  return String(typeof blank === 'string' ? blank : blank?.id || `inline-blank-${index + 1}`);
}
...
function answerPool(values) {
  ...
  return values
    ...
    .map((value) => typeof value === 'string' ? createSerializedRichContent(value) : value);
}
...
export function fillAnswersFromQuestion(question) {
  const canonical = (Array.isArray(question?.answer) ? question.answer : [question?.answer])
    .map(answerText);
  if (canonical.some((value) => value.trim())) return canonical;

  const element = question?.platformQuestion?.elements?.find((item) => item.type === 'inlineFill');
  ...
  const recovered = blankIds.map((id) => {
    ...
    const values = Array.isArray(group?.answerPools) ? group.answerPools : group?.answers?.[id];
...
function canonicalInlineFillAnswers(element, blankIds) {
  ...
  const groups = (element.answers || []).flatMap((sourceGroup) => {
    const group = sourceGroup && typeof sourceGroup === 'object' ? sourceGroup : {};
    const groupBlankIds = (group.blankIds || []).map(String).filter((id) => validBlankIds.has(id));
    if (Array.isArray(group.answerPools)) {
      return groupBlankIds.length ? [{ blankIds: groupBlankIds, answerPools: answerPool(group.answerPools) }] : [];
    }
    if ('answerOptionId' in group) {
      const id = String(group.blankId || '');
      return validBlankIds.has(id) ? [{ answerOptionId: String(group.answerOptionId || ''), blankId: id }] : [];
    }
    if (Array.isArray(group.answerOptionIds)) {
      return groupBlankIds.length ? [{
  ...
  const covered = new Set(groups.flatMap((group) => group.blankIds || (group.blankId ? [group.blankId] : [])));
  return [
    ...groups,
    ...blankIds.filter((id) => !covered.has(id)).map((id) => ({ blankIds: [id], answerPools: [] })),
  ];
}

function canonicalElement(element) {
  const { elementKey: ignoredElementKey, elementId: ignoredElementId, ...content } = element || {};
  if (content.type === 'choice') {
    return {
      ...content,
      answers: {
        ...(content.answers && typeof content.answers === 'object' && !Array.isArray(content.answers)
          ? content.answers : {}),
        optionIds: Array.isArray(content.answers?.optionIds) ? content.answers.optionIds : [],
      },
    };
  }
  if (['fill', 'judgement', 'ordering', 'textMarker'].includes(content.type)) {
    return { ...content, answers: Array.isArray(content.answers) ? content.answers : [] };
  }
  if (['classification', 'matching', 'lineConnect', 'wordBuilder'].includes(content.type)) {
    return {
      ...content,
      answers: content.answers && typeof content.answers === 'object' && !Array.isArray(content.answers)
        ? content.answers : {},
    };
  }
...
function canonicalSerializedDraft(serialized) {
  ...
  return {
    questionTypeKey: rawQuestionTypeKey !== null && rawQuestionTypeKey !== undefined
      && Number.isInteger(Number(rawQuestionTypeKey))
      ? Number(rawQuestionTypeKey) : null,
...
function connectionColumnIndexes(columns = []) {
  return new Map(connectionColumns(columns).flatMap((itemIds, columnIndex) => (
...
export function toConnectionResponseAnswers(questionType, value, columns = []) {
  ...
  const normalizedEdges = connectionEdges(questionType, value).flatMap((edge) => {
    ...
    if (!Number.isInteger(fromColumnIndex) || !Number.isInteger(toColumnIndex)
      || Math.abs(fromColumnIndex - toColumnIndex) !== 1) return [];
    return fromColumnIndex < toColumnIndex ? [edge] : [{ from: edge.to, to: edge.from }];
  });
  return normalizedEdges.reduce((answers, edge) => ({
    ...answers,
    [edge.from]: [...new Set([...(answers[edge.from] || []), edge.to])],
  }), {});
}

export function isConnectionAnswerComplete(value, columns, { oneToOne = false } = {}) {
  const columnItemIds = connectionColumns(columns);
  if (columnItemIds.length < 2 || (oneToOne && columnItemIds.length !== 2)) return false;
  const allItemIds = columnItemIds.flat();
  if (!allItemIds.length || new Set(allItemIds).size !== allItemIds.length) return false;
  const itemColumnIndexes = connectionColumnIndexes(columns);
  ...
  for (const edge of edges) {
    const fromColumnIndex = itemColumnIndexes.get(edge.from);
    const toColumnIndex = itemColumnIndexes.get(edge.to);
    if (!Number.isInteger(fromColumnIndex) || toColumnIndex !== fromColumnIndex + 1) return false;
    coveredItems.add(edge.from);
    coveredItems.add(edge.to);
    coveredBoundaries.add(fromColumnIndex);
  }
  if (!allItemIds.every((itemId) => coveredItems.has(itemId))
    || coveredBoundaries.size !== columnItemIds.length - 1) return false;
  if (!oneToOne) return true;

  const sources = edges.map((edge) => edge.from);
  const targets = edges.map((edge) => edge.to);
  return edges.length === columnItemIds[0].length
...
export function readQuestionPlayerAnswer(response, questionType) {
  ...
  if (item?.type === 'fill' || item?.type === 'inlineFill') {
    const values = (item.answers || []).flatMap((group) => group.answerPools?.[0]?.text || []);
    return values.length > 1 ? values : values[0] || '';
  }
  if (item?.type === 'judgement') {
    return typeof item.answers?.[0] === 'boolean' ? String(item.answers[0]) : '';
  }
  if (item?.type === 'ordering' || item?.type === 'textMarker') return item.answers || [];
  if (['classification', 'matching', 'lineConnect', 'wordBuilder'].includes(item?.type)) {
    return item.answers || {};
  }
