...
export const storageKeys = {
  ...
  scratchPaperSession: (scope) => `adaptive-scratch-paper-session-v1-${encodeURIComponent(scope)}`,
  ...
  teacherStudentAccessToken: (periodId, studentId) => `adaptive-teacher-student-access-token-v1-${encodeURIComponent(periodId)}-${encodeURIComponent(studentId)}`,
  studentSupportSession: 'adaptive-student-support-session-v1',
  collapsedStudentHelpRequest: 'adaptive-collapsed-help-request',
  teacherAgentSession: (lessonId) => `adaptive-teacher-agent-session-v1-${encodeURIComponent(lessonId)}`,
};
