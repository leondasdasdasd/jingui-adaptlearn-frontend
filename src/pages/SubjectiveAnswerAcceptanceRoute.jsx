...
export default function SubjectiveAnswerAcceptanceRoute() {
  ...
  const [state, setState] = useState({ loading: true, error: '', published: null });

  useEffect(() => {
    let active = true;
    setState({ loading: true, error: '', published: null });
    loadPublishedLessonContent(lessonId).then((published) => {
      if (!active) return;
      if (!published) {
        setState({ loading: false, error: '这个课时还没有发布可验收的题目', published: null });
        return;
      }
      setState({ loading: false, error: '', published });
    }).catch((error) => {
      if (active) setState({ loading: false, error: error.message, published: null });
    });
    return () => { active = false; };
  }, [lessonId]);
  ...
  const question = questions.find((item) => item.id === requestedQuestionId)
    || questions.find((item) => item.type === 'short_answer');

  if (state.loading || state.error || !lesson || !question) {
    const title = state.loading
      ? '正在读取已发布问答题'
      : state.error || (!lesson ? '未找到指定课时' : '当前发布版本没有问答题');
    return (
      <AppShell title="问答题验收" eyebrow="功能验收" onBack={() => navigate(routes.directory)} compact>
        <StatePanel
          tone={state.loading ? 'loading' : 'error'}
          title={title}
          description={state.loading ? '正在同步老师已发布的真实题目…' : '请检查课时发布状态后重试。'}
        />
      </AppShell>
    );
  }
