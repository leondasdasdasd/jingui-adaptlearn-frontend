...
import {
  activeLearningUnit, advanceLessonFlow, finishTemporaryLearning, routeForLearningUnit, startRelearning,
  resolveKnowledgeVerification, selectIndependentVerificationQuestion, startDirectCheckpoint,
} from '../student/domain/learningPlan';
import { recordLearningEvent } from '../student/data/learningEventRepository';
import { readLearningAttempts, readLocalStudentIdentity } from '../student/data/learningHistoryRepository';
import {
  createTutoringSession, terminalStateForPracticeDecision, transitionTutoringSession,
} from '../shared/domain/tutoringStateMachine';
import {
  calculatePracticeRoundMastery, currentPracticeMastery,
...
function questionsForUnit(context, unit) {
  ...
  if (['knowledge_practice', 'knowledge_verification'].includes(unit?.kind)) {
    const pool = context.publishedContent?.knowledgePracticePools?.[unit.knowledgePointId] || [];
    if (unit.kind === 'knowledge_verification') {
      const question = selectIndependentVerificationQuestion(
        pool, context.postAttempts, unit.knowledgePointId,
      );
    ...
    const fallback = (context.postQuestions || []).filter((question) => (
      question.phase !== 'review' && question.knowledgePointIds?.includes(unit.knowledgePointId)
    ));
    const seen = new Set();
  ...
  if (unit?.kind === 'composite_review') return context.publishedContent?.compositeReviewPool || [];
  return context.postQuestions || [];
}

function recentAttemptsForKnowledgePoints(context, knowledgePoints = [], studentId = '') {
  const currentRows = [
    ['pre', context.preQuestions || [], context.preAttempts || {}],
    ['post', context.postQuestions || [], context.postAttempts || {}],
  ].flatMap(([assessmentMode, questions, attempts]) => questions.flatMap((question) => {
    const attempt = attempts[question.id];
    return attempt ? [{ ...attempt, assessmentMode, questionId: question.id, questionSnapshot: question }] : [];
  }));

  return Object.fromEntries(knowledgePoints.map((knowledgePoint) => {
    const current = currentRows.filter((row) => (
      (row.questionSnapshot?.knowledgePointIds || []).includes(knowledgePoint.id)
    ));
    const historical = studentId
      ? readLearningAttempts({ studentId, knowledgePointId: knowledgePoint.id })
      : [];
    const merged = new Map();
    [...current, ...historical].forEach((attempt) => {
...
export default function PostAssessmentRoute() {
  ...
  const contentVersionId = context.publishedContent?.versionId || selection?.contentVersionId;
  ...
  const studentScope = selection?.studentId || readLocalStudentIdentity()?.id || '';
  const recentAttemptsByKnowledgePoint = recentAttemptsForKnowledgePoints(
    context, selection?.knowledgePoints || [], studentScope,
  );
  ...
  useEffect(() => {
    ...
    recordLearningEvent({
      type: 'stage_entered', stage: unit?.kind || 'knowledge_practice',
      lessonTitle: selection.section.title, knowledgePointId: unit?.knowledgePointId || '',
    });
  }, [unit?.id, unit?.kind, unit?.knowledgePointId, selection?.section?.title]);

  if (unit && !['knowledge_practice', 'knowledge_verification', 'composite_review', 'enhancement_training'].includes(unit.kind)) {
  ...
  const complete = (attempts) => {
    ...
    if (unit?.kind === 'knowledge_verification') {
      ...
      const resolution = resolveKnowledgeVerification(flow, verificationAttempt, unit.id);
    ...
    if (flow?.mode === 'enhancement') {
      setSession((current) => ({
        ...
        result: roundResult,
        resultSource: 'preview',
        learningFlow: { ...current.learningFlow, activeUnit: null, context: null },
      }));
    ...
    if (flow?.mode === 'direct') {
      setSession((current) => ({
        ...current,
        ...(current.learningFlow.context ? {} : { postAttempts: mergedAttempts, result: roundResult, resultSource: 'preview' }),
    ...
    setSession((current) => ({
      ...current, postAttempts: mergedAttempts, result: roundResult, resultSource: 'preview', learningFlow: nextFlow,
    }));
    if (nextUnit) navigate(routeForLearningUnit(nextUnit, routes.complete));
    else navigate(routes.complete);
  };
  ...
  const intervene = (practiceIntervention) => {
    ...
    setSession((current) => ({
      ...
      remediationOpenMaic: {
        jobId: '', status: 'idle', step: '', progress: 0, message: '', classroomId: '', classroomUrl: '',
      },
    }));
    const transition = tutoringSession.transitions[0];
    recordLearningEvent({
      type: 'tutoring_state_transition', stage: 'check_in',
      knowledgePointId: tutoringSession.knowledgePointId, ...transition,
    });
    navigate(routes.checkIn, { state: { practiceIntervention, tutoringSession } });
  };

  const finishRevalidation = (practiceDecision) => {
    const targetState = terminalStateForPracticeDecision(session.tutoringSession?.state, practiceDecision);
    if (!targetState) return;
    const nextTutoring = transitionTutoringSession(session.tutoringSession, targetState, {
      reasonCode: practiceDecision.reason || (targetState === 'COMPLETE' ? 'REVALIDATION_PASSED' : 'REVALIDATION_NOT_PASSED'),
      evidenceQuestionIds: practiceDecision.questionId ? [practiceDecision.questionId] : [],
    });
    setSession((current) => ({ ...current, tutoringSession: nextTutoring }));
    recordLearningEvent({
      type: 'tutoring_state_transition', stage: 'knowledge_practice',
  ...
  return (
    <QuizPage
      ...
      revalidationKnowledgePointId={session.tutoringSession?.state === 'REVALIDATING'
        ? session.tutoringSession.knowledgePointId : ''}
      onRevalidationComplete={finishRevalidation}
      onLearnAgain={unit?.kind === 'knowledge_practice' ? () => {
        setSession((current) => ({
          ...current,
          learningFlow: startRelearning(current.learningFlow, unit.knowledgePointId),
        }));
