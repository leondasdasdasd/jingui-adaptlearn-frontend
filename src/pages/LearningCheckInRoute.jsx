...
import {
  diagnosisTargetState, transitionTutoringSession, tutoringStates,
...
export default function LearningCheckInRoute() {
  ...
  const practiceContext = session.practiceIntervention || location.state?.practiceIntervention;
  const tutoringSession = session.tutoringSession || location.state?.tutoringSession;
  const hasCompletePracticeContext = Boolean(
    practiceContext?.knowledgePointId && (practiceContext.evidence?.length || 0) >= 3,
  );
  ...
  const save = (patch) => setSession((current) => ({
    ...current,
    learningCheckIn: { ...(current.learningCheckIn || {}), ...patch },
  }));

  const askTeacher = async (nextMessages) => analyzeLearningCheckIn({
    lesson,
  ...
  useEffect(() => {
    ...
    if (messages.length || started.current) return;
    started.current = true;
    recordLearningEvent({ type: 'stage_entered', stage: 'check_in', lessonTitle: lesson.title });
    setSending(true);
    askTeacher([])
      .then((diagnosis) => {
        const occurredAt = new Date().toISOString();
        recordLearningEvent({ type: 'ai_tutor_message', role: 'assistant', content: diagnosis.reply, occurredAt });
        save({
          messages: [{ id: `assistant-${Date.now()}`, role: 'assistant', content: diagnosis.reply, occurredAt }],
          diagnosis: null,
        });
      })
  ...
  const send = async (content) => {
    const userMessage = { id: `user-${Date.now()}`, role: 'user', content, occurredAt: new Date().toISOString() };
    recordLearningEvent({ type: 'ai_tutor_message', role: 'user', content, occurredAt: userMessage.occurredAt });
    ...
    try {
      ...
      recordLearningEvent({ type: 'ai_tutor_message', role: 'assistant', content: diagnosis.reply, occurredAt: assistantMessage.occurredAt });
      if (diagnosis.ready) {
        const targetState = diagnosisTargetState(diagnosis);
        const nextTutoring = transitionTutoringSession(tutoringSession, targetState, {
          reasonCode: diagnosis.needsRemediation ? 'GENUINE_DIFFICULTY' : 'CAUSE_AND_METHOD_CONFIRMED',
        ...
        setSession((current) => ({
          ...
          learningCheckIn: {
            ...
            messages: [...nextMessages, assistantMessage], diagnosis,
          },
        }));
        recordLearningEvent({
        ...
        save({ messages: [...nextMessages, assistantMessage], diagnosis: null });
      }
    } catch (requestError) {
  ...
  const continuePractice = () => {
    const nextTutoring = transitionTutoringSession(tutoringSession, tutoringStates.REVALIDATING, {
    ...
    setSession((current) => ({ ...current, practiceIntervention: null, tutoringSession: nextTutoring }));
    recordLearningEvent({
