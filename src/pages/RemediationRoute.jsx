...
import { transitionTutoringSession, tutoringStates } from '../shared/domain/tutoringStateMachine';
...
export default function RemediationRoute() {
  ...
  const knowledgePoints = session.selection.knowledgePoints.filter((kp) => selectedIds.has(kp.id));
  const scopedKnowledgePoints = knowledgePoints.length ? knowledgePoints : session.selection.knowledgePoints;

  const saveRuntime = (patch) => setSession((current) => ({
    ...current,
    remediationOpenMaic: { ...(current.remediationOpenMaic || emptyRuntime), ...patch },
  }));

  const pollJob = async (jobId) => {
    while (true) {
      const job = await getOpenMaicJob(jobId);
      saveRuntime({ jobId, status: job.status, step: job.step, progress: job.progress, message: job.message });
      if (job.status === 'succeeded') {
        saveRuntime({
          status: 'succeeded', progress: 100, classroomId: job.result.classroomId, classroomUrl: job.result.url,
        });
        return;
      }
      if (job.status === 'failed') throw new Error(job.error || '重点讲解准备失败');
  ...
  const createClassroom = async () => {
    ...
    setError('');
    saveRuntime({ ...emptyRuntime, status: 'queued', step: 'queued', progress: 2 });
    try {
      ...
      if (response.status === 'succeeded' && response.result?.classroomId) {
        saveRuntime({
          ...
          classroomId: response.result.classroomId, classroomUrl: response.result.url,
        });
        return;
      }
      saveRuntime({ jobId: response.jobId, status: response.status, step: response.step });
  ...
  const finishRemediation = () => {
    const nextTutoring = transitionTutoringSession(session.tutoringSession, tutoringStates.REVALIDATING, {
  ...
  return (
    <OpenMaicPage
      ...
      runtimeCredentials={session.selection?.studentSessionId && session.selection?.classroomAccessToken ? {
