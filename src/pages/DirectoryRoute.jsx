...
import { buildKnowledgeProgress, readPostQuizDraft } from '../student/domain/learningProgress';
import { loadPublishedLessonContent, mapContentVersionToStudentLesson } from '../student/data/publishedLessonRepository';
import { clearAllQuizDrafts, restoreQuizDrafts } from '../student/data/studentSessionRepository';
...
import { createSelfStudySession, getStudentLearningPeriods, getStudentSessionContent, startStudentSession } from '../shared/infrastructure/classroomApi';
import { flushClassroomOutbox } from '../student/data/classroomSyncRepository';
import { activeLearningUnit, restoredStudentRoute, routeForLearningUnit } from '../student/domain/learningPlan';
...
const courseLessons = course.chapters.flatMap((chapter) => chapter.sections.map((section) => ({ chapter, section })));
const catalogKnowledgeById = Object.fromEntries(courseLessons.flatMap(({ section }) => (
  section.knowledgePoints.map((knowledgePoint) => [knowledgePoint.id, knowledgePoint])
)));

function selectionForPublishedContent(contentVersion, publishedLesson) {
  const content = contentVersion.contentPackage || {};
  const objectiveById = Object.fromEntries((content.knowledgeObjectives || []).map((item) => [item.id, item]));
  if (publishedLesson.planType === 'MULTI_LESSON') {
    const sourceLessonIds = publishedLesson.sourceLessons.map((item) => item.textbookLessonId || item.lessonId);
    const sources = sourceLessonIds.map((id) => courseLessons.find((item) => item.section.id === id)).filter(Boolean);
    const knowledgePoints = (content.knowledgeObjectives || []).map((objective) => ({
      ...(catalogKnowledgeById[objective.id] || {}), ...objective,
    }));
    return {
      chapter: { id: 'classroom-plan', title: sources.map((item) => item.chapter.title).filter((value, index, values) => values.indexOf(value) === index).join('·') },
      section: {
  ...
  const lesson = courseLessons.find((item) => item.section.id === contentVersion.textbookLessonId);
  ...
  return {
    ...
    knowledgePoints: knowledgePoints.length ? knowledgePoints : lesson.section.knowledgePoints,
    generationPolicy: publishedLesson.generationPolicy,
    questionDistribution: publishedLesson.questionDistribution,
  };
}
...
function authoritativeMasteryFromServer(states = {}) {
  return Object.fromEntries(Object.entries(states || {}).map(([knowledgePointId, state]) => {
    const mastery = Number(state.mastery);
    return [knowledgePointId, {
      id: knowledgePointId,
      mastery: Number.isFinite(mastery) ? mastery : null,
      masterySource: 'authoritative',
      status: state.masteryStatus === 'DETERMINED' && isMasteredValue(mastery) ? 'mastered' : 'needs_review',
...
function resumeState(session) {
  if (!session.selection?.section || !session.selection?.knowledgePoints?.length) return null;
  ...
  const learningUnit = session.learningFlow?.mode === 'direct'
    ? null
  ...
  const planHasNoTargets = Array.isArray(plan?.targetKnowledgePointIds)
    && plan.targetKnowledgePointIds.length === 0;
  const masteryCoversWholeLesson = session.selection.knowledgePoints.every((knowledgePoint) => {
    const item = session.preMastery?.[knowledgePoint.id] || session.result?.[knowledgePoint.id];
    return isMasteredValue(item?.mastery);
  });
  const lessonMastered = planHasNoTargets || masteryCoversWholeLesson;
  const currentLessonIndex = courseLessons.findIndex((item) => item.section.id === session.selection.section.id);
  ...
  const completedPlanKnowledgePointIds = (plan?.units || [])
    ...
    .filter((unit) => ['knowledge_checkpoint', 'knowledge_verification'].includes(unit.kind))
    .map((unit) => unit.knowledgePointId)
    .filter(Boolean);
  const currentQuestionId = draft.order?.[draft.index];
  const currentQuestion = session.postQuestions?.find((question) => question.id === currentQuestionId);
  const currentKnowledgePointId = session.practiceIntervention?.knowledgePointId
  ...
  const completedKnowledgePointIds = lessonMastered
    ...
    : learningUnit
    ? [...new Set([...completedPlanKnowledgePointIds, ...(draft.completedKpIds || [])])]
    : resultIds.length
      ? resultIds
  ...
  if (session.practiceIntervention) {
    return { phase: '错题回顾', actionLabel: '继续回顾', route: routes.checkIn, items, lessonId: session.selection.section.id, preAssessmentCompleted };
  }
  if (lessonMastered && nextLesson) {
    return {
      phase: '本课已掌握', actionLabel: '继续学习下一课', route: '', nextLessonId: nextLesson.section.id,
      items, lessonId: session.selection.section.id, preAssessmentCompleted,
    };
  }
  if (learningUnit) {
    const isLearning = ['composite_learning', 'knowledge_learning'].includes(learningUnit.kind);
    return {
      phase: isLearning ? '互动学习' : learningUnit.kind === 'composite_review' ? '综合练习' : '知识点练习',
      actionLabel: '继续学习',
      route: routeForLearningUnit(learningUnit, routes.complete), items, lessonId: session.selection.section.id, preAssessmentCompleted,
    };
  }
  if (resultIds.length) {
    return { phase: '本课已完成', actionLabel: '查看学习结果', route: routes.complete, items, lessonId: session.selection.section.id, preAssessmentCompleted };
  }
  const adaptivePreAssessmentComplete = Boolean(session.preAssessment?.completedAt);
  if (adaptivePreAssessmentComplete || isPreAssessmentComplete(session.preQuestions, session.preAttempts)) {
    return { phase: '课前小测完成', actionLabel: '查看学习重点', route: routes.preResult, items, lessonId: session.selection.section.id, preAssessmentCompleted };
  }
  if (session.preQuestions?.length) {
    return { phase: '课前小测', actionLabel: '继续小测', route: routes.preAssessment, items, lessonId: session.selection.section.id, preAssessmentCompleted };
  }
  return null;
}
...
export default function DirectoryRoute() {
  ...
  const [classroomState, setClassroomState] = useState({ status: 'loading', items: [], message: '' });
  ...
  const activeClassroom = useMemo(() => classroomDirectoryItemFromSession(session), [session]);
  ...
  const directoryMode = searchParams.get('view') === 'classroom' || searchParams.has('periodId') ? 'classroom' : 'textbook';
  const selectedClassroomId = searchParams.get('classroomId') || searchParams.get('periodId') || activeClassroom?.id || '';
  ...
  useEffect(() => {
    ...
    restorePersistentStudentState(accessToken, {
      ...
      .catch((error) => {
        ...
        setRestoreState({ loading: false, error: error.message || '学习进度恢复失败，请刷新重试' });
      });
    return () => {
      cancelled = true;
      controller.abort();
  ...
  const enterClassroom = useCallback(async (periodId, accessToken) => {
    if (!periodId || !accessToken) throw new Error('该课堂缺少进入凭证，请从老师发布的课堂入口进入');
    ...
    if (snapshot.hydrated?.session) {
      ...
      setSession({
        ...
        selection: {
          ...
          questionDistribution: contentVersion.contentPackage?.questionDistribution || null,
        },
      });
      await flushClassroomOutbox();
      const safeRoute = restoredStudentRoute(
        snapshot.hydrated.route, snapshot.hydrated.session.learningFlow, routes.preAssessment,
      );
      navigate(safeRoute, { replace: true });
      return;
    }
    const publishedLesson = preferUnseenPublishedContent(
      mapContentVersionToStudentLesson(contentVersion), serverSession.studentId,
    );
    const selection = selectionForPublishedContent(contentVersion, publishedLesson);
    clearAllQuizDrafts();
    setSession({
      ...emptySession,
      selection: {
  ...
  useEffect(() => {
    const controller = new AbortController();
    const accessToken = searchParams.get('accessToken') || classStudentIdentity?.accessToken || activeClassroom?.accessToken;
    if (!accessToken) {
      setClassroomState({ status: 'unavailable', items: [], message: '' });
      return () => controller.abort();
    }
    setClassroomState((current) => ({ ...current, status: 'loading', message: '' }));
    getStudentLearningPeriods(accessToken, { signal: controller.signal })
      .then((payload) => {
        const items = normalizeStudentClassroomDirectory(payload, course).map((item) => ({
          ...item,
    ...
    return () => controller.abort();
  }, [activeClassroom?.accessToken, classStudentIdentity?.accessToken, classroomReload, searchParams]);
  ...
  const openClassroom = (classroom) => {
    if (classroom.id === session.selection?.learningPeriodId && classroom.studentSessionId) {
  ...
  const start = async (selection) => {
    ...
    if (fixedIdentity) {
      try {
        serverSession = await createSelfStudySession(selection.section.id, fixedIdentity.accessToken);
        const serverContent = await getStudentSessionContent(serverSession.id, fixedIdentity.accessToken);
        published = mapContentVersionToStudentLesson(serverContent.contentVersion);
        authoritativeMastery = authoritativeMasteryFromServer(serverContent.masteryStates);
        ...
        setEntryState({ loading: false, error: error.message || '自主学习任务创建失败，请稍后重试' });
        return;
      }
    } else {
    ...
    const unseenPublished = preferUnseenPublishedContent(published, localStudent.studentId || localStudent.id);
    const publishedObjectiveById = Object.fromEntries(
      (unseenPublished.knowledgeObjectives || []).map((objective) => [objective.id, objective]),
    );
    const publishedKnowledgePoints = selection.section.knowledgePoints
      ...
      .map((knowledgePoint) => ({ ...knowledgePoint, ...publishedObjectiveById[knowledgePoint.id] }));
    const selectedKnowledgePoints = selection.knowledgePoints
      .filter((knowledgePoint) => publishedObjectiveById[knowledgePoint.id])
      .map((knowledgePoint) => ({ ...knowledgePoint, ...publishedObjectiveById[knowledgePoint.id] }));
    const availableKnowledgePoints = selectedKnowledgePoints.length
      ? selectedKnowledgePoints
      : publishedKnowledgePoints;
    if (!availableKnowledgePoints.length) {
      setEntryState({ loading: false, error: '这个课时的已发布内容与当前教材知识点不匹配，请联系老师重新发布' });
      return;
    }
    setSession({
      ...emptySession,
      selection: {
        ...
        studentId: serverSession?.studentId || localStudent.studentId || localStudent.id,
        studentName: serverSession?.studentName || fixedIdentity?.studentName || '当前学生（本机）',
  ...
  return (
    <>
    {restoreState.loading && <div className="classroom-entry-status" role="status">正在恢复学习进度…</div>}
    {entryState.loading && <div className="classroom-entry-status" role="status">正在进入课堂…</div>}
    {restoreState.error && <div className="classroom-entry-status error" role="alert">{restoreState.error}</div>}
    {entryState.error && <div className="classroom-entry-status error" role="alert">{entryState.error}</div>}
    {!restoreState.loading && (
    <DirectoryPage
      course={course}
      progress={progress ? { ...progress, lessonTitle: session.selection.section.title } : null}
      directoryMode={directoryMode}
      classroomDirectory={{ ...classroomState, items: classroomItems }}
      selectedClassroomId={selectedClassroomId}
      onModeChange={(mode) => updateDirectoryQuery(mode)}
      onSelectClassroom={(classroomId) => updateDirectoryQuery('classroom', classroomId)}
      onRetryClassrooms={() => setClassroomReload((value) => value + 1)}
      onEnterClassroom={openClassroom}
      onContinue={() => {
        if (!progress) return;
        if (progress.nextLessonId) {
          const lesson = courseLessons.find((item) => item.section.id === progress.nextLessonId);
          if (lesson) {
            start({ chapter: lesson.chapter, section: lesson.section, knowledgePoints: lesson.section.knowledgePoints });
            return;
          }
        }
      ...
      onLearnKnowledge={(knowledgePointId) => navigate(`${routes.knowledgeLearning(knowledgePointId)}?returnTo=${encodeURIComponent(routes.directory)}`)}
      onStart={start}
