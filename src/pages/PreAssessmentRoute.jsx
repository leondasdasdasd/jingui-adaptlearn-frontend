...
import { createAutomaticLearningPlan, createLessonLearningFlow } from '../student/domain/learningPlan';
...
export default function PreAssessmentRoute() {
  ...
  const historicalMastery = useMemo(() => prepareHistoricalMasteryForPolicy({
  ...
  }), [session.selection.contentVersionId, session.selection.generationPolicy, session.selection.knowledgePoints]);

  const loadQuestions = () => setError('这个已发布课时没有可用的诊断题，请联系老师检查发布内容');

  useEffect(() => {
    recordLearningEvent({ type: 'stage_entered', stage: 'pre_assessment', lessonTitle: session.selection.section.title });
    if (!session.preQuestions.length && !generationStarted.current) {
      const summary = advancePreAssessment({
        questions: [], attempts: {}, knowledgePoints: session.selection.knowledgePoints, historicalMastery,
      });
  ...
  }, [historicalMastery, session.preAssessment?.completedAt, session.preQuestions]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  if (error) {
    return (
      <GeneratingPage
        ...
        onRetry={() => { generationStarted.current = true; loadQuestions(); }}
        onBack={() => { resetSession(); navigate(routes.directory); }}
      />
    );
  }

  if (!session.preQuestions.length) return null;
  ...
  return (
    <QuizPage
      ...
      studentScope={session.selection.studentId || session.selection.studentSessionId || ''}
      onComplete={complete}
