...
function catalogIndexes(course = {}) {
  const entries = (course.chapters || []).flatMap((chapter) => (
    (chapter.sections || []).flatMap((lesson) => (
      (lesson.knowledgePoints || []).map((knowledgePoint) => ({ chapter, lesson, knowledgePoint }))
    ))
  ));
  return {
    knowledgeById: Object.fromEntries(entries.map((item) => [item.knowledgePoint.id, item])),
  };
}
...
function attemptKnowledgePointIds(attempt = {}) {
  ...
  const ids = snapshot.knowledgePointIds?.length
    ...
    : snapshot.knowledgeObjectiveIds?.length
      ...
      : [attempt.knowledgeObjectiveId || snapshot.primaryKnowledgePointId].filter(Boolean);
  return [...new Set(ids.filter(Boolean))];
}

function attemptOutcome(attempt = {}) {
  const disposition = String(attempt.answerContent?.disposition || '').toUpperCase();
  const gradingStatus = String(attempt.gradingResult?.gradingStatus || '').toLowerCase();
  ...
  if (!Number.isFinite(score) || !Number.isFinite(maxScore) || maxScore <= 0) return 'pending';
...
export function knowledgeProfileFromAuthority(profile, course) {
  ...
  (profile?.attempts || []).forEach((attempt) => {
    attemptKnowledgePointIds(attempt).forEach((id) => {
      attemptsByKnowledge.set(id, (attemptsByKnowledge.get(id) || 0) + 1);
    });
  });
  const statesById = Object.fromEntries((profile?.masteryStates || []).map((state) => [
    state.knowledgeObjectiveId, state,
  ]));
  const ids = new Set([...attemptsByKnowledge.keys(), ...Object.keys(statesById)]);
  return Object.fromEntries([...ids].map((id) => {
    const catalog = knowledgeById[id];
    const state = statesById[id];
    const mastery = Number.isFinite(Number(state?.mastery)) ? Math.round(Number(state.mastery)) : null;
    const evidenceCount = Number(state?.evidenceCount || attemptsByKnowledge.get(id) || 0);
    const determined = String(state?.masteryStatus || '').toUpperCase() === 'DETERMINED';
    const status = determined && mastery != null
...
export function mergeKnowledgeProfiles(localProfile = {}, authoritativeProfile = {}) {
  const merged = { ...localProfile };
  Object.entries(authoritativeProfile).forEach(([id, authoritative]) => {
    const local = localProfile[id] || {};
    const hasAuthoritativeMastery = authoritative.mastery != null
      && Number.isFinite(Number(authoritative.mastery));
    const hasLocalMastery = local.mastery != null && Number.isFinite(Number(local.mastery));
    const mastery = hasAuthoritativeMastery
      ? Math.round(Number(authoritative.mastery))
      : hasLocalMastery ? Math.round(Number(local.mastery)) : null;
    const localHasFinalStatus = local.masterySource === 'authoritative'
    ...
    merged[id] = {
      ...
      masterySource: hasAuthoritativeMastery
        ...
        : hasLocalMastery ? local.masterySource : authoritative.masterySource,
      status: hasAuthoritativeMastery
        ? authoritative.status
        : localHasFinalStatus ? local.status : 'studying',
    };
  });
  return merged;
}
...
export function attemptsFromAuthority(profile, course) {
  const { knowledgeById } = catalogIndexes(course);
  return (profile?.attempts || []).map((attempt, index) => {
    const knowledgePointIds = attemptKnowledgePointIds(attempt);
    const catalog = knowledgeById[knowledgePointIds[0]];
    const maxScore = Number(attempt.maxScore);
    const score = Number(attempt.score);
    return {
      historyId: attempt.studentSessionId || profile?.currentSession?.id || 'authoritative-history',
      attemptId: attempt.clientSubmissionId || attempt.id || `authoritative-${index}`,
      ...
      questionSnapshot: attempt.questionSnapshot || { id: attempt.questionId },
      questionId: attempt.questionId || attempt.questionSnapshot?.id || `authoritative-${index}`,
      attemptType: attemptType(attempt),
      questionType: attempt.questionSnapshot?.type || '',
      outcome: attemptOutcome(attempt),
      scoreRatio: Number.isFinite(score) && Number.isFinite(maxScore) && maxScore > 0
        ? score / maxScore : null,
      knowledgePointIds,
      knowledgePoints: knowledgePointIds.map((id) => knowledgeById[id]?.knowledgePoint.name || id),
      lesson: catalog ? {
      ...
      } : { id: '', index: '', title: '课堂学习', chapterId: '', chapterTitle: '' },
      answer: attemptAnswer(attempt),
      recognizedAnswer: attempt.gradingResult?.recognizedAnswer,
      correctAnswer: attempt.gradingResult?.correctAnswer ?? attempt.questionSnapshot?.answer,
...
export function mergeLearningAttempts(localAttempts = [], authoritativeAttempts = []) {
  const authoritativeKeys = new Set(authoritativeAttempts.flatMap((attempt) => [
    attempt.attemptId,
  ...
  return [...authoritativeAttempts, ...localOnly]
    .sort((left, right) => String(right.submittedAt || '').localeCompare(String(left.submittedAt || '')));
}
