...
export function isPreAssessmentGateSatisfied(session = {}) {
  ...
  return Boolean(context.preAssessment?.completedAt)
    || isPreAssessmentComplete(context.preQuestions || [], context.preAttempts || {});
}
...
export function isPreAssessmentProgressEstablished(session = {}) {
  ...
  const hasPracticeAttempts = Object.keys(context.postAttempts || {}).length > 0;
  const hasLearningResult = Object.values(context.result || {}).some((item) => (
    Number(item?.evidenceCount ?? item?.total ?? 0) > 0
  ));
