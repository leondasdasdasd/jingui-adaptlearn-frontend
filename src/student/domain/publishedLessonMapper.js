...
export function mapContentVersionToStudentLesson(version) {
  const content = normalizePublishedContentPackage(version?.contentPackage || {});
  ...
  return {
    ...
    preQuestions: questionPool.filter((item) => item.purpose?.toUpperCase() === 'PRE').map(withContentVersion),
    postQuestions: questionPool.filter((item) => ['PRACTICE', 'POST'].includes(item.purpose?.toUpperCase())).map(withContentVersion),
    learningContent: content.learningContent,
    knowledgePracticePools: Object.fromEntries(
      Object.entries(content.knowledgePracticePools || {}).map(([knowledgePointId, questions]) => [
        knowledgePointId,
        (questions || []).map(withContentVersion),
      ]),
    ),
    compositeReviewPool: (content.compositeReviewPool || []).map(withContentVersion),
  };
}
