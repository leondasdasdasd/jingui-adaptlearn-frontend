...
const primaryKnowledgePointId = (question = {}) => knowledgeEvidenceProfile(question).primaryKnowledgePointId;

function scoreRatio(attempt = {}) {
  if (Number.isFinite(Number(attempt.scoreRatio))) return Math.max(0, Math.min(1, Number(attempt.scoreRatio)));
...
function answeredForKnowledgePoint(questions, attempts, knowledgePointId) {
  return questions
    .map((question, questionIndex) => ({ question, questionIndex, attempt: attempts[question.id] }))
    .filter(({ question, attempt }) => attempt?.submittedAt && primaryKnowledgePointId(question) === knowledgePointId)
    .sort((left, right) => new Date(left.attempt.submittedAt).getTime() - new Date(right.attempt.submittedAt).getTime()
      || left.questionIndex - right.questionIndex);
}

export function hasStableRecentHistory(history = {}, now = Date.now()) {
  if (history?.masterySource !== 'authoritative' || history?.status !== 'mastered') return false;
  if (!isMasteredValue(history.mastery)) return false;
  const confidence = Number(history.confidence || 0) <= 1
    ? Number(history.confidence || 0) * 100 : Number(history.confidence || 0);
  if (confidence < 50 || Number(history.evidenceCount || 0) < 3) return false;
  const updatedAt = new Date(history.updatedAt || 0).getTime();
  return Number.isFinite(updatedAt) && updatedAt > 0 && now - updatedAt <= HISTORY_MAX_AGE_MS;
}

export function prepareHistoricalMasteryForPolicy({
  knowledgePoints = [], historicalMastery = {}, generationPolicy, now = Date.now(),
}) {
  ...
  return Object.fromEntries(knowledgePoints.map((knowledgePoint) => {
    ...
    if (modules.masteredKnowledgePointPolicy === 'FORCE_LEARN') preAssessmentDisposition = bypassPreAssessment ? 'NEEDS_LEARNING' : '';
    else if (stableMastery && modules.masteredKnowledgePointPolicy === 'SKIP') preAssessmentDisposition = 'MASTERED';
    else if (bypassPreAssessment) preAssessmentDisposition = stableMastery ? 'MASTERED' : 'NEEDS_LEARNING';
...
function targetDifficulty(evidence = []) {
  if (!evidence.length) return 'D3';
  const latest = evidence.at(-1);
  const current = difficultyRank(latest.difficulty);
  if (latest.scoreRatio >= CORRECT_THRESHOLD) return U1_DIFFICULTIES[Math.min(4, current + 1)];
  if (latest.scoreRatio < FIRST_WRONG_THRESHOLD) return U1_DIFFICULTIES[Math.max(0, current - 1)];
  return normalizeU1Difficulty(latest.difficulty);
}

function evidenceFor(questions, attempts, knowledgePointId) {
  return answeredForKnowledgePoint(questions, attempts, knowledgePointId).map(({ question, attempt }) => ({
...
export function diagnosePreAssessmentKnowledgePoint({
  questions = [], attempts = {}, knowledgePointId, history = {}, now = Date.now(),
}) {
  const evidence = evidenceFor(questions, attempts, knowledgePointId);
  const recentHistory = hasStableRecentHistory(history, now);
  const base = {
    knowledgePointId,
    status: evidence.length ? PRE_DIAGNOSIS_STATUS.ASSESSING : PRE_DIAGNOSIS_STATUS.NOT_ASSESSED,
    reason: evidence.length ? 'MORE_EVIDENCE_REQUIRED' : 'NOT_STARTED',
    confidence: evidence.length ? 25 : 0,
    evidenceCount: evidence.length,
    historicalEvidenceUsed: false,
    evidence,
  };
  if (history?.preAssessmentDisposition === 'MASTERED') {
    return { ...base, status: PRE_DIAGNOSIS_STATUS.PROVISIONALLY_MASTERED, reason: 'STABLE_HISTORY_ACCEPTED', confidence: 85, historicalEvidenceUsed: true };
  }
  if (history?.preAssessmentDisposition === 'NEEDS_LEARNING') {
    return { ...base, status: PRE_DIAGNOSIS_STATUS.NEEDS_LEARNING, reason: 'ASSESSMENT_NOT_REQUIRED', confidence: recentHistory ? 70 : 0, historicalEvidenceUsed: recentHistory };
  }
  if (!evidence.length) return base;
  if (recentHistory && evidence.length === 1 && evidence[0].scoreRatio >= CORRECT_THRESHOLD
    && difficultyRank(evidence[0].difficulty) >= difficultyRank('D3')) {
    return { ...base, status: PRE_DIAGNOSIS_STATUS.PROVISIONALLY_MASTERED, reason: 'RECENT_MASTERY_VERIFIED', confidence: 85, historicalEvidenceUsed: true };
  }
  if (evidence[0].scoreRatio < FIRST_WRONG_THRESHOLD) {
    return { ...base, status: PRE_DIAGNOSIS_STATUS.NEEDS_LEARNING, reason: 'FIRST_WRONG_STOPPED', confidence: 45 };
  }
  ...
  if (evidence.length >= MAX_QUESTIONS_PER_KP) {
    const average = evidence.reduce((sum, item) => sum + item.scoreRatio, 0) / evidence.length;
    return {
      ...base,
      status: average >= CORRECT_THRESHOLD ? PRE_DIAGNOSIS_STATUS.PROVISIONALLY_MASTERED : PRE_DIAGNOSIS_STATUS.UNCERTAIN,
      reason: average >= CORRECT_THRESHOLD ? 'THREE_RESPONSES_TARGET_REACHED' : 'THREE_RESPONSES_LIMIT',
      confidence: average >= CORRECT_THRESHOLD ? 70 : 55,
    };
  }
  return { ...base, reason: evidence.length === 1 ? 'FIRST_CORRECT_RAISE_DIFFICULTY' : 'CONTINUE_SAME_KNOWLEDGE_POINT', confidence: evidence.length === 2 ? 55 : 25 };
}

export function buildPreAssessmentDiagnosis({
  questions = [], attempts = {}, knowledgePoints = [], historicalMastery = {}, now = Date.now(),
}) {
  return Object.fromEntries(knowledgePoints.map((knowledgePoint) => [knowledgePoint.id, diagnosePreAssessmentKnowledgePoint({
    questions, attempts, knowledgePointId: knowledgePoint.id, history: historicalMastery[knowledgePoint.id], now,
  })]));
}
...
function selectCandidate({ questions, attempts, knowledgePointId, evidence }) {
  const candidates = questions.filter((question) => !attempts[question.id]
...
export function selectNextPreAssessmentQuestion({
  questions = [], attempts = {}, knowledgePoints = [], historicalMastery = {},
}) {
  const diagnosis = buildPreAssessmentDiagnosis({ questions, attempts, knowledgePoints, historicalMastery });
  if (firstErrorStreak({ knowledgePoints, diagnosis }) >= GLOBAL_FIRST_ERROR_LIMIT) return null;
  ...
  for (const knowledgePoint of knowledgePoints) {
    ...
    const candidate = selectCandidate({
      questions, attempts, knowledgePointId: knowledgePoint.id, evidence: item.evidence,
    });
...
export function advancePreAssessment({
  questions = [], attempts = {}, knowledgePoints = [], historicalMastery = {}, currentQuestion = null,
}) {
  const diagnosisByKnowledgePoint = buildPreAssessmentDiagnosis({ questions, attempts, knowledgePoints, historicalMastery });
  const globalStopped = firstErrorStreak({ knowledgePoints, diagnosis: diagnosisByKnowledgePoint }) >= GLOBAL_FIRST_ERROR_LIMIT;
  const nextQuestion = globalStopped ? null : selectNextPreAssessmentQuestion({ questions, attempts, knowledgePoints, historicalMastery });
  const resolvedKnowledgePointCount = Object.values(diagnosisByKnowledgePoint).filter((item) => isTerminalPreDiagnosis(item.status)).length;
  const answeredCount = Object.values(attempts).filter((attempt) => attempt?.submittedAt).length;
  const assessmentComplete = globalStopped || (knowledgePoints.length > 0 && !nextQuestion
    && (answeredCount > 0 || resolvedKnowledgePointCount === knowledgePoints.length));
  const currentKnowledgePointId = primaryKnowledgePointId(currentQuestion || {});
  return {
...
export function calculatePreAssessmentProgress(summary = {}) {
  ...
  if (!total) return 0;
  const progressUnits = Object.values(summary.diagnosisByKnowledgePoint || {})
    .reduce((sum, diagnosis = {}) => {
      if (isTerminalPreDiagnosis(diagnosis.status)) return sum + 1;
      return sum + Math.min(1, Math.max(0, Number(diagnosis.evidenceCount || 0)) / MAX_QUESTIONS_PER_KP);
    }, 0);
  return Math.max(0, Math.min(100, Math.round((progressUnits / total) * 100)));
}

export function createPreAssessmentState({ questions = [], knowledgePoints = [], historicalMastery = {} }) {
  const firstQuestion = selectNextPreAssessmentQuestion({ questions, attempts: {}, knowledgePoints, historicalMastery });
  return { order: firstQuestion ? [firstQuestion.id] : [], targetByKp: {} };
}

export function mergePreAssessmentDiagnosisIntoMastery(mastery = {}, diagnosisByKnowledgePoint = {}, historicalMastery = {}) {
  return Object.fromEntries(Object.entries(mastery).map(([knowledgePointId, item]) => {
    ...
    return [knowledgePointId, {
      ...item,
      ...(acceptedHistory ? { mastery: history.mastery, evidenceCount: history.evidenceCount, evidenceWeight: history.evidenceCount } : {}),
      diagnosisStatus: diagnosis.status || PRE_DIAGNOSIS_STATUS.NOT_ASSESSED,
      ...
      notAssessed: diagnosis.status === PRE_DIAGNOSIS_STATUS.NOT_ASSESSED || item.evidenceCount === 0,
    }];
  }));
}
