import { isMasteredValue, MASTERY_THRESHOLD } from '../../shared/domain/masteryPolicy.js';
...
const hasMastered = (mastery = {}) => Number(mastery.evidenceCount || 0) >= 3
  && isMasteredValue(mastery.mastery);
const isProvisionallyMastered = (mastery = {}) => (
  mastery.diagnosisStatus === 'provisionally_mastered' && !hasMastered(mastery)
);

export function createAutomaticLearningPlan(knowledgePoints, mastery, generationPolicy) {
  ...
  const targetIds = knowledgePoints
    .filter((item) => forceLearn || (
      !hasMastered(mastery[item.id]) && !isProvisionallyMastered(mastery[item.id])
    ))
    .map((item) => item.id);
  const masteredIds = forceLearn ? [] : knowledgePoints
    .filter((item) => hasMastered(mastery[item.id]))
    .map((item) => item.id);
  const provisionalIds = forceLearn ? [] : knowledgePoints
  ...
  if (!targetIds.length && !provisionalIds.length) {
    return {
      source: 'lesson_flow',
  ...
  const units = [];
  if (targetIds.length === knowledgePoints.length && modules.compositeExplanation) {
    units.push({ id: 'composite-learning', kind: 'composite_learning' });
  }
  targetIds.forEach((knowledgePointId) => {
    if (targetIds.length !== knowledgePoints.length || !modules.compositeExplanation) {
      units.push({ id: `learn-${knowledgePointId}`, kind: 'knowledge_learning', knowledgePointId });
    }
    units.push({ id: `practice-${knowledgePointId}`, kind: 'knowledge_practice', knowledgePointId });
    units.push({ id: `checkpoint-${knowledgePointId}`, kind: 'knowledge_checkpoint', knowledgePointId });
  });
  const verificationIds = [
    ...provisionalIds,
    ...(modules.masteredKnowledgePointPolicy === 'VERIFY_ONCE' ? masteredIds : []),
  ];
  verificationIds.forEach((knowledgePointId) => {
    units.push({
  ...
  if (modules.postAssessment && (targetIds.length > 0 || verificationIds.length > 0)) {
...
export function selectIndependentVerificationQuestion(questions = [], attempts = {}, knowledgePointId = '') {
  const candidates = questions.filter((question) => (
    question.phase !== 'review' && question.knowledgePointIds?.[0] === knowledgePointId
  ));
...
export function resolveKnowledgeVerification(flow, result = {}, verificationUnitId = '') {
  ...
  const evidenceIsFinal = result.gradingStatus === 'final'
    && result.evidenceEligible !== false;
  const firstAttemptWasFullyCorrect = result.correctionAttempted !== true
    && (result.initialScoreRatio == null || Number(result.initialScoreRatio) >= 0.999);
  const fullyCorrect = result.correct === true
  ...
  const replacement = [
    { id: `learn-${knowledgePointId}`, kind: 'knowledge_learning', knowledgePointId },
    { id: `practice-${knowledgePointId}`, kind: 'knowledge_practice', knowledgePointId },
    { id: `checkpoint-${knowledgePointId}`, kind: 'knowledge_checkpoint', knowledgePointId },
  ];
  ...
  const nextPlan = {
    ...plan,
    units,
    masteredKnowledgePointIds: (plan.masteredKnowledgePointIds || [])
      .filter((id) => id !== knowledgePointId),
    targetKnowledgePointIds: [...new Set([
      ...(plan.targetKnowledgePointIds || []), knowledgePointId,
    ])],
  };
  return {
    flow: {
...
export function ensureLearningPlanCheckpoints(plan) {
  if (!plan?.units?.length || plan.units.some((unit) => unit.kind === 'knowledge_checkpoint')) return plan;
  const currentUnitId = plan.units[plan.currentIndex]?.id;
  const units = plan.units.flatMap((unit) => unit.kind === 'knowledge_practice'
    ? [unit, {
  ...
  const currentIndex = currentUnitId
    ? Math.max(0, units.findIndex((unit) => unit.id === currentUnitId))
    : Math.min(units.length, plan.currentIndex || 0);
  return { ...plan, units, currentIndex };
}

export function createLessonLearningFlow(plan) {
  return { mode: 'lesson_flow', plan, activeUnit: null, context: null, returnTo: '', returnUnit: null, returnMode: null };
}

export function startDirectLearning(flow, knowledgePointId, context, returnTo) {
  return {
    ...flow, mode: 'direct', context, returnTo,
    returnUnit: null,
    activeUnit: { id: `direct-learn-${knowledgePointId}`, kind: 'knowledge_learning', knowledgePointId },
  };
}
...
export function startDirectPractice(flow) {
  ...
  return {
    ...flow,
    activeUnit: { id: `direct-practice-${knowledgePointId}`, kind: 'knowledge_practice', knowledgePointId },
  };
}
...
export function startDirectCheckpoint(flow) {
  ...
  return {
    ...flow,
    activeUnit: { id: `direct-checkpoint-${knowledgePointId}`, kind: 'knowledge_checkpoint', knowledgePointId },
  };
}
...
export function startRelearning(flow, knowledgePointId) {
  return {
    ...
    activeUnit: { id: `relearn-${knowledgePointId}`, kind: 'knowledge_learning', knowledgePointId },
  };
}
...
export function finishTemporaryLearning(flow) {
  if (flow?.mode === 'relearn') {
    return {
      ...flow, mode: flow.returnMode || 'lesson_flow', activeUnit: flow.returnUnit || null,
      returnUnit: null, returnMode: null,
    };
  }
  return { ...flow, mode: 'lesson_flow', activeUnit: null, context: null, returnTo: '', returnUnit: null, returnMode: null };
}

export function advanceLessonFlow(flow) {
  const plan = advanceLearningPlan(ensureLearningPlanCheckpoints(flow?.plan));
  return { ...flow, mode: 'lesson_flow', plan, activeUnit: null, context: null, returnTo: '', returnUnit: null, returnMode: null };
}
...
export function advanceLearningPlan(plan) {
  if (!plan) return null;
  return { ...plan, currentIndex: Math.min(plan.units.length, plan.currentIndex + 1) };
}

export function routeForLearningUnit(unit, fallback = '/adaptive-learning/session/complete') {
  if (!unit) return fallback;
  if (unit.kind === 'knowledge_checkpoint') return '/adaptive-learning/session/knowledge-checkpoint';
...
export function restoredStudentRoute(snapshotRoute, flow, fallback = '/adaptive-learning/session/pre-assessment') {
  const route = String(snapshotRoute || '');
  const safeStudentRoute = route.startsWith('/adaptive-learning/')
  ...
  const resumableLearningRoute = /^\/adaptive-learning\/session\/(?:pre-assessment|pre-result|learning|check-in|remediation|post-assessment|knowledge-checkpoint|complete)$/.test(path)
