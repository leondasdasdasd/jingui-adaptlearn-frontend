...
import { studentRequest, uploadStudentSessionMedia } from '../../shared/infrastructure/classroomApi.js';
...
function nextSequence() {
  ...
  const sequence = Math.max(Number.isSafeInteger(current) ? current + 1 : 1, Date.now() * 1000);
...
async function flushEventGroup(group) {
  ...
  await studentRequest(`/api/v1/student-sessions/${group.sessionId}/events`, group.accessToken, {
...
async function flushAnswer(record) {
  ...
  if (record.imageDataUrl && !submission.answerContent?.mediaId) {
    ...
    const media = await uploadStudentSessionMedia(record.sessionId, record.accessToken, {
      blob: new Blob([bytes], { type: match[1] }),
      filename: record.imageName || `answer-${record.outboxId}`,
      idempotencyKey: `answer-image-${record.outboxId}`,
      metadata: { purpose: 'ANSWER_SUBMISSION', questionId: submission.questionId },
    });
    submission = {
      ...submission,
      answerContent: {
  ...
  return studentRequest(`/api/v1/student-sessions/${record.sessionId}/answers`, record.accessToken, {
...
function groupedEvents(records) {
  ...
  records.forEach((record) => {
    ...
    if (!groups.has(key)) groups.set(key, { sessionId: record.sessionId, accessToken: record.accessToken, records: [] });
  ...
  try {
    ...
    for (const group of groupedEvents(events)) {
      try {
        ...
        writeJson(storageKeys.classroomEventOutbox, readJson(storageKeys.classroomEventOutbox, []).filter((item) => !uploaded.has(item.outboxId)));
      } catch (error) {
        // 保留本组，恢复网络后使用相同 clientSequence 重传。
        const failedIds = new Set(group.records.map((item) => item.outboxId));
        writeJson(storageKeys.classroomEventOutbox, readJson(storageKeys.classroomEventOutbox, []).map((item) => (
          failedIds.has(item.outboxId) ? { ...item, ...syncFailure(error, item) } : item
        )));
      }
    }
    for (const record of readJson(storageKeys.classroomAnswerOutbox, [])) {
      try {
        ...
        writeJson(storageKeys.classroomAnswerOutbox, readJson(storageKeys.classroomAnswerOutbox, []).filter((item) => item.outboxId !== record.outboxId));
        if (typeof CustomEvent !== 'undefined') window.dispatchEvent?.(new CustomEvent('adaptive-classroom-answer-synced', {
          detail: { sessionId: record.sessionId, clientSubmissionId: record.submission.clientSubmissionId, answer: authoritativeAnswer },
        }));
      } catch (error) {
        // 作答使用稳定 clientSubmissionId，重复重传不会重复生成证据。
        writeJson(storageKeys.classroomAnswerOutbox, readJson(storageKeys.classroomAnswerOutbox, []).map((item) => (
          item.outboxId === record.outboxId ? { ...item, ...syncFailure(error, item) } : item
        )));
      }
    }
  } finally {
...
export function getClassroomOutboxStatus(sessionId = '') {
  ...
  const answerRecords = readJson(storageKeys.classroomAnswerOutbox, []).filter(forSession);
  const eventRecords = readJson(storageKeys.classroomEventOutbox, []).filter(forSession);
  const latestFailure = [...answerRecords, ...eventRecords]
    .filter((item) => item.lastError)
    .sort((left, right) => String(right.lastAttemptAt).localeCompare(String(left.lastAttemptAt)))[0];
  return {
...
export function enqueueClassroomEvent(event, occurredAt = new Date().toISOString(), credentials = null) {
  const { sessionId, accessToken } = activeCredentials(credentials);
  if (!sessionId || !accessToken) return;
  append(storageKeys.classroomEventOutbox, {
    outboxId: createClientId(), sessionId, accessToken, clientSequence: nextSequence(), occurredAt, event,
  });
  void flushClassroomOutbox();
}

export function enqueueAnswerSubmission({ question, attempt, mode, image = null, credentials = null }) {
  const { sessionId, accessToken } = activeCredentials(credentials);
  if (!sessionId || !accessToken) return;
  const knowledgePointIds = question.knowledgePointIds?.length ? question.knowledgePointIds : ['unmapped'];
  const rawWeights = question.knowledgePointWeights || {};
  const primaryKnowledgePointId = question.primaryKnowledgePointId || knowledgePointIds[0] || '';
  ...
  const scoreForKnowledgePoint = (id) => {
    const questionEvidence = questionEvidenceMap.filter((item) => item.knowledgePointId === id);
    const scoringPointScores = attempt.scoringPointScores && typeof attempt.scoringPointScores === 'object'
      ? attempt.scoringPointScores : {};
    if (questionEvidence.length && Object.keys(scoringPointScores).length) {
      const rows = questionEvidence.map((item) => scoringPointScores[item.scoringPointId]).filter(Boolean);
      ...
      const weightedConfidence = rows.reduce((sum, item) => (
        sum + Number(item.gradingConfidence ?? attempt.gradingConfidence ?? attempt.confidence ?? 0.9)
          * Number(item.maxScore || 0)
      ), 0);
      if (Number.isFinite(score) && Number.isFinite(maxScore) && maxScore > 0) {
        return { score, maxScore, gradingConfidence: weightedConfidence / maxScore };
      }
      return null;
    }
    const firstEvidence = questionEvidence[0];
    const attemptEvidence = attemptEvidenceMap.find((item) => item.knowledgePointId === id)
      || attempt.knowledgePointScores?.[id]
      || attempt.knowledgeObjectiveScores?.[id];
    const scoringPointId = firstEvidence?.scoringPointId || firstEvidence?.subQuestionId;
    ...
    return Number.isFinite(score) && Number.isFinite(maxScore) && maxScore > 0
      ? { score, maxScore, gradingConfidence: Number(resolved?.gradingConfidence ?? attempt.gradingConfidence ?? attempt.confidence ?? 0.9) }
      : null;
  };
  ...
  const knowledgeObjectives = knowledgePointIds.flatMap((id) => {
    ...
    if (compoundRequiresIndependentScores && id !== primaryKnowledgePointId && !independentScore) return [];
    return [{
      id,
      weight: Math.max(0.05, Math.min(1, Number(rawWeights[id] || (id === primaryKnowledgePointId ? 1 : 0.3)))),
      ...(independentScore || {}),
    }];
  });
  ...
  append(storageKeys.classroomAnswerOutbox, {
      ...
      submission: {
        ...
        secondaryKnowledgePointIds: (question.knowledgePointIds || []).filter((id) => id !== primaryKnowledgePointId),
        knowledgeObjectives,
        ...
        itemConfidence: Math.max(0, Math.min(1, Number(attempt.confidence ?? 0.9))),
        ...
        gradingConfidence: Number(attempt.gradingConfidence ?? attempt.confidence ?? 0.9),
        gradingResult: { preview: true },
        attemptStage: attempt.attemptStage === 'correction' || attempt.correctionAttempted
          ? 'correction' : 'initial',
...
if (typeof window !== 'undefined') window.addEventListener('online', () => { void flushClassroomOutbox(); });
