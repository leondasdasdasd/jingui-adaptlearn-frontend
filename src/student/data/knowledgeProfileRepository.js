...
function knowledgePointIdsForQuestion(question = {}) {
  const ids = question.knowledgePointIds?.length
    ...
    : question.knowledgeObjectiveIds?.length
      ...
      : [question.primaryKnowledgePointId || question.primaryKnowledgeObjectiveId].filter(Boolean);
  return ids;
}
...
export function markKnowledgePointLearned(knowledgePoint, lessonId = '') {
  ...
  const next = {
    ...
    [knowledgePoint.id]: {
      ...
      name: knowledgePoint.name || current[knowledgePoint.id]?.name || knowledgePoint.id,
      lessonId,
      status: current[knowledgePoint.id]?.mastery != null
...
export function syncKnowledgeProfileFromSession(session) {
  ...
  const questions = [...(session.preQuestions || []), ...(session.postQuestions || [])];
  const attemptedQuestionIds = [...Object.keys(session.preAttempts || {}), ...Object.keys(session.postAttempts || {})];
  const attemptedKnowledgePointIds = new Set(attemptedQuestionIds.flatMap((questionId) => (
    knowledgePointIdsForQuestion(questions.find((question) => question.id === questionId))
  )));
  const lastAttemptedQuestion = questions.find((question) => question.id === attemptedQuestionIds.at(-1));
  const plannedUnit = session.learningFlow?.activeUnit
    || session.learningFlow?.plan?.units?.[session.learningFlow?.plan?.currentIndex];
  ...
  for (const knowledgePoint of knowledgePoints) {
    const finalResult = session.resultSource === 'authoritative' ? session.result?.[knowledgePoint.id] : null;
    const mastery = finalResult?.mastery ?? next[knowledgePoint.id]?.mastery ?? null;
    const hasEvidence = Boolean(finalResult || attemptedKnowledgePointIds.has(knowledgePoint.id));
    if (!hasEvidence && knowledgePoint.id !== currentKnowledgePointId) {
      if (next[knowledgePoint.id]?.mastery == null
    ...
    const status = finalResult?.mastery != null
      ? isMasteredValue(mastery) ? 'mastered' : 'needs_review'
      : knowledgePoint.id === currentKnowledgePointId ? 'studying' : 'learned';
    next[knowledgePoint.id] = {
      ...next[knowledgePoint.id],
      id: knowledgePoint.id,
      name: knowledgePoint.name,
      lessonId: session.selection?.section?.id || '',
      mastery: mastery != null && Number.isFinite(Number(mastery)) ? Math.round(Number(mastery)) : null,
      masterySource: finalResult?.mastery != null ? 'authoritative' : next[knowledgePoint.id]?.masterySource || '',
      confidence: finalResult?.confidence ?? next[knowledgePoint.id]?.confidence ?? 0,
      evidenceCount: finalResult?.evidenceCount ?? next[knowledgePoint.id]?.evidenceCount ?? 0,
      status,
      updatedAt: new Date().toISOString(),
    };
  }
...
export function buildKnowledgeMapProfile(session) {
  ...
  const knowledgePoints = session.selection?.knowledgePoints || [];
  const attemptedPreQuestionIds = new Set(Object.keys(session.preAttempts || {}));
  const attemptedQuestionIds = new Set(Object.keys(session.postAttempts || {}));
  const attemptedPreKnowledgePointIds = new Set((session.preQuestions || []).flatMap((question) => (
    attemptedPreQuestionIds.has(question.id) ? knowledgePointIdsForQuestion(question) : []
  )));
  const attemptedKnowledgePointIds = new Set((session.postQuestions || []).flatMap((question) => (
    attemptedQuestionIds.has(question.id) ? knowledgePointIdsForQuestion(question) : []
  )));

  for (const knowledgePoint of knowledgePoints) {
    const result = session.result?.[knowledgePoint.id];
    const practiceEvidenceCount = Number(result?.evidenceCount ?? result?.total ?? 0);
    ...
    const hasPracticeEvidence = Number.isFinite(practiceMastery) && (
      practiceEvidenceCount > 0 || attemptedKnowledgePointIds.has(knowledgePoint.id)
    );
    const hasPreEvidence = Number.isFinite(preMastery) && (
    ...
    const source = hasPracticeEvidence ? 'preview' : hasPreEvidence ? 'pre_assessment_preview' : '';
    const mastery = hasPracticeEvidence ? practiceMastery : preMastery;
    if (!source || !Number.isFinite(mastery)) continue;
    if (source === 'pre_assessment_preview' && persisted[knowledgePoint.id]?.mastery != null) continue;
