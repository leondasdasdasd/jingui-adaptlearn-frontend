...
function persist(key, value) {
  if (!writeJson(key, value)) throw new Error('浏览器存储空间不足，学习记录未能保存');
  return value;
}
...
function sourceMetadata(session, options) {
  ...
  if (classroom) {
    ...
    return {
      source: 'classroom',
      authority: options.authority || (authoritative ? 'authoritative' : 'preview'),
      syncStatus: options.syncStatus || (authoritative ? 'synced' : 'pending'),
    };
  }
  return { source: 'self_study', authority: 'local_only', syncStatus: 'local_only' };
}
...
function questionRows(session) {
  const rows = [
    ...(session.preQuestions || []).map((question) => ({
      ...clone(question, {}), assessmentMode: 'pre',
    })),
    ...(session.postQuestions || []).map((question) => ({
      ...clone(question, {}), assessmentMode: 'post',
    })),
    ...Object.values(session.publishedContent?.knowledgePracticePools || {}).flat().map((question) => ({
      ...clone(question, {}), assessmentMode: 'post',
    })),
    ...(session.publishedContent?.compositeReviewPool || []).map((question) => ({
      ...clone(question, {}), assessmentMode: 'post',
    })),
  ];
  return [...new Map(rows.map((question) => [questionKey(question), question])).values()];
}
...
function attemptRows(session, questions) {
  const questionByKey = new Map(questions.map((question) => [questionKey(question), question]));
  return [
    ['pre', session.preAttempts || {}],
    ['post', session.postAttempts || {}],
  ].flatMap(([assessmentMode, attempts]) => Object.entries(attempts).map(([questionId, attempt]) => {
    const submittedAt = attempt?.submittedAt || '';
    return {
      ...clone(attempt, {}),
      attemptId: attempt?.clientSubmissionId || `${assessmentMode}:${questionId}:${submittedAt || 'latest'}`,
      assessmentMode,
      questionId,
      questionSnapshot: clone(questionByKey.get(`${assessmentMode}:${questionId}`), { id: questionId }),
    };
  }));
}
...
function historyIdFor(session, studentId, lesson, startedAt, options) {
  ...
  if (session.selection?.studentSessionId) return String(session.selection.studentSessionId);
  if (lesson.id && startedAt) return `learning-session:${studentId}:${lesson.id}:${startedAt}`;
  throw new Error('学习会话缺少 studentSessionId 或课时开始时间，无法建立稳定历史记录');
}

function summaryFor(record) {
  const correctCount = record.attempts.filter((attempt) => (
    attempt.correct === true || Number(attempt.scoreRatio) >= 1
  )).length;
  return {
...
export function upsertLearningSessionSnapshot(session, options = {}) {
  ...
  const historyId = historyIdFor(session, selection.studentId || identity.id, lesson, startedAt, options);
  const records = readStoredHistory();
  const existingIndex = records.findIndex((item) => item.historyId === historyId);
  const existing = existingIndex >= 0 ? records[existingIndex] : null;
  const questions = mergeBy(existing?.questions || [], questionRows(session), questionKey);
  const attempts = mergeBy(existing?.attempts || [], attemptRows(session, questions), (attempt) => attempt.attemptId);
  ...
  const record = {
    ...
    updatedAt,
    contentVersion: selection.contentVersion ?? existing?.contentVersion ?? null,
    contentVersionId: selection.contentVersionId || existing?.contentVersionId || '',
    sessionType: selection.sessionType || existing?.sessionType || 'lesson',
    knowledgePoints: clone(selection.knowledgePoints, existing?.knowledgePoints || []),
    questions,
    attempts,
    preAssessment: clone(session.preAssessment, existing?.preAssessment || null),
  ...
  if (record.status === 'completed' && !record.completedAt) record.completedAt = updatedAt;
...
export function readLearningHistory(filters = {}) {
  return readStoredHistory()
    ...
    .filter((record) => !filters.lessonId || record.lesson?.id === filters.lessonId)
    .filter((record) => !filters.source || record.source === filters.source)
    .filter((record) => !filters.status || record.status === filters.status)
    .sort((left, right) => String(right.completedAt || right.updatedAt || right.startedAt)
      .localeCompare(String(left.completedAt || left.updatedAt || left.startedAt)))
    .map(summaryFor);
}

export function readLearningHistoryDetail(historyId) {
  const record = readStoredHistory().find((item) => item.historyId === historyId);
...
function attemptTypeFor(attempt = {}) {
  ...
  if (attempt.questionSnapshot?.phase === 'review' || attempt.questionSnapshot?.purpose === 'POST') return 'composite';
  return 'practice';
}

function questionTypeFor(attempt = {}) {
  const rawType = attempt.questionSnapshot?.type
...
function flattenAttempt(record, attempt, index) {
  ...
  const knowledgePointById = Object.fromEntries((record.knowledgePoints || []).map((item) => [item.id, item]));
  const knowledgePointIds = snapshot.knowledgePointIds?.length
    ...
    : snapshot.primaryKnowledgePointId ? [snapshot.primaryKnowledgePointId] : [];
  return {
    ...clone(attempt, {}),
    historyId: record.historyId,
    studentId: record.studentId,
    ...
    knowledgePointIds,
    knowledgePoints: knowledgePointIds.map((id) => knowledgePointById[id]?.name || id),
  };
}

export function readLearningAttempts(filters = {}) {
  const records = readStoredHistory().filter((record) => !filters.studentId || record.studentId === filters.studentId);
  const from = filters.from ? new Date(filters.from).getTime() : null;
  const to = filters.to ? new Date(filters.to).getTime() : null;
  const attempts = records.flatMap((record) => record.attempts.map((attempt, index) => flattenAttempt(record, attempt, index)));
  return attempts
...
export function readLearningAttemptFacets(filters = {}) {
  ...
  const knowledgePoints = unique(attempts.flatMap((attempt) => attempt.knowledgePointIds)).map((id) => {
    const index = attempts.findIndex((attempt) => attempt.knowledgePointIds.includes(id));
