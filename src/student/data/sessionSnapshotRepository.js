...
function emitStatus(detail) {
  ...
  window.dispatchEvent(new CustomEvent(clientEvents.persistenceStatusChanged, { detail }));
}
...
function withoutCredentials(session = {}) {
  ...
  const contextSelection = session.learningFlow?.context?.selection
    ? { ...session.learningFlow.context.selection } : null;
  ...
  return {
    ...
    learningFlow: contextSelection ? {
      ...
      context: { ...session.learningFlow.context, selection: contextSelection },
    } : session.learningFlow,
  };
}
...
async function persistDraftImage(draft, credentials) {
  ...
  const media = await uploadStudentSessionMedia(credentials.sessionId, credentials.accessToken, {
    blob: parsed.blob,
...
async function hydrateDraftImage(draft, credentials) {
  const image = draft?.currentImage;
  if (!image?.mediaId || image.dataUrl) return draft;
  const blob = await getStudentSessionMedia(
    credentials.sessionId, credentials.accessToken, image.mediaId,
  );
  return { ...draft, currentImage: { ...image, dataUrl: await blobToDataUrl(blob) } };
}
...
export function rememberSnapshotRevision(sessionId, revision) {
  writeJson(storageKeys.classroomSnapshotSync, { sessionId, revision, updatedAt: new Date().toISOString() });
}
...
export async function buildSessionSnapshotPayload(session, route, credentials) {
  const draftEntries = await Promise.all(Object.entries(readAllQuizDrafts()).map(async ([id, draft]) => (
    [id, await persistDraftImage(draft, credentials)]
  )));
  return {
...
export async function hydrateSessionSnapshot(snapshot, credentials) {
  const payload = snapshot?.payload || {};
  const draftEntries = await Promise.all(Object.entries(payload.drafts || {}).map(async ([id, draft]) => (
    [id, await hydrateDraftImage(draft, credentials)]
  )));
  ...
  const postQuestions = Array.isArray(payload.session?.postQuestions)
    ? payload.session.postQuestions.map((question) => (
      question.phase === 'review' ? question : normalizeKnowledgePracticeQuestion(question)
    ))
  ...
  return {
    ...
    session: payload.session ? {
      ...
      learningFlow: contextSelection ? {
        ...
        context: { ...payload.session.learningFlow.context, selection: contextSelection },
      } : payload.session.learningFlow,
    } : null,
    drafts: Object.fromEntries(draftEntries),
  };
...
export async function loadSessionSnapshot(credentials, options = {}) {
  const snapshot = await getStudentSessionSnapshot(
    credentials.sessionId, credentials.accessToken, options,
  );
  rememberSnapshotRevision(credentials.sessionId, snapshot.revision);
  if (!snapshot.revision || !snapshot.payload?.session) return { ...snapshot, hydrated: null };
  return { ...snapshot, hydrated: await hydrateSessionSnapshot(snapshot, credentials) };
}

export async function saveSessionSnapshot({ session, route, credentials, expectedRevision }) {
  emitStatus({ status: 'saving' });
  try {
    const payload = await buildSessionSnapshotPayload(session, route, credentials);
    const saved = await putStudentSessionSnapshot(credentials.sessionId, credentials.accessToken, {
      expectedRevision,
      idempotencyKey: createClientId(),
      payload,
    });
    rememberSnapshotRevision(credentials.sessionId, saved.revision);
    emitStatus({ status: 'saved', revision: saved.revision, updatedAt: saved.updatedAt });
    return saved;
  } catch (error) {
    emitStatus({
      status: error.status === 409 ? 'conflict' : navigator.onLine ? 'error' : 'offline',
