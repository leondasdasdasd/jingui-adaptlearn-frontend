...
import { readJson, removeStoredValue, writeJson } from '../../shared/infrastructure/browserStorage.js';
...
function validSnapshotRecord(record) {
  return record?.contractVersion === SNAPSHOT_CONTRACT_VERSION && record.snapshot
    ? record : null;
}
  ...
  databasePromise = new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = () => {
      if (!request.result.objectStoreNames.contains(SNAPSHOT_STORE_NAME)) {
        request.result.createObjectStore(SNAPSHOT_STORE_NAME, { keyPath: 'persistenceKey' });
      }
    };
...
async function readIndexedDbRecord(persistenceKey) {
  ...
  return new Promise((resolve, reject) => {
    let request;
    try {
      request = database.transaction(SNAPSHOT_STORE_NAME, 'readonly')
...
async function writeIndexedDbRecord(persistenceKey, record, generation) {
  if (generation !== currentGeneration(persistenceKey)) return { status: 'superseded' };
  const database = await openSnapshotDatabase();
  if (generation !== currentGeneration(persistenceKey)) return { status: 'superseded' };
  return new Promise((resolve, reject) => {
    let transaction;
    try {
      ...
      transaction.objectStore(SNAPSHOT_STORE_NAME).put({ persistenceKey, ...record });
    } catch (error) {
      reject(error);
      return;
    }
...
async function deleteIndexedDbRecord(persistenceKey) {
  ...
  return new Promise((resolve) => {
    ...
    transaction.onabort = () => resolve({ status: 'error' });
  });
}
...
function removeLegacyDatabaseIndex(databaseName) {
  ...
  try {
    writeJson(TLDRAW_DATABASE_INDEX_KEY, names.filter((name) => name !== databaseName));
  } catch {
...
export async function readScratchPaperSnapshot(persistenceKey) {
  ...
  const journalRecord = validSnapshotRecord(readJson(journalKey(persistenceKey), null));
  if (journalRecord) {
  ...
  try {
    const indexedDbRecord = validSnapshotRecord(await readIndexedDbRecord(persistenceKey));
...
export function persistScratchPaperSnapshot(persistenceKey, snapshot) {
  if (!persistenceKey || !snapshot) return Promise.resolve({ status: 'ignored' });
  knownPersistenceKeys.add(persistenceKey);
  ...
  const pending = previous
    ...
    .catch(() => ({ status: journalPersisted ? 'journal-only' : 'memory-only' }));
  pendingWrites.set(persistenceKey, pending);
  void pending.finally(() => {
    if (pendingWrites.get(persistenceKey) === pending) pendingWrites.delete(persistenceKey);
  });
  return pending;
}

export function flushScratchPaperSnapshot(persistenceKey) {
  return pendingWrites.get(persistenceKey) || Promise.resolve({ status: 'idle' });
}
...
export async function clearScratchPaperSession(scope) {
  ...
  if (typeof persistenceKey !== 'string' || !persistenceKey) return { status: 'empty' };
  ...
  await (pendingWrites.get(persistenceKey) || Promise.resolve()).catch(() => undefined);
...
export async function clearAllScratchPaperSessions() {
  ...
  scopes.forEach((storageKey) => {
    const persistenceKey = readJson(storageKey, '');
    if (typeof persistenceKey === 'string' && persistenceKey) persistenceKeys.add(persistenceKey);
    try {
      removeStoredValue(storageKey);
    } catch {
  ...
  await Promise.all([...persistenceKeys].map(async (persistenceKey) => {
    await (pendingWrites.get(persistenceKey) || Promise.resolve()).catch(() => undefined);
    pendingWrites.delete(persistenceKey);
    await Promise.all([
      deleteIndexedDbRecord(persistenceKey),
