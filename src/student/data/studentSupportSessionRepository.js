...
import {
  readJson, readSessionValue, removeSessionValue, writeJson, writeSessionValue,
...
function desiredIdentity(selection, stored) {
  ...
  return {
    clientInstanceId,
    studentId: selection?.studentId || stored.studentId || `local-${clientInstanceId.slice(0, 8)}`,
    studentName: selection?.studentName || stored.studentName || '当前学生（本机）',
...
export async function ensureStudentSupportSession(selection) {
  ...
  if (stored.id && stored.accessToken
    && stored.studentId === identity.studentId && stored.studentName === identity.studentName) return stored;
  const identityKey = `${identity.clientInstanceId}:${identity.studentId}:${identity.studentName}`;
  if (pendingSession?.identityKey === identityKey) return pendingSession.promise;
  writeJson(storageKeys.studentSupportSession, { ...stored, ...identity });
  const promise = createStudentSupportSession(identity).then((credentials) => {
