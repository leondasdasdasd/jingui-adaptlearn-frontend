...
import QuestionReferenceAnswer, { hasReferenceAnswer } from './QuestionReferenceAnswer';
...
export default function QuestionReviewDisplay({
  ...
  const model = useMemo(() => {
    ...
    if (canUseQuestionPlatformPlayer(reviewQuestion)) {
      return { kind: 'player', ...adaptLegacyQuestion(reviewQuestion, studentAnswer) };
    }
  ...
  const renderKey = JSON.stringify([
    question.id, question.type, question.stem, question.options,
    studentAnswer, correctAnswer, analysis, model.kind,
  ]);
  const showAnswerStack = model.kind !== 'player';

  return (
    <div className="question-review-display">
      <MathContent as="div" className={`question-review-platform ${choiceLayoutClassName(question)}`} renderKey={renderKey}>
        {model.kind === 'player' && <QuestionPlayer
          ...
          value={model.draft}
        />}
        {model.kind === 'preview' && <QuestionPreview
          ...
          value={model.draft}
        />}
        {model.kind === 'fallback' && <MathContent as="h2" renderKey={question.stem}>{question.stem}</MathContent>}
      </MathContent>

      {showAnswerStack && <div className="question-review-answer-stack">
        <div>
          <span className="detail-section-label">你的答案</span>
          <MathContent as="p" renderKey={studentAnswerText}>{studentAnswerText}</MathContent>
        </div>
      </div>}
