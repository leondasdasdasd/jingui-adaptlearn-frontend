import { BookOpenCheck, CircleAlert, LoaderCircle, RefreshCw } from 'lucide-react';
...
export default function LearningResourceStatePage({
  ...
  const Icon = isLoading ? LoaderCircle : isUnpublished ? BookOpenCheck : CircleAlert;

  return (
    <AppShell title={lessonTitle} eyebrow="互动学习" onBack={onBack} compact>
      <section className={`learning-resource-state ${state}`} role={isLoading ? 'status' : 'alert'} aria-live="polite">
        ...
        {!isLoading && (
          <div className="learning-resource-actions">
            ...
            <button className="neutral-button" type="button" onClick={onBack}>返回学习列表</button>
          </div>
        )}
      </section>
    </AppShell>
