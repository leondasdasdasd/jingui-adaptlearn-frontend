...
export default function OpenMaicPage({
  ...
  const studentRuntimeUrl = useMemo(() => {
    ...
    return url.toString();
  }, [frameKey, runtimeCredentials?.accessToken, runtimeCredentials?.sessionId, runtimeUrl]);
  ...
  useEffect(() => {
    ...
    const errorTimer = window.setTimeout(() => {
      ...
      setFrameState('error');
      onRuntimeEventRef.current?.({ type: 'runtime_timeout', durationMs: 30_000 });
    }, 30_000);
    return () => { window.clearTimeout(slowTimer); window.clearTimeout(errorTimer); };
  }, [studentRuntimeUrl]);
  ...
  useEffect(() => {
    ...
    const receiveRuntimeEvent = (event) => {
      ...
      if (['runtime_loading', 'runtime_ready', 'runtime_error'].includes(type)) {
        if (data.source !== 'openmaic-classroom' || String(data.runtimeInstanceId) !== String(frameKey)) return;
        if (type === 'runtime_loading') {
          runtimeSettledRef.current = false;
          setFrameState('loading');
          return;
        }
        const durationMs = Number.isFinite(Number(data.durationMs)) ? Math.max(0, Number(data.durationMs)) : null;
        if (type === 'runtime_ready') {
          runtimeSettledRef.current = true;
          setFrameState('ready');
          setFrameError('');
          onRuntimeEventRef.current?.({ type, durationMs, loadMode: String(data.loadMode || 'unknown') });
        } else {
          runtimeSettledRef.current = true;
          setFrameState('error');
          setFrameError(String(data.message || '课堂内容加载失败').slice(0, 160));
          onRuntimeEventRef.current?.({ type, durationMs, code: String(data.code || 'CLASSROOM_LOAD_FAILED') });
        }
        return;
      }
      if (!['scene_entered', 'scene_completed', 'progress_changed', 'classroom_completed'].includes(type)) return;
          ...
          {frameState !== 'ready' && (
            <div className="openmaic-frame-state">
              <StatePanel
                ...
                title={frameState === 'slow' ? '课堂加载时间较长' : frameState === 'error' ? '课堂加载失败' : '正在进入互动课堂'}
                description={frameState === 'slow' ? '可以继续等待，或重新加载课堂' : frameState === 'error' ? frameError : undefined}
                action={['slow', 'error'].includes(frameState) ? <button className="neutral-button" type="button" onClick={() => setFrameKey((value) => value + 1)}>重新加载</button> : null}
              />
            </div>
          )}
          <iframe
            ...
            onLoad={() => onRuntimeEventRef.current?.({ type: 'runtime_document_loaded' })}
            onError={() => { runtimeSettledRef.current = true; setFrameError('课堂页面无法打开，请重新加载'); setFrameState('error'); }}
          />
        </div>
      </div>
    </AppShell>
