...
import { forgetClassStudentIdentity, readClassStudentIdentity } from '../student/data/classStudentIdentityRepository';
...
export default function AppShell({
  ...
  const displayName = identity?.studentName || session.selection?.studentName || '学生用户';
    ...
    <div className={`app-shell${shellClassName ? ` ${shellClassName}` : ''}`}>
      <header className={`app-header${headerClassName ? ` ${headerClassName}` : ''}`}>
        ...
        <div className="header-side header-side-right">
          {actions}
          {typeof progress === 'number' && <span className="header-progress">{progress}%</span>}
          <div className="student-user-menu" ref={userMenuRef}>
            ...
            >
              <span className="student-user-avatar" aria-hidden="true"><UserRound size={16} /></span>
              <span className="student-user-name">{displayName}</span>
              <ChevronDown className="student-user-chevron" size={15} aria-hidden="true" />
            </button>
            {userMenuOpen && (
              <div className="student-user-dropdown" role="menu" aria-label="账号菜单">
      ...
      <PersistenceNotice />
      <main className={`main-content${compact ? ' compact' : ''}${immersive ? ' immersive' : ''}`}>
        {children}
      </main>
      {helpContext && (
