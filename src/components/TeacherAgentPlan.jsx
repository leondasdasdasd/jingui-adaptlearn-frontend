import { CheckCircle2, Circle, Clock3, LoaderCircle, Play, RefreshCw, X, XCircle } from 'lucide-react';
...
function teacherFacingInstruction(value) {
  return String(value || '')
    .replace(/[（(]题目ID[：:]\s*[^）)\s]+[）)]/gi, '')
    .replace(/题目ID[：:]\s*[a-zA-Z0-9_-]+(?:__[a-zA-Z0-9_-]+)+/gi, '')
    .replace(/\s{2,}/g, ' ')
    .trim();
}

export default function TeacherAgentPlan({ plan, stepStatuses, runLink, executing, onConfirm, onCancel }) {
  if (!plan) return null;
  ...
  const backgroundSubmitted = statuses.some((status) => status === 'submitted');
  const canRetryReadOnly = hasFailed && plan.steps.length === 1 && plan.steps[0]?.kind === 'inspect_lesson';
    ...
    <section className="teacher-agent-plan" aria-label="教师智能体执行计划">
      <header>
        <div>
          ...
          <small>{executing
            ? plan.confirmationRequired ? '正在执行教师已确认的计划' : '正在执行只读检查，不会修改内容'
            : hasFailed
              ? plan.confirmationRequired ? '执行未完成；已完成步骤仍保留，未自动重放' : '只读检查未完成，没有修改课时内容'
              : backgroundSubmitted
                ? '后台任务已提交，刷新页面后仍会恢复进度和结果'
                : allCompleted
                  ? plan.confirmationRequired ? '执行已完成，结果已保存为待教师确认内容' : '只读检查已完成，没有修改课时内容'
                  : plan.confirmationRequired
                    ? containsBackgroundTask
                      ? '尚未开始；确认后提交后台任务并持续质检'
                      : '尚未开始；确认后按顺序执行并写入课时草稿'
                    : '准备执行只读检查，不会修改内容'}</small>
          {runLink?.runId && (
            <small className="teacher-agent-plan-run">
              <Clock3 aria-hidden="true" />后台任务 · {runStatusLabels[runLink.backendStatus] || '已提交'}
            </small>
          )}
        </div>
      </header>
        ...
        {plan.steps.map((step, index) => {
          ...
          return (
            <li key={step.id} data-status={status}>
              <span className="teacher-agent-plan-icon">{statusIcon(status)}</span>
              <div>
                <strong>{index + 1}. {toolLabels[step.kind] || '处理内容'}</strong>
                <p>{teacherFacingInstruction(step.instruction || plan.summary)}</p>
              </div>
            </li>
          );
        })}
      </ol>
      {!executing && (hasFailed || (plan.confirmationRequired && canConfirm)) && (
        <footer>
          <button className="teacher-agent-plan-secondary" type="button" onClick={onCancel}>
            <X aria-hidden="true" />{hasFailed ? '关闭计划' : '取消'}
          </button>
          {canConfirm && (
            <button className="teacher-agent-plan-primary" type="button" onClick={onConfirm}>
              <Play aria-hidden="true" />确认执行
            </button>
          )}
          {canRetryReadOnly && (
            <button className="teacher-agent-plan-primary" type="button" onClick={onConfirm}>
