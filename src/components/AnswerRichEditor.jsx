...
import {
  createEditor, Editor, Element as SlateElement, Node, Transforms,
} from 'slate';
...
export function readAnswerEditorText(nodes = []) {
  return nodes
    .filter((node) => !SlateElement.isElement(node) || node.type !== ANSWER_IMAGE_TYPE)
...
function withAnswerImages(editor) {
  ...
  editor.isVoid = (element) => element.type === ANSWER_IMAGE_TYPE || isVoid(element);
  editor.normalizeNode = (entry) => {
    const [node] = entry;
    if (Editor.isEditor(node)) {
      const hasParagraph = node.children.some((child) => (
        SlateElement.isElement(child) && child.type !== ANSWER_IMAGE_TYPE
      ));
      if (!hasParagraph) {
        Transforms.insertNodes(editor, paragraph(), { at: [node.children.length] });
        return;
      }
    }
    normalizeNode(entry);
...
function imageEntry(editor) {
  const entry = Editor.nodes(editor, {
    at: [],
    match: (node) => SlateElement.isElement(node) && node.type === ANSWER_IMAGE_TYPE,
  }).next();
...
function imageMaxWidth(imageElement) {
  ...
  const horizontalPadding = styles
    ...
    : 0;
  const width = Math.floor((editable?.clientWidth || 0) - horizontalPadding) - 10;
...
function AnswerImageElement({ attributes, children, disabled, element }) {
  ...
  const startResize = (event) => {
    ...
    const handlePointerMove = (moveEvent) => {
      ...
      latest = { width: nextWidth, height: Math.max(1, Math.round(nextWidth * ratio)) };
  ...
  const resizeWithKeyboard = (event) => {
    ...
    Transforms.setNodes(editor, {
      width: nextWidth,
      height: Math.max(1, Math.round(nextWidth * (currentHeight / currentWidth))),
    }, { at: path });
  };

  const setInitialSize = () => {
    const imageElement = imageRef.current;
    if (!imageElement || width || !imageElement.naturalWidth || !imageElement.naturalHeight) return;
    ...
    const scale = Math.min(
      ...
      drawing ? Infinity : MAX_INITIAL_IMAGE_HEIGHT / imageElement.naturalHeight,
    );
    Transforms.setNodes(editor, {
      width: Math.max(MIN_IMAGE_WIDTH, Math.round(imageElement.naturalWidth * scale)),
  ...
  return (
    <div {...attributes} className="answer-rich-image-node">
      ...
      <figure
        ...
        style={width ? { width, ...(height ? { aspectRatio: `${width} / ${height}` } : {}) } : undefined}
      >
...
const AnswerRichEditor = forwardRef(function AnswerRichEditor({
  ...
  const editor = useMemo(() => withAnswerImages(withHistory(withReact(createEditor()))), []);
  const initialValue = useMemo(() => createAnswerEditorValue(value, image), []); // eslint-disable-line react-hooks/exhaustive-deps
  const imageRef = useRef(image);
  imageRef.current = image;

  const replaceExternalText = useCallback((nextText) => {
    if (readAnswerEditorText(editor.children) === String(nextText || '')) return;
    ...
    Editor.withoutNormalizing(editor, () => {
      [...editor.children].reverse().forEach((node, indexFromEnd) => {
        if (SlateElement.isElement(node) && node.type === ANSWER_IMAGE_TYPE) return;
        const index = editor.children.length - 1 - indexFromEnd;
        Transforms.removeNodes(editor, { at: [index] });
      });
      const imagePosition = currentImage
        ? Math.max(0, Math.min(Number.isInteger(requestedPosition) ? requestedPosition : paragraphs.length, paragraphs.length))
        : paragraphs.length;
  ...
  useImperativeHandle(ref, () => ({ replaceText: replaceExternalText }), [replaceExternalText]);
  ...
  useEffect(() => {
    ...
    if (existing) {
      ...
      if (Object.entries(nextProperties).some(([key, nextValue]) => node[key] !== nextValue)) {
  ...
  const renderElement = useCallback((properties) => {
    if (properties.element.type === ANSWER_IMAGE_TYPE) {
      return <AnswerImageElement {...properties} disabled={disabled} />;
    }
    return <p {...properties.attributes} className="answer-rich-paragraph">{properties.children}</p>;
  ...
  const syncValue = () => {
    ...
    const nextLayout = {
      ...
      ...(Number(node.height) ? { height: Math.round(Number(node.height)) } : {}),
    };
    const currentLayout = imageRef.current.layout || {};
    if (nextLayout.position !== currentLayout.position
  ...
  const dropImage = (event) => {
    ...
    try {
      ...
      const targetPosition = Math.max(0, Math.min(range.anchor.path[0], editor.children.length - 1));
      Transforms.moveNodes(editor, { at: [sourcePosition], to: [targetPosition] });
      ReactEditor.focus(editor);
    } catch {
    ...
    <div className={`answer-rich-editor${disabled ? ' disabled' : ''}`}>
      <Slate editor={editor} initialValue={initialValue} onChange={syncValue}>
        <Editable
          ...
          onDragOver={(event) => {
            if (event.dataTransfer.types.includes(IMAGE_DRAG_TYPE)) event.preventDefault();
          }}
          onDrop={dropImage}
          onKeyDown={(event) => {
            if (disabled || !['Backspace', 'Delete'].includes(event.key) || !editor.selection) return;
