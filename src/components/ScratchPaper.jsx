...
class ScratchPaperBoardBoundary extends Component {
  ...
  render() {
    if (this.state.error) {
      return <div className={styles.loading} role="status">草稿纸正在恢复，请稍候...</div>;
    }
    return this.props.children;
  }
}

export default function ScratchPaper({ sessionScope, onActivity, triggerVariant = 'floating' }) {
          ...
          <div className={styles.actions}>
            <label className={styles.opacityControl} title="调整纸张透明度">
              ...
              <input
                ...
                value={paperOpacity}
                aria-label="纸张透明度"
                onChange={(event) => setPaperOpacity(Number(event.currentTarget.value))}
              />
      ...
      >
        <div className={styles.boardBody}>
          <div className={styles.canvasWrap}>
            <Suspense fallback={<div className={styles.loading}>草稿纸加载中...</div>}>
              {snapshotReady ? (
                <ScratchPaperBoardBoundary resetKey={persistenceKey}>
                  <HandwritingBoard
                    ...
                    onSnapshotChange={(snapshot) => {
                      volatileSnapshotRef.current = snapshot;
                      void persistScratchPaperSnapshot(persistenceKey, snapshot).then((result) => {
                        if (persistenceKeyRef.current !== persistenceKey) return;
                        if (result.status === 'memory-only') setPersistenceMode('memory');
                        else if (result.status === 'journal-only') setPersistenceMode('journal');
                        else if (result.status === 'persisted') setPersistenceMode('application');
                      });
                    }}
                    onStateChange={setBoardState}
                  />
                </ScratchPaperBoardBoundary>
              ) : <div className={styles.loading} role="status">草稿纸正在恢复...</div>}
            </Suspense>
          </div>
        </div>
      </DrawingBoardDialog>
