...
const viewportSize = () => ({
  width: typeof window === 'undefined' ? DEFAULT_SIZE.width + VIEWPORT_GUTTER * 2 : window.innerWidth,
  height: typeof window === 'undefined' ? DEFAULT_SIZE.height + VIEWPORT_GUTTER * 2 : window.innerHeight,
});
...
const clampOffset = (current, { width, height }) => {
  const viewport = viewportSize();
  const horizontalLimit = Math.max(0, (viewport.width - width) / 2 - VIEWPORT_GUTTER);
  const verticalLimit = Math.max(0, (viewport.height - height) / 2 - VIEWPORT_GUTTER);
  return {
...
const resizeFromRectangle = (rectangle, corner, delta) => {
  ...
  if (corner.endsWith('left')) {
    left = clamp(rectangle.left + delta.x, VIEWPORT_GUTTER, rectangle.right - minWidth);
  } else {
    right = clamp(
  ...
  if (corner.startsWith('top')) {
    top = clamp(rectangle.top + delta.y, VIEWPORT_GUTTER, rectangle.bottom - minHeight);
  } else {
    bottom = clamp(
...
export default function DrawingBoardDialog({
  ...
  const [offset, setOffset] = useState(() => initialOffsetFor(defaultPlacement, clampSize(defaultSize)));
  ...
  useEffect(() => {
    ...
    const nextSize = shouldResetLayout ? clampSize(defaultSize) : clampSize(sizeRef.current);
  ...
  }, [defaultPlacement, defaultSize, modal, open, resetLayoutOnOpen, returnFocusRef]);
  ...
  useEffect(() => {
    ...
    const handleViewportResize = () => {
      ...
      const nextSize = clampSize(returnedToDesktop ? defaultSize : sizeRef.current);
  ...
  const moveInteraction = (event) => {
    ...
    const next = resizeFromRectangle(interaction.rectangle, interaction.corner, delta);
  ...
  return createPortal(
    ...
    >
      <section
        ref={dialogRef}
        className={joinClassNames(styles.dialog, !modal && styles.nonModalDialog, className)}
        role="dialog"
      ...
      >
        ...
        {RESIZE_CORNERS.map((corner) => (
          <button
            key={corner}
            className={joinClassNames(styles.resizeHandle, styles[corner.replace('-', '')])}
            type="button"
            role="slider"
