...
export default function VoiceAnswerInput({ value, onChange, disabled }) {
  ...
  const recorder = useVoiceRecorder({ onPartial: applyText, onTranscription: applyText });
  const busy = recorder.connecting || recorder.transcribing;

  return (
    <div className="answer-voice-row">
      <button
        className={recorder.recording ? 'answer-voice-button recording' : 'answer-voice-button'}
        type="button"
        aria-busy={busy}
        disabled={disabled || busy}
        onClick={recorder.recording
