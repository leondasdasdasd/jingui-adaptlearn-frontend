...
export default function DrawingBoardInput({ disabled, image, onChange }) {
  ...
  const closeBoard = async () => {
    ...
    try {
      ...
      if (!answerContent?.inkDataUrl) throw new Error('画板内容生成失败，请重试');
      onChange({
  ...
  return (
    <>
      ...
      >
        ...
        {image?.source === 'photo'
          ? '在照片上标注'
          : image?.source === 'drawing' ? '重新绘制' : '画板作答'}
      </button>
        ...
        headerActions={(
          <div className="drawing-board-header-actions">
            ...
            <button
              ...
              onClick={closeBoard}
            >
              {exporting ? <Loader2 className="drawing-board-spinner" size={18} /> : <X size={20} />}
            </button>
          </div>
        )}
      >
        <div className="drawing-board-body">
          <div className="drawing-board-canvas-wrap">
            <Suspense fallback={<div className="drawing-board-loading">画板加载中…</div>}>
              <HandwritingBoard
                ref={boardRef}
          ...
          {error ? <p className="drawing-board-error" role="alert">{error}</p> : null}
        </div>
      </DrawingBoardDialog>
    </>
  );
