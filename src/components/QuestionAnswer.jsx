...
function PlatformQuestionAnswer({ question, value, onChange, disabled, grading }) {
  const { question: displayQuestion, showAnswer } = useMemo(
    () => prepareQuestionForGradingDisplay(question, grading),
    [question, grading],
  );
  const adapted = useMemo(() => adaptLegacyQuestion(displayQuestion, value), [displayQuestion, value]);
  const mathRenderKey = useMemo(() => JSON.stringify([
    displayQuestion.id, displayQuestion.stem, displayQuestion.options,
    displayQuestion.platformQuestion, showAnswer,
  ]), [displayQuestion, showAnswer]);
  return (
    <MathContent as="div" className={`question-platform-player ${choiceLayoutClassName(displayQuestion)}`} renderKey={mathRenderKey}>
      <QuestionPlayer
        disabled={disabled}
        locale="zh-CN"
        onResponseChange={(response) => onChange(readLegacyAnswer(response, question.type))}
...
function InlineFillBlank({
  index, question, value, mode, onModeChange, onChange, disabled,
  formulaTargeting, onFormulaTargeted,
}) {
  ...
  const resolvedMode = mode || (
    FORMULA_ANSWER_KINDS.has(String(question.answerKind || '').toLowerCase()) || looksLikeLatex(value)
      ? 'formula'
      : 'text'
  );
  ...
  useEffect(() => {
    if (resolvedMode !== 'formula') return undefined;
    let cancelled = false;
  ...
  useEffect(() => {
    ...
    if (!answerRoot || !measure) return undefined;
    const resize = () => {
      const containerWidth = answerRoot.closest('.inline-fill-answer')?.getBoundingClientRect().width || 360;
      const availableWidth = Math.max(64, containerWidth - 76);
      const contentWidth = Math.ceil(measure.getBoundingClientRect().width) + 22;
  ...
  return (
    ...
    >
      <span className="inline-fill-input-shell" style={{ '--inline-fill-width': `${inputWidth}px` }}>
        {resolvedMode === 'formula' ? (
          <math-field
            ref={mathFieldRef}
            aria-label={formulaTargeting ? `选择${label}输入公式` : `${label}公式`}
        ...
        <span className="inline-fill-measure" ref={measureRef} aria-hidden="true">
          {resolvedMode === 'formula' && value
            ? <MathContent as="span" renderKey={value}>{`\\(${value}\\)`}</MathContent>
            : value || '答'}
        </span>
      </span>
    </span>
...
function InlineFillAnswer({
  question, value, onChange, inputModes = [], onInputModesChange, disabled,
  formulaTargeting, onFormulaTargeted,
}) {
  const parts = String(question.stem || '').split(/_{2,}/g);
  const blankCount = Math.max(1, parts.length - 1);
  const values = Array.isArray(value)
    ? Array.from({ length: blankCount }, (_, index) => String(value[index] || ''))
    : Array.from({ length: blankCount }, (_, index) => index === 0 ? String(value || '') : '');
  ...
  return (
    <div className="inline-fill-answer" aria-label="题干内填空">
      {parts.map((part, index) => (
        <span className="inline-fill-fragment" key={`${question.id}-fragment-${index}`}>
          {part && <MathContent as="span" renderKey={`${question.id}-${index}-${part}`}>{part}</MathContent>}
          {index < blankCount && (
            <InlineFillBlank
              disabled={disabled}
              index={index}
              ...
              onModeChange={(nextMode) => {
                const nextModes = Array.from({ length: blankCount }, (_, modeIndex) => (
                  modeIndex === index ? nextMode : inputModes[modeIndex] || 'text'
                ));
...
export default function QuestionAnswer({
  question, value, onChange, image, onImageChange, disabled, grading,
  fillInputModes = [], onFillInputModesChange = () => {},
  formulaTargeting = false, onFormulaTargeted = () => {},
}) {
  if (question.type === 'fill_blank') {
    return <InlineFillAnswer
      question={question}
      value={value}
      onChange={onChange}
  ...
  if (canUseQuestionPlatformPlayer(question)) {
    return <PlatformQuestionAnswer question={question} value={value} onChange={onChange} disabled={disabled} grading={grading} />;
  }

  if (['single_choice', 'multiple_choice'].includes(question.type)) {
    const multiple = question.type === 'multiple_choice';
    const selectedValues = multiple ? (value || []) : [];
    const visibleAnswer = grading?.showAnswer ? grading.correctAnswer : question.answer;
    return (
      <div className={`option-list ${choiceLayoutClassName(question)}`} role={multiple ? 'group' : 'radiogroup'} aria-label="答案选项">
        {question.options.map((option) => {
          const active = multiple ? selectedValues.includes(option.id) : value === option.id;
          const correctAnswers = Array.isArray(visibleAnswer) ? visibleAnswer : [visibleAnswer];
          ...
          return (
            <button
              ...
              onClick={() => {
                ...
                else onChange(active ? selectedValues.filter((id) => id !== option.id) : [...selectedValues, option.id]);
              }}
            >
              <span className={multiple ? 'option-key square' : 'option-key'}>
                {correctOption ? <Check size={17} /> : option.id}
              </span>
              <MathContent as="span" renderKey={option.text}>{option.text}</MathContent>
            </button>
          );
        })}
      </div>
  ...
  return (
    <div className="written-answer subjective">
      ...
      <div className="answer-composer" role="group" aria-labelledby={`answer-label-${question.id}`}>
        <AnswerRichEditor
          key={question.id}
          disabled={disabled}
          image={image}
        ...
        <div className="answer-tools" aria-label="作答工具">
          <VoiceAnswerInput value={value} onChange={onChange} disabled={disabled} />
          <PhotoAnswerInput image={image} onChange={onImageChange} disabled={disabled} />
          <DrawingBoardInput image={image} onChange={onImageChange} disabled={disabled} />
        </div>
      </div>
    </div>
  );
