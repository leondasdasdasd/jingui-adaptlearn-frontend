...
function groupModules(modules, task) {
  ...
  const knowledgeQuestions = modules.filter((module) => module.kind === 'knowledge-questions');
  const review = modules.filter((module) => module.kind === 'composite-review');
  const compositeClassroom = modules.filter((module) => module.kind === 'composite-classroom');
  const knowledgeClassrooms = modules.filter((module) => module.kind === 'knowledge-classroom');
  return [
    {
      id: 'pre', label: '课前测验', items: pre,
    },
    {
      id: 'knowledge-questions', label: '单点题池', items: knowledgeQuestions,
    },
    {
      id: 'review', label: '综合练习', items: review,
    },
    {
      id: 'composite-classroom', label: '复合 MAIC 课堂', items: compositeClassroom,
    },
    {
      id: 'knowledge-classrooms', label: '单点 MAIC 课堂', items: knowledgeClassrooms,
    },
  ].filter((group) => group.items.length).map((group) => ({
    ...group,
    status: combinedStatus(group.items, task),
    readyCount: group.items.filter((item) => moduleStatus(item, task) === 'ready').length,
    generatingCount: group.items.filter((item) => moduleStatus(item, task) === 'generating').length,
    durationSeconds: Math.max(0, ...group.items.map((item) => Number(task.moduleProgress?.[item.id]?.durationSeconds || 0))),
    queuePosition: Math.min(
      ...group.items.map((item) => Number(task.moduleProgress?.[item.id]?.queuePosition || Number.POSITIVE_INFINITY)),
    ),
    activeItem: group.items.find((item) => moduleStatus(item, task) === 'generating'),
  }));
}
...
export default function LessonContentGenerationPanel({
  ...
  const running = ['generating', 'validating', 'repairing'].includes(task.phase);
  ...
  const statusMessage = hasIssues && task.phase === 'ready'
  ...
  const statusLabel = publishing
    ...
    : running
      ...
      : processStatus === 'completed'
        ? task.phase === 'published' ? '已发布' : '内容已就绪'
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="teacher-agent-task-status" role="status">{statusLabel}</span>
        <ChevronDown className="teacher-agent-task-chevron" aria-hidden="true" />
      </button>

      {expanded && (
        <div className="teacher-agent-task-body">
          {statusMessage && <p className="teacher-agent-task-message">{statusMessage}</p>}
          <div
          ...
          >
            <span style={{ width: `${groups.length ? (readyCount / groups.length) * 100 : 0}%` }} />
          </div>

          <div className="teacher-agent-task-groups" aria-label="整课生成子任务">
            {groups.map((group) => (
              <div className={`teacher-agent-task-group ${group.status}`} key={group.id}>
                <span className="teacher-agent-task-group-icon" aria-hidden="true">
                  {group.status === 'ready'
                ...
                <span className="teacher-agent-task-group-meta">
                  {group.durationSeconds > 0 && <><Clock3 size={13} />{group.durationSeconds}s</>}
                  {group.status === 'queued'
                    ? Number.isFinite(group.queuePosition) ? `排队第 ${group.queuePosition} 位` : '排队中'
                    : group.status === 'failed' ? '需重试' : ''}
                </span>
              </div>
            ))}
          </div>
          ...
          {task.issues?.length > 0 && (
            <div className="teacher-agent-task-issues" role="status">
              <strong><AlertTriangle size={15} />还有 {task.issues.length} 项需要处理</strong>
              <ul>{task.issues.map((issue, index) => <li key={`${issue.code || index}-${issue.message}`}>{issue.message}</li>)}</ul>
            </div>
          )}
        </div>
      )}
