import { choiceOptionText, questionChoiceOptions } from '../shared/question-platform/choiceLayout';
...
function choiceAnswerRows(question, answer) {
  ...
  return values.map((value, index) => {
    ...
    const match = options.map((option, optionIndex) => ({
      id: String(typeof option === 'string'
        ? String.fromCharCode(65 + optionIndex)
        : option?.id || String.fromCharCode(65 + optionIndex)),
      option,
    })).find(({ id, option }) => (
      id === normalizedValue || choiceOptionText(option) === normalizedValue
    ));
...
function ReferenceSection({ title, children }) {
  return (
    <section className="question-review-reference-section" aria-label={title}>
      <header><span aria-hidden="true" /><h3>{title}</h3></header>
...
export default function QuestionReferenceAnswer({
  ...
  if (!resolvedCorrectAnswer && !hasCorrectAnswerText && !hasReferenceAnswer(analysis)) return null;

  return (
    <div className="question-review-reference-list">
      {(resolvedCorrectAnswer || hasCorrectAnswerText) && <ReferenceSection title={correctAnswerLabel}>
        {correctChoiceRows.length > 0
          ? <div className="question-review-reference-options">{correctChoiceRows.map((option) => (
              <div className="question-review-reference-option" key={option.key}>
                <span>{option.id}</span>
                <MathContent as="div" renderKey={`${option.key}-${option.text}`}>{option.text}</MathContent>
              </div>
            ))}</div>
          : <MathContent as="p" renderKey={correctAnswerText}>{correctAnswerText}</MathContent>}
      </ReferenceSection>}
      {hasReferenceAnswer(analysis) && <ReferenceSection title="答案解析">
        <MathContent as="p" renderKey={analysis}>{analysis}</MathContent>
      </ReferenceSection>}
    </div>
  );
}
