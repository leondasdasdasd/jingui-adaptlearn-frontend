...
const structuralMathPattern = new RegExp(`\\\\(?:frac|binom)\\b|\\\\begin\\s*\\{(?:${structuralMathEnvironmentPattern})\\}`);
const explicitMathStylePattern = /\\(?:displaystyle|textstyle|scriptstyle|scriptscriptstyle|dfrac|tfrac|cfrac)\b/;
...
function bareMathEnd(text, index) {
  const environmentMatch = text.slice(index).match(new RegExp(`^\\\\begin\\s*\\{(${structuralMathEnvironmentPattern})\\}`));
  if (environmentMatch) {
    const environmentEnd = new RegExp(`\\\\end\\s*\\{${environmentMatch[1]}\\}`);
    const closingMatch = text.slice(index + environmentMatch[0].length).match(environmentEnd);
    if (!closingMatch || closingMatch.index == null) return -1;
    return index + environmentMatch[0].length + closingMatch.index + closingMatch[0].length;
  }
  ...
  for (let argument = 0; argument < command.bracedArguments; argument += 1) {
    cursor = consumeBalanced(text, cursor, '{', '}');
    if (cursor < 0) return -1;
    if (argument < command.bracedArguments - 1) cursor = skipWhitespace(text, cursor);
  }
  return cursor;
}
...
export function normalizeBareMath(value) {
  ...
  if (!/\\(?:dfrac|tfrac|frac|sqrt)\b/.test(text) && !new RegExp(`\\\\begin\\s*\\{(?:${structuralMathEnvironmentPattern})\\}`).test(text)) return text;
  ...
  while (cursor < text.length) {
    ...
    if (closingDelimiter) {
      const openingLength = text.startsWith('$$', cursor) ? 2 : text.startsWith('\\[', cursor) || text.startsWith('\\(', cursor) ? 2 : 1;
...
function mathDelimiterAt(text, index) {
  if (text.startsWith('$$', index)) return { left: '$$', right: '$$', display: true };
  if (text.startsWith('\\[', index)) return { left: '\\[', right: '\\]', display: true };
  if (text.startsWith('\\(', index)) return { left: '\\(', right: '\\)', display: false };
  if (text[index] === '$' && text[index - 1] !== '\\') return { left: '$', right: '$', display: false };
  return null;
}

function needsDisplayStyle(expression) {
  return structuralMathPattern.test(expression) && !explicitMathStylePattern.test(expression);
}
...
export function normalizeMathForRendering(value) {
  ...
  while (cursor < text.length) {
    ...
    const renderedExpression = !delimiter.display && needsDisplayStyle(expression)
