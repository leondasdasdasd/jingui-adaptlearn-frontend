export async function getLessonGenerationTasks(lessonIds, { signal } = {}) {
  ...
  const response = await fetch(`/api/generation-tasks?lessonIds=${encodeURIComponent(ids.join(','))}`, { signal });
...
export function databaseGenerationState(tasks = []) {
  ...
  const newest = [...tasks].sort((a, b) => String(b.createdAt).localeCompare(String(a.createdAt)))[0];
  const runId = newest?.runId || newest?.payload?.generationRunId;
  const runTasks = runId
    ? tasks.filter((task) => (task.runId || task.payload?.generationRunId) === runId)
    : [newest];
