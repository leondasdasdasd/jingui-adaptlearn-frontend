...
export async function gradeWrittenAnswer({
  question, contentVersionId, answerText, imageDataUrl, attemptStage = 'initial',
  ...
  const response = await fetch('/api/answers/grade', {
    ...
    body: JSON.stringify({
      contentVersionId, questionId: question.id, purpose, answerText, imageDataUrl, attemptStage,
      priorFormalGradeReceipt,
    }),
  });
...
export async function loadAnswerReviews(contentVersionId, questionIds, {
  studentSessionId = '', accessToken = '',
} = {}) {
  const ids = [...new Set((Array.isArray(questionIds) ? questionIds : []).map(String).filter(Boolean))];
  if (!contentVersionId || !ids.length) return {};
  if (!studentSessionId || !accessToken) throw new Error('复盘需要当前学习会话凭据');
  const batches = [];
  for (let index = 0; index < ids.length; index += 100) batches.push(ids.slice(index, index + 100));
  const bodies = await Promise.all(batches.map(async (batch) => {
    const response = await fetch('/api/answers/review', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${accessToken}` },
      body: JSON.stringify({ contentVersionId, questionIds: batch, studentSessionId }),
    });
    const body = await response.json().catch(() => ({}));
    if (!response.ok) throw new Error(body.message || '参考答案加载失败');
    return body;
  }));
  return Object.fromEntries(bodies.flatMap((body) => body.items || [])
...
export function assessAnswerQuality(question, answerText) {
  ...
  if (/(随便|乱写|瞎写|蒙的|测试流程|开发工程师|无关答案|asdf|test)/i.test(answer)) {
    return {
      quality: 'off_task',
      message: '这次答案还不能用于判断。请回到题目，写下一个相关条件、公式或步骤。',
    };
  }
  ...
  if (question.type === 'fill_blank' && /^(是的|不是|好的|对|错|随便|哈哈+)$/.test(compact)) {
    return {
      quality: 'off_task',
...
export function gradeObjectiveAnswer(question, answer, { revealSolution = false } = {}) {
  ...
  return {
    ...
    scoreRatio,
    correct,
    feedback: correct ? '回答正确' : scoreRatio > 0 ? '部分正确，已按正确部分计分' : quality.message || '再检查一下关键条件和计算过程',
    strengths: correct ? ['结论正确'] : scoreRatio > 0 ? ['部分判断正确'] : [],
    improvements: correct ? [] : ['核对遗漏项和多选项'],
    recognizedAnswer: Array.isArray(answer) ? answer.join('、') : String(answer || ''),
    ...
    ...((revealSolution && !correct) ? { analysis: question.analysis || '' } : {}),
  };
}
...
export async function gradeAnswerWithFallback({
  ...
  if ([
    'multiple_choice', 'single_choice', 'fill_blank', 'judgement', 'ordering',
    'classification', 'matching', 'line_connect', 'text_marker', 'word_builder',
  ].includes(question?.type)) {
    if (contentVersionId) {
      try {
        return await gradeWrittenAnswer({
          question, contentVersionId, answerText, imageDataUrl, attemptStage, priorFormalGradeReceipt,
        });
      } catch (error) {
    ...
    if (question?.answer !== undefined) {
      return gradeObjectiveAnswer(question, answerText, { revealSolution: attemptStage === 'correction' });
    }
