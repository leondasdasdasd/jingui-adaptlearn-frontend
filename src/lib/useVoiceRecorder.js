...
import { speechStreamUrl, voiceInputEnvironment } from './voiceInputEnvironment';
...
function downsampleSamples(source, sourceRate) {
  ...
  for (let outputIndex = 0; outputIndex < outputLength; outputIndex += 1) {
    ...
    let total = 0;
    for (let sourceIndex = start; sourceIndex < end; sourceIndex += 1) total += source[sourceIndex];
...
export default function useVoiceRecorder({ onPartial, onTranscription }) {
  ...
  const closeSocket = (recorder) => {
    if (recorder?.socket && recorder.socket.readyState < WebSocket.CLOSING) recorder.socket.close();
  };

  const connectStream = (recorder) => new Promise((resolve, reject) => {
    const socket = new WebSocket(speechStreamUrl(window.location));
    socket.binaryType = 'arraybuffer';
    recorder.socket = socket;
    const timeoutId = window.setTimeout(() => reject(new Error('实时转写连接超时')), 15000);
    ...
    socket.onmessage = (event) => {
      ...
      if (message.type === 'ready') {
        ...
        recorder.latestText = String(message.text || recorder.latestText || '');
        onPartial?.(recorder.latestText);
        recorder.resolveFinal?.(recorder.latestText);
      } else if (message.type === 'error') {
        recorder.streamError = String(message.message || '实时转写暂时中断');
        if (!recorder.streaming) reject(new Error(recorder.streamError));
        else recorder.rejectFinal?.(new Error(recorder.streamError));
      }
    };
    socket.onerror = () => {
      window.clearTimeout(timeoutId);
      const streamError = new Error('实时转写暂时不可用');
      if (!recorder.streaming) reject(streamError);
      else recorder.rejectFinal?.(streamError);
    };
  });
  ...
  const stopRecording = async () => {
    ...
    try {
      ...
      if (recorder.streaming && recorder.socket?.readyState === WebSocket.OPEN) {
        ...
        text = await Promise.race([
          finalPromise,
          new Promise((_, reject) => window.setTimeout(() => reject(new Error('实时转写等待超时')), 12000)),
        ]);
      }
      if (!text) {
        const pcm = downsampleChunks(recorder.chunks, recorder.sampleRate);
        if (pcm.length < TARGET_SAMPLE_RATE * 2 * 0.35) throw new Error('录音太短，请再说一次');
  ...
  const startRecording = async () => {
    ...
    setError('');
    setConnecting(true);
    const recorder = { chunks: [], streaming: false, latestText: '', socket: null };
    ...
    try {
      ...
      recorder.timeoutId = window.setTimeout(() => stopRef.current?.(), MAX_RECORDING_MS);
      setElapsedSeconds(0);
      setRecording(true);
    } catch (requestError) {
      await releaseAudio(recorder);
