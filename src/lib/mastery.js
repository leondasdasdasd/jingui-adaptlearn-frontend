...
function attemptScoreRatio(question, attempt = {}) {
  if (Number.isFinite(Number(attempt.scoreRatio))) return clamp(attempt.scoreRatio);
...
function evidenceRows(questions = [], attempts = {}, knowledgePointId, defaultSource = 'PRE') {
  return questions
    .map((question, index) => ({ question, attempt: attempts[question.id], index }))
    .filter(({ question, attempt }) => attempt && (
    ...
    .sort((a, b) => new Date(a.attempt.submittedAt || 0) - new Date(b.attempt.submittedAt || 0) || a.index - b.index)
    .flatMap(({ question, attempt }) => {
      const map = Array.isArray(question.knowledgeEvidenceMap) ? question.knowledgeEvidenceMap : [];
      const mapped = map.find((item) => item.knowledgePointId === knowledgePointId);
      const weight = mapped?.weight ?? question.knowledgePointWeights?.[knowledgePointId] ?? (
        question.primaryKnowledgePointId === knowledgePointId || question.knowledgePointIds?.[0] === knowledgePointId ? 1 : 0.3
      );
      if (Number(weight) <= 0) return [];
      return [{
        questionId: question.id,
        source: String(question.source || question.purpose || defaultSource).toUpperCase(),
        ...
        role: mapped?.role ? String(mapped.role).toUpperCase() : question.primaryKnowledgePointId === knowledgePointId || question.knowledgePointIds?.[0] === knowledgePointId ? 'PRIMARY' : 'SECONDARY_OBSERVED',
        knowledgePointWeight: Number(weight),
        independence: mapped?.independence ?? question.independence ?? attempt.independence ?? 1,
        itemQuality: question.itemQuality ?? attempt.itemQuality ?? 1,
        gradingConfidence: attempt.gradingConfidence ?? question.gradingConfidence ?? 1,
...
export function calculateUnifiedMastery(questions = [], attempts = {}, knowledgePoints = [], {
  source = 'PRE',
  priorMastery = {},
} = {}) {
  const result = Object.fromEntries(knowledgePoints.map((kp) => {
    ...
    const correctRate = answered
      ? Math.round((evidence.reduce((sum, row) => sum + row.scoreRatio, 0) / answered) * 100)
      : null;
    return [kp.id, {
      ...item,
      ...(unassessed ? { mastery: null, confidence: 0, lowerBound: null, upperBound: null, status: 'NOT_ASSESSED' } : {}),
      masterySource: 'U1',
      algorithmVersion: U1_ALGORITHM_VERSION,
      correctRate,
      evidenceCount: item.evidenceCount,
      evidenceWeight: Number(evidence.reduce((sum, row) => sum + row.knowledgePointWeight, 0).toFixed(2)),
      status: item.status,
    }];
  }));
  return result;
...
export function calculatePreMastery(questions, attempts, knowledgePoints) {
  return calculateUnifiedMastery(questions, attempts, knowledgePoints, { source: 'PRE' });
}
...
export function isPreAssessmentComplete(questions = [], attempts = {}) {
  return questions.length > 0 && questions.every((question) => {
    ...
    return Boolean(attempt?.submittedAt)
      && (attempt.skipped === true || Number.isFinite(Number(attempt.scoreRatio))
        || (Number.isFinite(Number(attempt.score)) && Number.isFinite(Number(attempt.maxScore))));
  });
}

export function overallPreAssessmentMastery(result = {}) {
  const values = Object.values(result)
    .filter((item) => Number(item.evidenceCount || 0) > 0 && Number.isFinite(Number(item.mastery)))
...
export function overallPreAssessmentCorrectRate(questions = [], attempts = {}) {
  ...
  const total = answered.reduce((sum, question) => sum + attemptScoreRatio(question, attempts[question.id]), 0);
  return Math.round((total / answered.length) * 100);
}

export function calculatePostMastery(questions, attempts, knowledgePoints, preMastery = {}) {
  const result = calculateUnifiedMastery(questions, attempts, knowledgePoints, {
    source: 'POST',
    priorMastery: preMastery,
  });
  return Object.fromEntries(knowledgePoints.map((kp) => {
    ...
    return [kp.id, {
      ...
      improvement: Number.isFinite(pre) ? Math.round((item.mastery - pre) * 100) / 100 : null,
    }];
  }));
}

export function overallMastery(result) {
  const values = Object.values(result).filter((item) => item.mastery != null && item.evidenceCount > 0);
  return values.length ? Math.round(values.reduce((sum, item) => sum + Number(item.mastery), 0) / values.length) : null;
}
...
export function previewU1Update({ question, attempt, previous = {}, knowledgePointId }) {
  ...
  const [trace] = [applyU1Evidence(state, {
    questionId: question.id,
    source: question.source || question.purpose || 'PRACTICE',
    difficulty: question.difficulty,
    scoreRatio: attemptScoreRatio(question, attempt),
    role: question.primaryKnowledgePointId === knowledgePointId || question.knowledgePointIds?.[0] === knowledgePointId ? 'PRIMARY' : 'SECONDARY_OBSERVED',
    knowledgePointWeight: question.knowledgePointWeights?.[knowledgePointId] ?? 1,
    itemQuality: question.itemQuality ?? 1,
    gradingConfidence: attempt.gradingConfidence ?? 1,
    hintUsed: Boolean(attempt.hintUsed),
    novelty: question.novelty || 'NEW',
  })];
  return { ...trace, masteryBefore: before.mastery, confidenceBefore: before.confidence, isPreview: true };
}
