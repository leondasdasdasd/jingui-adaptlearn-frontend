...
export const difficultyMeta = Object.freeze({
  D1: { label: 'D1 基础识别', shortLabel: '基础识别', className: 'basic' },
  D2: { label: 'D2 直接理解', shortLabel: '直接理解', className: 'basic' },
  D3: { label: 'D3 标准应用', shortLabel: '标准应用', className: 'standard' },
  D4: { label: 'D4 变式综合', shortLabel: '变式综合', className: 'advanced' },
  D5: { label: 'D5 迁移应用', shortLabel: '迁移应用', className: 'advanced' },
  // Existing published questions can still contain numeric difficulty. Keep
  // this read-only compatibility map while new content uses D1–D5.
  1: { label: 'D1 基础识别', shortLabel: '基础识别', className: 'basic' },
  2: { label: 'D2 直接理解', shortLabel: '直接理解', className: 'basic' },
  3: { label: 'D3 标准应用', shortLabel: '标准应用', className: 'standard' },
  4: { label: 'D4 变式综合', shortLabel: '变式综合', className: 'advanced' },
  5: { label: 'D5 迁移应用', shortLabel: '迁移应用', className: 'advanced' },
});
...
export function normalizeDifficulty(value, fallback = 'D3') {
  ...
  if (Number.isFinite(numeric)) return DIFFICULTY_LEVELS[Math.max(0, Math.min(4, Math.round(numeric) - 1))] || fallback;
  return fallback;
}
...
export function buildInterventionEvidence({
  questions = [], attempts = {}, knowledgePointId, limit = 3,
}) {
  return questions
...
function scoreRatioForAttempt(attempt = {}) {
  if (attempt?.skipped || attempt?.disposition === 'SKIPPED_DONT_KNOW') return null;
  ...
  return Number.isFinite(score) && Number.isFinite(maxScore) && maxScore > 0
    ? Math.max(0, Math.min(1, score / maxScore)) : null;
}

export function recentAttemptSummary(attempts = [], limit = RECENT_ATTEMPT_LIMIT) {
  const valid = attempts
    .map((attempt, index) => ({ attempt, index, ratio: scoreRatioForAttempt(attempt) }))
    .filter((item) => item.ratio != null)
    .sort((left, right) => (
      String(right.attempt.submittedAt || '').localeCompare(String(left.attempt.submittedAt || ''))
...
export function targetDifficultyFromSignals({ mastery = 0, recentAttempts = [], limit = RECENT_ATTEMPT_LIMIT } = {}) {
  const masteryValue = Math.max(0, Math.min(100, Number(mastery) || 0));
  const recent = recentAttemptSummary(recentAttempts, limit);
  const recentRate = recent.averageScoreRatio == null ? null : recent.averageScoreRatio * 100;
  ...
  const effectiveAbility = recentRate == null || recent.count < 3
    ? masteryValue
    : masteryValue * 0.6 + recentRate * 0.4;
  let targetDifficulty = targetDifficultyFromMastery(effectiveAbility);
  const latestTwo = recent.attempts.slice(0, 2)
    .map(scoreRatioForAttempt)
    .filter((ratio) => ratio != null);
  const recentWeakStreak = latestTwo.length === 2 && latestTwo.every((ratio) => ratio < 0.5);
  if (recentWeakStreak && difficultyRank(targetDifficulty) > difficultyRank('D3')) targetDifficulty = 'D3';
  return {
    targetDifficulty,
    ...
    reason: recentRate == null || recent.count < 3
...
export function createAdaptiveState(
  questions, mode, startingMastery = {}, recentAttemptsByKnowledgePoint = {}, selectionSeed = '',
) {
  if (mode !== 'post') {
  ...
  const targetDecisions = Object.fromEntries(kpIds.map((kpId) => {
    const signal = targetDifficultyFromSignals({
      mastery: startingMastery[kpId]?.mastery || 0,
      recentAttempts: recentAttemptsByKnowledgePoint[kpId] || startingMastery[kpId]?.recentAttempts || [],
    });
    return [kpId, signal];
  }));
  const targetByKp = Object.fromEntries(kpIds.map((kpId) => [kpId, targetDecisions[kpId].targetDifficulty]));
  const firstKp = kpIds[0];
  const first = questions
    .filter((question) => questionKnowledgePointId(question) === firstKp)
    .sort((a, b) => questionDistance(a, targetByKp[firstKp]) - questionDistance(b, targetByKp[firstKp])
      || questionTieBreak(a, selectionSeed) - questionTieBreak(b, selectionSeed))[0];
  return {
...
export function evaluateKnowledgePoint({
  questions, attempts, knowledgePointId, mastery = null,
  ...
  if (answered.length < 1) return { status: 'continue', answered: answered.length };

  const hasU1Estimate = mastery !== null && mastery !== undefined && mastery !== ''
  ...
  const reachedMasteryTarget = hasU1Estimate
    && Number(mastery) >= MASTERY_THRESHOLD_RATIO * 100;
  let correctStreak = 0;
  for (let index = answered.length - 1; index >= 0; index -= 1) {
    if (Number(answered[index].attempt.scoreRatio || 0) < PRACTICE_PASS_RATIO) break;
    correctStreak += 1;
  }
  ...
  const threeWeak = lastThree.length === 3
    && lastThree.every(({ attempt }) => Number(attempt.scoreRatio || 0) < PRACTICE_PASS_RATIO);
...
export function selectNextAdaptiveQuestion({
  ...
  const sameKp = completed.has(currentKp)
    ? []
    : unanswered.filter(
      (question) => question.phase !== 'review' && questionKnowledgePointId(question) === currentKp,
    );
  const nextKnowledgeQuestion = unanswered.find(
    (question) => question.phase !== 'review' && !completed.has(questionKnowledgePointId(question)),
  );
  const candidates = sameKp.length
    ? sameKp
    : nextKnowledgeQuestion
      ? unanswered.filter(
        (question) => question.phase !== 'review'
          && questionKnowledgePointId(question) === questionKnowledgePointId(nextKnowledgeQuestion),
      )
  ...
  return [...candidates].sort((a, b) => questionDistance(a, target) - questionDistance(b, target)
    || questionTieBreak(a, selectionSeed) - questionTieBreak(b, selectionSeed))[0];
}
